# XClaw iOS (Super Alpha)

This iPhone app is super-alpha and internal-use only. It connects to an XClaw Gateway as a `role: node`.
...
- Beta release uses canonical `ai.xclaw.client*` bundle IDs through a temporary generated xcconfig in `apps/ios/build/BetaRelease.xcconfig`.
- Beta release also switches the app to `XClawPushTransport=relay`, `XClawPushDistribution=official`, and `XClawPushAPNsEnvironment=production`.
...
- `XCLAW_PUSH_RELAY_BASE_URL=https://relay.example.com`
...
- `apps/ios/Sources/XClaw.entitlements` sets `aps-environment` to `development`.
- APNs token registration to gateway happens only after gateway connection (`push.apns.register`).
- Local/manual builds default to `XClawPushTransport=direct` and `XClawPushDistribution=local`.
- Your selected team/profile must support Push Notifications for the app bundle ID you are signing.
- If push capability or provisioning is wrong, APNs registration fails at runtime (check Xcode logs for `APNs registration failed`).
- Debug builds default to `XClawPushAPNsEnvironment=sandbox`; Release builds default to `production`.
...
- Gateway-side relay sending is configured through `gateway.push.apns.relay.baseUrl` in `xclaw.json`. `XCLAW_APNS_RELAY_BASE_URL` remains a temporary env override only.
...
This exists to keep the hosted relay limited to genuine XClaw official builds and to ensure a
