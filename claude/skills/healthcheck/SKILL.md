---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for XClaw deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, XClaw cron scheduling for periodic checks, or version status checks on a machine running XClaw (laptop, workstation, Pi, VPS).
---

# XClaw Host Hardening

## Overview

Assess and harden the host running XClaw, then align it to a user-defined risk tolerance without breaking access. Use XClaw security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim XClaw changes the host firewall, SSH, or OS updates; it does not.
...
5. XClaw gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if XClaw can access sensitive data.
...
### 2) Run XClaw security audits (read-only)

As part of the default read-only checks, run `xclaw security audit --deep`. Only offer alternatives if the user requests them:

1. `xclaw security audit` (faster, non-probing)
2. `xclaw security audit --json` (structured output)

Offer to apply XClaw safe defaults (numbered):

1. `xclaw security audit --fix`

Be explicit that `--fix` only tightens XClaw defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check XClaw version/update status (read-only)

As part of the default read-only checks, run `xclaw update status`.
...
- Credential hygiene notes (location of XClaw creds, prefer disk encryption)
...
After XClaw install or first hardening pass, run at least one baseline audit and version check:

- `xclaw security audit`
- `xclaw security audit --deep`
- `xclaw update status`

Ongoing monitoring is recommended. Use the XClaw cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `xclaw cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `xclaw update status`
...
Before creating, `xclaw cron list` and match on exact `name`. If found, `xclaw cron edit <id> ...`.
If not found, `xclaw cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `xclaw update status` (preferred for source checkouts and channels)
2. `npm view xclaw version` (published npm version)

## XClaw command accuracy

Use only supported commands and flags:

- `xclaw security audit [--deep] [--fix] [--json]`
- `xclaw status` / `xclaw status --deep`
- `xclaw health --json`
- `xclaw update status`
- `xclaw cron add|list|runs|run`

Do not invent CLI flags or imply XClaw enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by XClaw compaction:
