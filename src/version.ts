...
declare const __XCLAW_VERSION__: string | undefined;
const CORE_PACKAGE_NAME = "xclaw";
...
export function resolveRuntimeServiceVersion(
  ...
  return (
    firstNonEmpty(
      env["XCLAW_VERSION"],
      runtimeVersion,
      env["XCLAW_SERVICE_VERSION"],
      env["npm_package_version"],
    ) ?? fallback
  );
}

// Single source of truth for the current XClaw version.
// - Embedded/bundled builds: injected define or env var.
// - Dev/npm builds: package.json.
export const VERSION = resolveBinaryVersion({
  moduleUrl: import.meta.url,
  injectedVersion: typeof __XCLAW_VERSION__ === "string" ? __XCLAW_VERSION__ : undefined,
  bundledVersion: process.env.XCLAW_BUNDLED_VERSION,
});
