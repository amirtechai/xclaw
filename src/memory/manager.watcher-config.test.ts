...
describe("memory watcher config", () => {
  ...
  async function setupWatcherWorkspace(seedFile: { name: string; contents: string }) {
    workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "xclaw-memory-watch-"));
  ...
  function createWatcherConfig(overrides?: Partial<MemorySearchConfig>): XClawConfig {
    const defaults: NonNullable<NonNullable<XClawConfig["agents"]>["defaults"]> = {
  ...
  async function expectWatcherManager(cfg: XClawConfig) {
