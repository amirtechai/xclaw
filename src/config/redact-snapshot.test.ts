...
import type { ConfigFileSnapshot } from "./types.xclaw.js";
...
function makeSnapshot<TConfig extends Record<string, unknown>>(
  ...
  return {
    path: "/home/user/.xclaw/config.json5",
    exists: true,
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/xclaw/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/xclaw/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/xclaw/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/xclaw/nickserv-password.txt");
