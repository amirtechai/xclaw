...
function createSourceConfig(): XClawConfig {
  return {
    models: {
      providers: {
        openai: {
...
function createRuntimeConfig(): XClawConfig {
  return {
    models: {
      providers: {
        openai: {
...
describe("runtime config snapshot writes", () => {
  it("returns the source snapshot when runtime snapshot is active", async () => {
    await withTempHome("xclaw-config-runtime-source-", async () => {
  ...
  it("skips source projection for non-runtime-derived configs", async () => {
    await withTempHome("xclaw-config-runtime-projection-shape-", async () => {
      ...
      const independentConfig: XClawConfig = {
        models: {
          providers: {
            openai: {
              baseUrl: "https://api.openai.com/v1",
  ...
  it("preserves source secret refs when writeConfigFile receives runtime-resolved config", async () => {
    await withTempHome("xclaw-config-runtime-write-", async (home) => {
  ...
  it("refreshes the runtime snapshot after writes so follow-up reads see persisted changes", async () => {
    await withTempHome("xclaw-config-runtime-write-refresh-", async (home) => {
      const configPath = path.join(home, ".xclaw", "xclaw.json");
      const sourceConfig: XClawConfig = {
        models: {
          providers: {
      ...
      const nextRuntimeConfig: XClawConfig = {
        ...runtimeConfig,
  ...
  it("keeps the last-known-good runtime snapshot active while a specialized refresh is pending", async () => {
    await withTempHome("xclaw-config-runtime-refresh-pending-", async (home) => {
      const configPath = path.join(home, ".xclaw", "xclaw.json");
      const sourceConfig = createSourceConfig();
      const runtimeConfig = createRuntimeConfig();
      const nextRuntimeConfig: XClawConfig = {
