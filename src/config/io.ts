...
import type { XClawConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
...
function formatConfigValidationFailure(pathLabel: string, issueMessage: string): string {
  ...
  return [
    ...
    "",
    "Fix with:",
    `  xclaw config set ${allowPath} '["*"]'`,
    "",
    "Or switch policy:",
    `  xclaw config set ${policyPath} "pairing"`,
  ].join("\n");
}
...
function unsetPathForWrite(
  root: XClawConfig,
  pathSegments: string[],
): { changed: boolean; next: XClawConfig } {
...
function coerceConfig(value: unknown): XClawConfig {
...
function stampConfigVersion(cfg: XClawConfig): XClawConfig {
  const now = new Date().toISOString();
  return {
    ...cfg,
    meta: {
...
function warnIfConfigFromFuture(cfg: XClawConfig, logger: Pick<typeof console, "warn">): void {
  ...
  if (shouldWarnOnTouchedVersion(VERSION, touched)) {
    logger.warn(
      `Config was last written by a newer XClaw (${touched}); current version is ${VERSION}.`,
    );
  }
}
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as XClawConfig, env);
  }
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): XClawConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as XClawConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: XClawConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfigBase =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as XClawConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.XCLAW_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.XCLAW_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      argv: process.argv.slice(0, 8),
      execArgv: process.execArgv.slice(0, 8),
      watchMode: deps.env.XCLAW_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.XCLAW_WATCH_SESSION === "string" &&
        deps.env.XCLAW_WATCH_SESSION.trim().length > 0
          ? deps.env.XCLAW_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.XCLAW_WATCH_COMMAND === "string" &&
        deps.env.XCLAW_WATCH_COMMAND.trim().length > 0
          ? deps.env.XCLAW_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `XCLAW_CONFIG_PATH` (and friends) are expected to work even
...
let runtimeConfigSnapshot: XClawConfig | null = null;
let runtimeConfigSourceSnapshot: XClawConfig | null = null;
let runtimeConfigSnapshotRefreshHandler: RuntimeConfigSnapshotRefreshHandler | null = null;

function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.XCLAW_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.XCLAW_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function getRuntimeConfigSnapshot(): XClawConfig | null {
  return runtimeConfigSnapshot;
}

export function getRuntimeConfigSourceSnapshot(): XClawConfig | null {
  return runtimeConfigSourceSnapshot;
}

function isCompatibleTopLevelRuntimeProjectionShape(params: {
  runtimeSnapshot: XClawConfig;
  candidate: XClawConfig;
}): boolean {
...
export function projectConfigOntoRuntimeSourceSnapshot(config: XClawConfig): XClawConfig {
...
export function loadConfig(): XClawConfig {
...
export async function readBestEffortConfig(): Promise<XClawConfig> {
...
export async function writeConfigFile(
  ...
  // edits to xclaw.json remain visible (no stale snapshot).
}
