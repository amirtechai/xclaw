...
import type { XClawConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: XClawConfig): ConfigValidationIssue[] {
...
function validateGatewayTailscaleBind(config: XClawConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: XClawConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = XClawSchema.safeParse(normalizedRaw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as XClawConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as XClawConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  const gatewayTailscaleBindIssues = validateGatewayTailscaleBind(validated.data as XClawConfig);
  if (gatewayTailscaleBindIssues.length > 0) {
    return { ok: false, issues: gatewayTailscaleBindIssues };
  }
  return {
    ok: true,
    config: validated.data as XClawConfig,
  };
}
...
function validateConfigObjectWithPluginsBase(
  ...
  const warnings: ConfigValidationIssue[] = listLegacyWebSearchConfigPaths(raw).map((path) => ({
    ...
    message:
      ...
      "Move it under plugins.entries.<plugin>.config.webSearch.*; XClaw mapped it automatically for compatibility.",
  }));
  ...
  let compatConfig: XClawConfig | null | undefined;

  const ensureCompatConfig = (): XClawConfig => {
  ...
  for (const record of registry.plugins) {
    ...
    if (shouldValidate) {
      if (record.configSchema) {
        ...
        // Compatible bundles currently expose no native XClaw config schema.
