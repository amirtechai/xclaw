...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "xclaw-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ XCLAW_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-xclaw",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-xclaw#general");
  });
  ...
  it("derives session transcripts dir from XCLAW_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { XCLAW_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent paths when XCLAW_STATE_DIR differs from stored paths", () => {
