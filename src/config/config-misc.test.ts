...
import { buildWebSearchProviderConfig, withTempHome, writeXClawConfig } from "./test-helpers.js";
import { XClawSchema } from "./zod-schema.js";

describe("$schema key in config (#14998)", () => {
  it("accepts config with $schema string", () => {
    const result = XClawSchema.safeParse({
      $schema: "https://xclaw.ai/config.json",
    });
    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.data.$schema).toBe("https://xclaw.ai/config.json");
    }
  });
  ...
  it("rejects non-string $schema", () => {
    const result = XClawSchema.safeParse({ $schema: 123 });
...
describe("cron webhook schema", () => {
  it("accepts cron.webhookToken and legacy cron.webhook", () => {
    const res = XClawSchema.safeParse({
      cron: {
        enabled: true,
...
describe("config strict validation", () => {
  ...
  it("does not mark resolved-only gateway.bind aliases as auto-migratable legacy", async () => {
    await withTempHome(async (home) => {
      await writeXClawConfig(home, {
        gateway: { bind: "${XCLAW_BIND}" },
      });

      const prev = process.env.XCLAW_BIND;
      process.env.XCLAW_BIND = "0.0.0.0";
      try {
        const snap = await readConfigFileSnapshot();
        expect(snap.valid).toBe(false);
        ...
        if (prev === undefined) {
          delete process.env.XCLAW_BIND;
        } else {
          process.env.XCLAW_BIND = prev;
        }
      }
    });
  });
