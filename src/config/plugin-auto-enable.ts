...
const ENV_CATALOG_PATHS = ["XCLAW_PLUGIN_CATALOG_PATHS", "XCLAW_MPM_CATALOG_PATHS"];

function collectModelRefs(cfg: XClawConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: XClawConfig, providerId: string): boolean {
...
function parseExternalCatalogChannelEntries(raw: unknown): ExternalCatalogChannelEntry[] {
  ...
  for (const entry of list) {
    if (!isRecord(entry) || !isRecord(entry.xclaw) || !isRecord(entry.xclaw.channel)) {
      continue;
    }
    const channel = entry.xclaw.channel;
...
function collectCandidateChannelIds(cfg: XClawConfig): string[] {
...
function configMayNeedPluginManifestRegistry(cfg: XClawConfig): boolean {
  const configuredChannels = cfg.channels as Record<string, unknown> | undefined;
  if (!configuredChannels || typeof configuredChannels !== "object") {
    return false;
  }
...
function isPluginExplicitlyDisabled(cfg: XClawConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: XClawConfig, pluginId: string): boolean {
...
function registerPluginEntry(cfg: XClawConfig, pluginId: string): XClawConfig {
  const builtInChannelId = normalizeChatChannelId(pluginId);
  if (builtInChannelId) {
    const channels = cfg.channels as Record<string, unknown> | undefined;
