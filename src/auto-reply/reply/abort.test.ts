...
describe("abort detection", () => {
  ...
  async function createAbortConfig(params?: {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "xclaw-abort-"));
  ...
  it("isAbortRequestText aligns abort command semantics", () => {
    ...
    expect(isAbortRequestText("stop xclaw!!!")).toBe(true);
    ...
    expect(isAbortRequestText("/stop@xclaw_bot", { botUsername: "xclaw_bot" })).toBe(true);
    expect(isAbortRequestText("/Stop@xclaw_bot", { botUsername: "xclaw_bot" })).toBe(true);
