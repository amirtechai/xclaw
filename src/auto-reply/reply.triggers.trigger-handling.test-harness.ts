...
const piEmbeddedMocks = getSharedMocks("xclaw.trigger-handling.pi-embedded-mocks", () => ({
...
const modelCatalogMocks = getSharedMocks("xclaw.trigger-handling.model-catalog-mocks", () => ({
...
const modelFallbackMocks = getSharedMocks("xclaw.trigger-handling.model-fallback-mocks", () => ({
...
const webSessionMocks = getSharedMocks("xclaw.trigger-handling.web-session-mocks", () => ({
...
function snapshotTempHomeEnv(): TempHomeEnvSnapshot {
  return {
    ...
    xclawHome: process.env.XCLAW_HOME,
...
function restoreTempHomeEnv(snapshot: TempHomeEnvSnapshot): void {
  ...
  restoreKey("XCLAW_HOME", snapshot.xclawHome);
  restoreKey("XCLAW_STATE_DIR", snapshot.stateDir);
}
...
function setTempHomeEnv(home: string): void {
  ...
  process.env.XCLAW_STATE_DIR = join(home, ".xclaw");
...
beforeAll(async () => {
  suiteTempHomeRoot = await fs.mkdtemp(join(os.tmpdir(), "xclaw-triggers-suite-"));
});
...
export async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  ...
  await fs.mkdir(join(home, ".xclaw", "agents", "main", "sessions"), { recursive: true });
  setTempHomeEnv(home);
...
export function makeCfg(home: string): XClawConfig {
  return {
    agents: {
      defaults: {
        model: { primary: "anthropic/claude-opus-4-5" },
        workspace: join(home, "xclaw"),
