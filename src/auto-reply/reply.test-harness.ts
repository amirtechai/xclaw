...
type HomeEnvSnapshot = {
  ...
  XCLAW_STATE_DIR: string | undefined;
  XCLAW_AGENT_DIR: string | undefined;
...
function snapshotHomeEnv(): HomeEnvSnapshot {
  return {
    ...
    XCLAW_AGENT_DIR: process.env.XCLAW_AGENT_DIR,
...
export function createTempHomeHarness(options: { prefix: string; beforeEachCase?: () => void }) {
  ...
  async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    await fs.mkdir(path.join(home, ".xclaw", "agents", "main", "sessions"), { recursive: true });
    const envSnapshot = snapshotHomeEnv();
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.XCLAW_STATE_DIR = path.join(home, ".xclaw");
    process.env.XCLAW_AGENT_DIR = path.join(home, ".xclaw", "agent");
    process.env.PI_CODING_AGENT_DIR = path.join(home, ".xclaw", "agent");
