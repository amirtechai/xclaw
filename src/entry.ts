...
import { ensureXClawExecMarkerOnProcess } from "./infra/xclaw-exec-env.js";
import { installProcessWarningFilter } from "./infra/warning-filter.js";
import { attachChildProcessBridge } from "./process/child-process-bridge.js";

const ENTRY_WRAPPER_PAIRS = [
  { wrapperBasename: "xclaw.mjs", entryBasename: "entry.js" },
  { wrapperBasename: "xclaw.js", entryBasename: "entry.js" },
] as const;
...
if (
  ...
  if (shouldForceReadOnlyAuthStore(process.argv)) {
    process.env.XCLAW_AUTH_STORE_READONLY = "1";
  }
  ...
  function tryHandleRootVersionFastPath(argv: string[]): boolean {
    ...
    Promise.all([import("./version.js"), import("./infra/git-commit.js")])
      .then(([{ VERSION }, { resolveCommitHash }]) => {
        ...
        console.log(commit ? `XClaw ${VERSION} (${commit})` : `XClaw ${VERSION}`);
        process.exit(0);
      })
      .catch((error) => {
        console.error(
          "[xclaw] Failed to resolve version:",
          error instanceof Error ? (error.stack ?? error.message) : error,
        );
        process.exitCode = 1;
      });
  ...
  if (!ensureCliRespawnReady()) {
    ...
    if (!parsedContainer.ok) {
      console.error(`[xclaw] ${parsedContainer.error}`);
      process.exit(2);
    }
    ...
    if (!parsed.ok) {
      ...
      console.error(`[xclaw] ${parsed.error}`);
      process.exit(2);
    }
    ...
    if (
      ...
      console.error(
        "[xclaw] --container cannot be combined with --profile/--dev or gateway override env vars",
      );
      process.exit(2);
    }
