...
describe("env test utils", () => {
  it("captureEnv restores mutated keys", () => {
    const keyA = "XCLAW_ENV_TEST_A";
    const keyB = "XCLAW_ENV_TEST_B";
  ...
  it("captureFullEnv restores added keys and baseline values", () => {
    const key = "XCLAW_ENV_TEST_ADDED";
  ...
  it("withEnv restores values when callback throws", () => {
    const key = "XCLAW_ENV_TEST_SYNC_THROW";
    const prev = process.env[key];
  ...
  it("withEnv can delete a key only inside callback", () => {
    const key = "XCLAW_ENV_TEST_SYNC_DELETE";
    const prev = process.env[key];
  ...
  it("withEnvAsync restores values when callback throws", async () => {
    const key = "XCLAW_ENV_TEST_ASYNC";
    const prev = process.env[key];
  ...
  it("withEnvAsync applies values only inside async callback", async () => {
    const key = "XCLAW_ENV_TEST_ASYNC_OK";
    const prev = process.env[key];
  ...
  it("withEnvAsync can delete a key only inside callback", async () => {
    const key = "XCLAW_ENV_TEST_ASYNC_DELETE";
    const prev = process.env[key];
    process.env[key] = "outer";
  ...
  it("createPathResolutionEnv clears leaked path overrides before applying explicit ones", () => {
    const homeDir = path.join(path.sep, "tmp", "xclaw-home");
    const resolvedHomeDir = path.resolve(homeDir);
    const previousXClawHome = process.env.XCLAW_HOME;
    const previousStateDir = process.env.XCLAW_STATE_DIR;
    const previousBundledDir = process.env.XCLAW_BUNDLED_PLUGINS_DIR;
    process.env.XCLAW_HOME = "/srv/xclaw-home";
    process.env.XCLAW_STATE_DIR = "/srv/xclaw-state";
    process.env.XCLAW_BUNDLED_PLUGINS_DIR = "/srv/xclaw-bundled";
    ...
    try {
      ...
      expect(env.XCLAW_HOME).toBeUndefined();
      expect(env.XCLAW_BUNDLED_PLUGINS_DIR).toBeUndefined();
      expect(env.XCLAW_STATE_DIR).toBe("~/state");
    } finally {
      restoreEnvKey("XCLAW_HOME", previousXClawHome);
      restoreEnvKey("XCLAW_STATE_DIR", previousStateDir);
      restoreEnvKey("XCLAW_BUNDLED_PLUGINS_DIR", previousBundledDir);
    }
  });
  ...
  it("withPathResolutionEnv only applies the explicit path env inside the callback", () => {
    ...
    try {
      const seen = withPathResolutionEnv(
        homeDir,
        { XCLAW_BUNDLED_PLUGINS_DIR: "~/bundled" },
        (env) => ({
          processHome: process.env.HOME,
          processXClawHome: process.env.XCLAW_HOME,
          processBundledDir: process.env.XCLAW_BUNDLED_PLUGINS_DIR,
          envBundledDir: env.XCLAW_BUNDLED_PLUGINS_DIR,
        }),
      );

      expect(seen).toEqual({
        processHome: resolvedHomeDir,
        processXClawHome: undefined,
        processBundledDir: "~/bundled",
        envBundledDir: "~/bundled",
      });
      expect(process.env.XCLAW_HOME).toBe("/srv/xclaw-home");
    } finally {
      restoreEnvKey("XCLAW_HOME", previousXClawHome);
    }
  });
