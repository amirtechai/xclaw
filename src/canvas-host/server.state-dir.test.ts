...
describe("canvas host state dir defaults", () => {
  ...
  it("uses XCLAW_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("xclaw-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("XClaw Canvas");
      } finally {
        await handler.close();
      }
    });
