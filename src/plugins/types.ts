 ...
 */
export type XClawPluginConfigSchema = {
...
export type XClawPluginToolContext = {
  config?: XClawConfig;
  workspaceDir?: string;
  agentDir?: string;
  agentId?: string;
...
export type XClawPluginToolFactory = (
  ctx: XClawPluginToolContext,
) => AnyAgentTool | AnyAgentTool[] | null | undefined;

export type XClawPluginToolOptions = {
  name?: string;
  names?: string[];
  optional?: boolean;
};

export type XClawPluginHookOptions = {
  entry?: HookEntry;
  name?: string;
  description?: string;
  register?: boolean;
...
export type ProviderAuthResult = {
   ...
   */
  configPatch?: Partial<XClawConfig>;
  defaultModel?: string;
  notes?: string[];
};
...
export type ProviderAuthMethod = {
   ...
   * key + OAuth, or region-specific login flows). XClaw uses this to expose
 ...
 *
 * Runs after XClaw resolves an explicit/discovered/dynamic model and before
 ...
 *
 * The helper methods cover the common XClaw auth resolution paths:
 *
 ...
 *
 * Called when OAuth refresh fails and XClaw wants a provider-specific repair
...
/**
 * Provider-owned extra-param normalization before XClaw builds its generic
...
/**
 * Provider-owned stream wrapper hook after XClaw applies its generic
 ...
 *
 * Return `true` or `false` to override XClaw's built-in provider cache TTL
 ...
 *
 * Runs only after XClaw exhausts normal env/profile/config auth resolution
 ...
 *
 * Runs after XClaw loads the discovered model catalog and merges configured
...
export type ProviderPlugin = {
   ...
   *
   * XClaw calls this only from async model resolution paths. After it
  ...
  /**
   * Provider-owned stream wrapper applied after generic XClaw wrappers.
   *
   ...
   *
   * Called after XClaw resolves the raw configured credential but before the
   ...
   * hint than XClaw's generic auth-store guidance.
   */
   ...
   * `errorMessage` when XClaw should surface a provider-specific hint for
   ...
   * merging. XClaw deduplicates by `provider/id`, so plugins only need to
   ...
   *
   * XClaw uses this when a stored auth profile is already valid and needs to
  ...
  /**
   * Legacy auth-profile ids that should be retired by `xclaw doctor`.
   *
   ...
   *
   * XClaw calls this before falling back to the shared `pi-ai` OAuth
   ...
   * path. Return nothing to keep XClaw's generic error text.
   */
  buildAuthDoctorHint?: (
...
export type WebSearchProviderPlugin = {
  ...
  getConfiguredCredentialValue?: (config?: XClawConfig) => unknown;
  setConfiguredCredentialValue?: (configTarget: XClawConfig, value: unknown) => void;
  applySelectionConfig?: (config: XClawConfig) => XClawConfig;
...
export type XClawPluginGatewayMethod = {
  method: string;
 ...
 */
export type XClawPluginCommandDefinition = {
...
export type XClawPluginHttpRouteAuth = "gateway" | "plugin";
export type XClawPluginHttpRouteMatch = "exact" | "prefix";

export type XClawPluginHttpRouteHandler = (
  req: IncomingMessage,
  res: ServerResponse,
) => Promise<boolean | void> | boolean | void;

export type XClawPluginHttpRouteParams = {
  path: string;
  handler: XClawPluginHttpRouteHandler;
  auth: XClawPluginHttpRouteAuth;
  match?: XClawPluginHttpRouteMatch;
  replaceExisting?: boolean;
};

export type XClawPluginCliContext = {
  program: Command;
  config: XClawConfig;
  workspaceDir?: string;
  logger: PluginLogger;
...
export type XClawPluginCliRegistrar = (ctx: XClawPluginCliContext) => void | Promise<void>;

/** Context passed to long-lived plugin services. */
export type XClawPluginServiceContext = {
  config: XClawConfig;
  workspaceDir?: string;
  stateDir: string;
  logger: PluginLogger;
...
export type XClawPluginService = {
  id: string;
  start: (ctx: XClawPluginServiceContext) => void | Promise<void>;
  stop?: (ctx: XClawPluginServiceContext) => void | Promise<void>;
};

export type XClawPluginChannelRegistration = {
  plugin: ChannelPlugin;
};

/** Module-level plugin definition loaded from a native plugin entry file. */
export type XClawPluginDefinition = {
  id?: string;
  name?: string;
  description?: string;
  version?: string;
  kind?: PluginKind;
  configSchema?: XClawPluginConfigSchema;
  register?: (api: XClawPluginApi) => void | Promise<void>;
  activate?: (api: XClawPluginApi) => void | Promise<void>;
};

export type XClawPluginModule =
  | XClawPluginDefinition
  | ((api: XClawPluginApi) => void | Promise<void>);

export type PluginRegistrationMode = "full" | "setup-only" | "setup-runtime";

/** Main registration API injected into native plugin entry files. */
export type XClawPluginApi = {
  id: string;
  name: string;
  version?: string;
  description?: string;
  ...
  logger: PluginLogger;
  registerTool: (
    tool: AnyAgentTool | XClawPluginToolFactory,
    opts?: XClawPluginToolOptions,
  ) => void;
  registerHook: (
    events: string | string[],
    handler: InternalHookHandler,
    opts?: XClawPluginHookOptions,
  ) => void;
  registerHttpRoute: (params: XClawPluginHttpRouteParams) => void;
  /** Register a native messaging channel plugin (channel capability). */
  registerChannel: (registration: XClawPluginChannelRegistration | ChannelPlugin) => void;
  registerGatewayMethod: (method: string, handler: GatewayRequestHandler) => void;
  registerCli: (registrar: XClawPluginCliRegistrar, opts?: { commands?: string[] }) => void;
  registerService: (service: XClawPluginService) => void;
   ...
   */
  registerCommand: (command: XClawPluginCommandDefinition) => void;
...
export type PluginFormat = "xclaw" | "bundle";
