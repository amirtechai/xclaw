...
const APPROVALS_PATH = "~/.xclaw/plugin-binding-approvals.json";
...
const LEGACY_CODEX_PLUGIN_SESSION_PREFIXES = [
  "xclaw-app-server:thread:",
  "xclaw-codex-app-server:thread:",
] as const;
...
const PLUGIN_BINDING_PENDING_REQUESTS_KEY = Symbol.for("xclaw.pluginBindingPendingRequests");
...
const pluginBindingGlobalStateKey = Symbol.for("xclaw.plugins.binding.global-state");
...
export function buildPluginBindingUnavailableText(binding: PluginConversationBinding): string {
  return `The bound plugin ${resolvePluginBindingDisplayName(binding)} is not currently loaded. Routing this message to XClaw instead.${buildDetachHintSuffix(binding.detachHint)}`;
}
