...
function throwGatewayAuthResolutionError(reason: string): never {
  throw new Error(
    [
      reason,
      "Fix: set XCLAW_GATEWAY_TOKEN/XCLAW_GATEWAY_PASSWORD, pass --token/--password,",
...
export async function resolveGatewayConnection(
  ...
  const envToken = trimToUndefined(env.XCLAW_GATEWAY_TOKEN);
  const envPassword = trimToUndefined(env.XCLAW_GATEWAY_PASSWORD);
