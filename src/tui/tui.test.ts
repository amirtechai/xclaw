...
describe("resolveInitialTuiAgentId", () => {
  const cfg: XClawConfig = {
    agents: {
      list: [
        { id: "main", workspace: "/tmp/xclaw" },
        { id: "ops", workspace: "/tmp/xclaw/projects/ops" },
      ],
    },
  };
  ...
  it("infers agent from cwd when session is not agent-prefixed", () => {
    expect(
      resolveInitialTuiAgentId({
        ...
        initialSessionInput: "",
        cwd: "/tmp/xclaw/projects/ops/src",
      }),
    ).toBe("ops");
  });
...
describe("resolveGatewayDisconnectState", () => {
  it("returns pairing recovery guidance when disconnect reason requires pairing", () => {
    ...
    expect(state.activityStatus).toBe("pairing required: run xclaw devices list");
    expect(state.pairingHint).toContain("xclaw devices list");
  });
