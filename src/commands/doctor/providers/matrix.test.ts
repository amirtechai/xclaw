...
describe("doctor matrix provider helpers", () => {
  ...
  it("warns on stale custom Matrix plugin paths", async () => {
    const missingPath = path.join(tmpdir(), "xclaw-matrix-missing-provider-test");
    ...
    expect(warnings[1]).toContain("xclaw plugins install @xclaw/matrix");
    expect(warnings[2]).toContain("xclaw plugins install ./extensions/matrix");
  });
  ...
  it("removes stale Matrix plugin config when install path is missing", async () => {
    const missingPath = path.join(tmpdir(), "xclaw-matrix-stale-cleanup-test-" + Date.now());
    await fs.rm(missingPath, { recursive: true, force: true });
