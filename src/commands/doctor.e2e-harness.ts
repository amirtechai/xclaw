...
export const resolveXClawPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  loadXClawPlugins: () => createEmptyPluginRegistry(),
}));
...
beforeEach(() => {
  ...
  resolveXClawPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.XCLAW_STATE_DIR;
  originalUpdateInProgress = process.env.XCLAW_UPDATE_IN_PROGRESS;
  process.env.XCLAW_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "xclaw-doctor-state-"));
