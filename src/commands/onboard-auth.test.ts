...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under XCLAW_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("xclaw-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "xclaw-oauth-sync-"));
    ...
    process.env.XCLAW_AGENT_DIR = kidAgentDir;
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "xclaw-oauth-nosync-"));
    process.env.XCLAW_STATE_DIR = tempStateDir;
  ...
  it("syncs siblings from explicit agentDir outside XCLAW_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "xclaw-oauth-external-"));
    process.env.XCLAW_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* XCLAW_STATE_DIR
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to XCLAW_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("xclaw-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
