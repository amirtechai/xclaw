...
beforeAll(() => {
  envSnapshot = captureEnv(["XCLAW_PROFILE"]);
  process.env.XCLAW_PROFILE = "isolated";
});
...
vi.mock("../infra/xclaw-root.js", () => ({
  resolveXClawPackageRoot: vi.fn().mockResolvedValue("/tmp/xclaw"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/xclaw/pnpm-lock.yaml",
      markerPath: "/tmp/xclaw/node_modules/.modules.yaml",
    },
...
vi.mock("../daemon/service.js", () => ({
  resolveGatewayService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.xclaw.gateway.plist",
    }),
  }),
}));
vi.mock("../daemon/node-service.js", () => ({
  resolveNodeService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.xclaw.node.plist",
    }),
  }),
}));
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(
      logs.some(
        (line) =>
          line.includes("xclaw status --all") ||
          line.includes("xclaw --profile isolated status --all"),
      ),
    ).toBe(true);
  });
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    await withEnvVar("XCLAW_GATEWAY_TOKEN", "abcd1234", async () => {
