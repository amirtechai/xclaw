...
const debugHealth = (...args: unknown[]) => {
  if (isTruthyEnvValue(process.env.XCLAW_DEBUG_HEALTH)) {
...
async function inspectHealthAccount(plugin: ChannelPlugin, cfg: XClawConfig, accountId: string) {
  return (
...
export async function healthCommand(
  opts: { json?: boolean; timeoutMs?: number; verbose?: boolean; config?: XClawConfig },
  runtime: RuntimeEnv,
) {
  ...
  if (opts.json) {
    ...
    const debugEnabled = isTruthyEnvValue(process.env.XCLAW_DEBUG_HEALTH);
    const rich = isRich();
    if (opts.verbose) {
