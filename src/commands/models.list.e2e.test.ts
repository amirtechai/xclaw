...
const ensureXClawModelsJson = vi.fn().mockResolvedValue(undefined);
const resolveXClawAgentDir = vi.fn().mockReturnValue("/tmp/xclaw-agent");
...
beforeEach(() => {
  ...
  ensureXClawModelsJson.mockClear();
...
describe("models list/status", () => {
  ...
  it("loadModelRegistry does not persist models.json as a side effect", async () => {
    ...
    expect(ensureXClawModelsJson).not.toHaveBeenCalled();
  });
  ...
  it("modelsListCommand persists using the write snapshot config when provided", async () => {
    ...
    expect(ensureXClawModelsJson).toHaveBeenCalled();
    expect(ensureXClawModelsJson.mock.calls[0]?.[0]).toEqual(resolvedConfig);
  });
