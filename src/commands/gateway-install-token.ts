...
import { readConfigFileSnapshot, writeConfigFile, type XClawConfig } from "../config/config.js";
...
function formatAmbiguousGatewayAuthModeReason(): string {
  return [
    ...
    `Set ${formatCliCommand("xclaw config set gateway.auth.mode token")} or ${formatCliCommand("xclaw config set gateway.auth.mode password")}.`,
  ].join(" ");
}
