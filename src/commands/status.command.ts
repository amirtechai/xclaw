...
export async function statusCommand(
  ...
  const daemonValue = (() => {
    ...
    const installedPrefix = daemon.managedByXClaw ? "installed · " : "";
  ...
  const nodeDaemonValue = (() => {
    ...
    const installedPrefix = nodeDaemon.managedByXClaw ? "installed · " : "";
  ...
  runtime.log(theme.heading("XClaw status"));
  runtime.log("");
  ...
  if (pairingRecovery) {
    ...
    if (pairingRecovery.requestId) {
      runtime.log(
        theme.muted(
          `Recovery: ${formatCliCommand(`xclaw devices approve ${pairingRecovery.requestId}`)}`,
        ),
      );
    }
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("xclaw security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("xclaw security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.xclaw.ai/faq");
  runtime.log("Troubleshooting: https://docs.xclaw.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("xclaw status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("xclaw logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("xclaw status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("xclaw gateway probe")}`);
  }
}
