...
type PromptDefaultModelResult = { model?: string; config?: XClawConfig };
...
function resolveConfiguredModelRaw(cfg: XClawConfig): string {
  return resolveAgentModelPrimaryValue(cfg.agents?.defaults?.model) ?? "";
}

function resolveConfiguredModelKeys(cfg: XClawConfig): string[] {
...
export function applyModelAllowlist(cfg: XClawConfig, models: string[]): XClawConfig {
