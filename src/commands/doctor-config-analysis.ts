...
import { XClawSchema } from "../config/zod-schema.js";
...
export function stripUnknownConfigKeys(config: XClawConfig): {
  config: XClawConfig;
  removed: string[];
} {
  const parsed = XClawSchema.safeParse(config);
  if (parsed.success) {
...
export function noteOpencodeProviderOverrides(cfg: XClawConfig): void {
