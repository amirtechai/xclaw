...
function resolveMode(cfg: XClawConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  const root = await resolveXClawPackageRoot({
  ...
  let cfg: XClawConfig = configResult.cfg;
  ...
  if (!cfg.gateway?.mode) {
    const lines = [
      ...
      `Fix: run ${formatCliCommand("xclaw configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("xclaw config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("xclaw setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  if (resolveMode(cfg) === "local" && hasAmbiguousGatewayAuthModeConfig(cfg)) {
    note(
      [
        ...
        `Set token mode: ${formatCliCommand("xclaw config set gateway.auth.mode token")}`,
        `Set password mode: ${formatCliCommand("xclaw config set gateway.auth.mode password")}`,
      ].join("\n"),
      "Gateway auth",
    );
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("xclaw doctor --fix")}" to apply changes.`);
  }
