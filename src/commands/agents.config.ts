...
function resolveAgentName(cfg: XClawConfig, agentId: string) {
...
function resolveAgentModel(cfg: XClawConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: XClawConfig): AgentSummary[] {
