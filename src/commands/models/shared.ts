...
export async function loadValidConfigOrThrow(): Promise<XClawConfig> {
...
export async function updateConfig(
  mutator: (cfg: XClawConfig) => XClawConfig,
): Promise<XClawConfig> {
...
export function resolveModelTarget(params: { raw: string; cfg: XClawConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: XClawConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("xclaw agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
