...
const SEARCH_TOOL = "https://docs.xclaw.ai/mcp.SearchXClaw";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.xclaw.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('xclaw docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.xclaw.ai/");
      runtime.log(`Search: ${formatCliCommand('xclaw docs "your query"')}`);
    }
    return;
  }
