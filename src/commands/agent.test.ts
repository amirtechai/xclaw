...
vi.mock("../agents/workspace.js", () => {
  const resolveDefaultAgentWorkspaceDir = () => "/tmp/xclaw-workspace";
  return {
    DEFAULT_AGENT_WORKSPACE_DIR: "/tmp/xclaw-workspace",
...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "xclaw-agent-" });
}
...
function mockConfig(
  home: string,
  storePath: string,
  agentOverrides?: Partial<NonNullable<NonNullable<XClawConfig["agents"]>["defaults"]>>,
  telegramOverrides?: Partial<NonNullable<NonNullable<XClawConfig["channels"]>["telegram"]>>,
  ...
  const cfg = {
    agents: {
      defaults: {
        ...
        workspace: path.join(home, "xclaw"),
        ...agentOverrides,
      },
      list: agentsList,
    },
...
async function withCrossAgentResumeFixture(
  run: (params: { sessionId: string; sessionKey: string; cfg: XClawConfig }) => Promise<void>,
): Promise<void> {
...
beforeEach(() => {
  ...
  readConfigFileSnapshotForWriteSpy.mockResolvedValue({
    snapshot: { valid: false, resolved: {} as XClawConfig },
    writeOptions: {},
