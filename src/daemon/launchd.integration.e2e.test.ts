...
describeLaunchdIntegration("launchd integration", () => {
  ...
  beforeAll(async () => {
    ...
    homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `xclaw-launchd-int-${testId}-`));
    env = {
      HOME: homeDir,
      XCLAW_LAUNCHD_LABEL: `ai.xclaw.launchd-int-${testId}`,
      XCLAW_LOG_PREFIX: `gateway-launchd-int-${testId}`,
    };
  });
