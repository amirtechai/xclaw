...
describe("resolveTaskScriptPath", () => {
  it.each([
    {
      name: "uses default path when XCLAW_PROFILE is unset",
      env: { USERPROFILE: "C:\\Users\\test" },
      expected: path.join("C:\\Users\\test", ".xclaw", "gateway.cmd"),
    },
    {
      name: "uses profile-specific path when XCLAW_PROFILE is set to a custom value",
      env: { USERPROFILE: "C:\\Users\\test", XCLAW_PROFILE: "jbphoenix" },
      expected: path.join("C:\\Users\\test", ".xclaw-jbphoenix", "gateway.cmd"),
    },
    {
      name: "prefers XCLAW_STATE_DIR over profile-derived defaults",
      env: {
        USERPROFILE: "C:\\Users\\test",
        XCLAW_PROFILE: "rescue",
        XCLAW_STATE_DIR: "C:\\State\\xclaw",
      },
      expected: path.join("C:\\State\\xclaw", "gateway.cmd"),
    },
    {
      name: "falls back to HOME when USERPROFILE is not set",
      env: { HOME: "/home/test", XCLAW_PROFILE: "default" },
      expected: path.join("/home/test", ".xclaw", "gateway.cmd"),
    },
  ])("$name", ({ env, expected }) => {
...
describe("readScheduledTaskCommand", () => {
  async function withScheduledTaskScript(
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "xclaw-schtasks-test-"));
    try {
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          "rem XClaw Gateway",
          "cd /d C:\\Projects\\xclaw",
          "set NODE_ENV=production",
          "set XCLAW_PORT=18789",
      ...
      async (env) => {
        ...
        expect(result).toEqual({
          ...
          workingDirectory: "C:\\Projects\\xclaw",
          environment: {
            NODE_ENV: "production",
            XCLAW_PORT: "18789",
          },
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"C:\\Program Files\\nodejs\\node.exe" C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\xclaw\\dist\\index.js gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "C:\\Program Files\\nodejs\\node.exe",
            "C:\\Users\\test\\AppData\\Roaming\\npm\\node_modules\\xclaw\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"\\\\fileserver\\XClaw Share\\node.exe" "\\\\fileserver\\XClaw Share\\dist\\index.js" gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "\\\\fileserver\\XClaw Share\\node.exe",
            "\\\\fileserver\\XClaw Share\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
  ...
  it("reads script from XCLAW_STATE_DIR override", async () => {
    await withScheduledTaskScript(
      {
        env: (tmpDir) => ({ XCLAW_STATE_DIR: path.join(tmpDir, "custom-state") }),
