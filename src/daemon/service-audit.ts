...
function auditGatewayToken(
  ...
  issues.push({
    ...
    message:
      "Gateway service XCLAW_GATEWAY_TOKEN does not match gateway.auth.token in xclaw.json",
...
export function readEmbeddedGatewayToken(command: GatewayServiceCommand): string | undefined {
  ...
  if (command.environmentValueSources?.XCLAW_GATEWAY_TOKEN === "file") {
    return undefined;
  }
  return command.environment?.XCLAW_GATEWAY_TOKEN?.trim() || undefined;
}
