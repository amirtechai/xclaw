...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with base env and drops undefined values", async () => {
    ...
    expect(resolved.XCLAW_BASE_ENV).toBe("base");
    expect(resolved.XCLAW_TEST_ENV).toBe("ok");
    expect(resolved.XCLAW_TO_REMOVE).toBeUndefined();
    expect(resolved.XCLAW_CLI).toBe(XCLAW_CLI_ENV_VALUE);
  });
