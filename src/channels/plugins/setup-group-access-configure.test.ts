...
async function runConfigureChannelAccess<TResolved>(params: {
  ...
  label?: string;
  placeholder?: string;
  setPolicy: (cfg: XClawConfig, policy: ChannelAccessPolicy) => XClawConfig;
  resolveAllowlist: (params: { cfg: XClawConfig; entries: string[] }) => Promise<TResolved>;
  applyAllowlist: (params: { cfg: XClawConfig; resolved: TResolved }) => XClawConfig;
}) {
...
describe("configureChannelAccessWithAllowlist", () => {
  it("returns input config when user skips access configuration", async () => {
    ...
    const setPolicy = vi.fn((next: XClawConfig) => next);
    const resolveAllowlist = vi.fn(async () => [] as string[]);
    const applyAllowlist = vi.fn((params: { cfg: XClawConfig }) => params.cfg);
  ...
  it("applies non-allowlist policy directly", async () => {
    ...
    const setPolicy = vi.fn(
      (next: XClawConfig, policy: ChannelAccessPolicy): XClawConfig => ({
        ...next,
  ...
  it("resolves allowlist entries and applies them after forcing allowlist policy", async () => {
    ...
    const calls: string[] = [];
    const setPolicy = vi.fn((next: XClawConfig, policy: ChannelAccessPolicy): XClawConfig => {
    ...
    const resolveAllowlist = vi.fn(async (params: { cfg: XClawConfig; entries: string[] }) => {
      calls.push("resolve");
      expect(params.cfg).toBe(cfg);
      expect(params.entries).toEqual(["#general", "#support"]);
      return ["C1", "C2"];
    });
    const applyAllowlist = vi.fn((params: { cfg: XClawConfig; resolved: string[] }) => {
      calls.push("apply");
