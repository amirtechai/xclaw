...
let originalXClawLogLevel: string | undefined;
...
beforeEach(() => {
  originalTestFileLog = process.env.XCLAW_TEST_FILE_LOG;
  originalXClawLogLevel = process.env.XCLAW_LOG_LEVEL;
  delete process.env.XCLAW_TEST_FILE_LOG;
  delete process.env.XCLAW_LOG_LEVEL;
...
afterEach(() => {
  if (originalTestFileLog === undefined) {
    ...
    process.env.XCLAW_TEST_FILE_LOG = originalTestFileLog;
  }
  if (originalXClawLogLevel === undefined) {
    delete process.env.XCLAW_LOG_LEVEL;
  } else {
    process.env.XCLAW_LOG_LEVEL = originalXClawLogLevel;
  }
  logging.resetLogger();
  logging.setLoggerOverride(null);
  vi.restoreAllMocks();
