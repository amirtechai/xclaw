...
`;
    ...
    expect(parsed.xclaw.emoji).toBe("💾");
    expect(parsed.xclaw.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.xclaw.emoji).toBe("📝");
    expect(parsed.xclaw.events).toEqual(["command"]);
    expect(parsed.xclaw.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.xclaw.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"xclaw": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"xclaw": {"events": ["test"]}}');
  });
...
describe("resolveXClawMetadata", () => {
  it("extracts xclaw metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveXClawMetadata(frontmatter);
  ...
  it("returns undefined when xclaw key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        xclaw: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with XClaw" },
            { id: "npm", kind: "npm", package: "@xclaw/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@xclaw/hook");
  });
    ...
    "xclaw":
      {
        ...
        "events": ["command:new", "command:reset"],
        "requires": { "config": ["workspace.dir"] },
        "install": [{ "id": "bundled", "kind": "bundled", "label": "Bundled with XClaw" }],
      },
  }
---
...
`;
    ...
    const xclaw = resolveXClawMetadata(frontmatter);
    expect(xclaw).toBeDefined();
    expect(xclaw?.emoji).toBe("💾");
    expect(xclaw?.events).toEqual(["command:new", "command:reset"]);
    expect(xclaw?.requires?.config).toEqual(["workspace.dir"]);
    expect(xclaw?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const xclaw = resolveXClawMetadata(frontmatter);
    expect(xclaw?.emoji).toBe("disk");
    expect(xclaw?.events).toEqual(["command:new"]);
  });
});
