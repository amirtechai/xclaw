...
homepage: https://docs.xclaw.ai/automation/hooks#command-logger
metadata:
  {
    "xclaw":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.xclaw/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.xclaw/logs/commands.log ~/.xclaw/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/xclaw`:
   ```
   /home/username/.xclaw/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
