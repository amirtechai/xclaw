...
This directory contains hooks that ship with XClaw. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
- **Managed hooks**: `~/.xclaw/hooks/` (shared across workspaces)
...
Hooks can be configured in `~/.xclaw/xclaw.json`:
...
2. Restart gateway: `pkill -9 -f 'xclaw.*gateway' && pnpm xclaw gateway`
3. Enable the hook: `xclaw hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.xclaw.ai/automation/hooks
