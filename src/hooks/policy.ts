import type { XClawConfig, HookConfig } from "../config/config.js";
...
const HOOK_SOURCE_POLICIES: Record<HookSource, HookSourcePolicy> = {
  "xclaw-bundled": {
    ...
    canOverride: ["xclaw-bundled"],
    canBeOverriddenBy: ["xclaw-managed", "xclaw-plugin"],
  },
  "xclaw-plugin": {
    precedence: 20,
    trustedLocalCode: true,
    defaultEnableMode: "default-on",
    canOverride: ["xclaw-bundled", "xclaw-plugin"],
    canBeOverriddenBy: ["xclaw-managed"],
  },
  "xclaw-managed": {
    precedence: 30,
    trustedLocalCode: true,
    defaultEnableMode: "default-on",
    canOverride: ["xclaw-bundled", "xclaw-managed", "xclaw-plugin"],
    canBeOverriddenBy: ["xclaw-managed"],
  },
  "xclaw-workspace": {
    precedence: 40,
    ...
    canOverride: ["xclaw-workspace"],
    canBeOverriddenBy: ["xclaw-workspace"],
  },
};
...
export function resolveHookEnableState(params: {
  ...
  if (entry.hook.source === "xclaw-plugin") {
    return { enabled: true };
  }
