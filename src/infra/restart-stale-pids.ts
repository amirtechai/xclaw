 ...
 * returning control to the caller (typically `triggerXClawRestart` →
...
/**
 * Parse xclaw gateway PIDs from lsof -Fpc stdout.
...
function parsePidsFromLsofOutput(stdout: string): number[] {
  ...
  for (const line of stdout.split(/\r?\n/).filter(Boolean)) {
    if (line.startsWith("p")) {
      if (currentPid != null && currentCmd && currentCmd.toLowerCase().includes("xclaw")) {
 ...
 * Returns only PIDs that belong to xclaw gateway processes (not the current process).
 */
