...
export async function getTailscaleBinary(): Promise<string> {
  const forcedBinary = process.env.XCLAW_TEST_TAILSCALE_BINARY?.trim();
  if (forcedBinary) {
...
export async function ensureFunnel(
  ...
  try {
    ...
    runtime.error(
      info(
        `Tip: Funnel is optional for XClaw. You can keep running the web gateway without it: \`${formatCliCommand("xclaw gateway")}\``,
      ),
    );
    if (shouldLogVerbose()) {
      const rich = isRich();
