...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.XCLAW_DISABLE_BONJOUR)) {
    return true;
  }
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? trimmed : "XClaw";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(XClaw\)\s*$/i, "").trim() || normalized;
}
...
export async function startGatewayBonjourAdvertiser(
  ...
  const hostnameRaw = process.env.XCLAW_MDNS_HOSTNAME?.trim() || "xclaw";
  const hostname =
    hostnameRaw
      .replace(/\.local$/i, "")
      .split(".")[0]
