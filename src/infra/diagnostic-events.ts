...
function getDiagnosticEventsState(): DiagnosticEventsGlobalState {
  const globalStore = globalThis as typeof globalThis & {
    __xclawDiagnosticEventsState?: DiagnosticEventsGlobalState;
  };
  if (!globalStore.__xclawDiagnosticEventsState) {
    globalStore.__xclawDiagnosticEventsState = {
      seq: 0,
      listeners: new Set<(evt: DiagnosticEventPayload) => void>(),
      dispatchDepth: 0,
    };
  }
  return globalStore.__xclawDiagnosticEventsState;
}

export function isDiagnosticsEnabled(config?: XClawConfig): boolean {
