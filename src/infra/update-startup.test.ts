...
describe("update-startup", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "xclaw-update-check-suite-"));
  });
  ...
  beforeEach(async () => {
    ...
    envSnapshot = captureEnv(["XCLAW_STATE_DIR", "NODE_ENV", "VITEST"]);
    process.env.XCLAW_STATE_DIR = tempDir;
    ...
    if (!loaded) {
      ({ resolveXClawPackageRoot } = await import("./xclaw-root.js"));
    ...
    vi.mocked(resolveXClawPackageRoot).mockClear();
  ...
  function mockPackageInstallStatus() {
    vi.mocked(resolveXClawPackageRoot).mockResolvedValue("/opt/xclaw");
  ...
  it("uses current runtime + entrypoint for default auto-update command execution", async () => {
    ...
    process.argv = [process.execPath, "/opt/xclaw/dist/entry.js"];
    try {
      await runAutoUpdateCheckWithDefaults({
        cfg: createBetaAutoUpdateConfig(),
      });
