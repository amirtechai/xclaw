...
function enableAdvertiserUnitMode(hostname = "test-host") {
  ...
  process.env.XCLAW_MDNS_HOSTNAME = hostname;
}
...
describe("gateway bonjour advertiser", () => {
  ...
  it("does not block on advertise and publishes expected txt keys", async () => {
    ...
    expect(gatewayCall?.[0]?.type).toBe("xclaw-gw");
  ...
  it("normalizes hostnames with domains for service names", async () => {
    ...
    expect(gatewayCall?.[0]?.name).toBe("xclaw (XClaw)");
    expect(gatewayCall?.[0]?.domain).toBe("local");
    expect(gatewayCall?.[0]?.hostname).toBe("xclaw");
    expect((gatewayCall?.[0]?.txt as Record<string, string>)?.lanHost).toBe("xclaw.local");
