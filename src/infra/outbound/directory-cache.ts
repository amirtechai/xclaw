...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: XClawConfig | null = null;
  ...
  get(key: string, cfg: XClawConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: XClawConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: XClawConfig): void {
