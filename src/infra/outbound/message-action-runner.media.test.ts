...
describe("runMessageAction media behavior", () => {
  ...
  describe("sandboxed media validation", () => {
    ...
    it("allows media paths under preferred XClaw tmp root", async () => {
      ...
      try {
        ...
        const hostTmpOutsideXClaw = path.join(os.tmpdir(), "outside-xclaw", "test-media.png");
        await expect(
          runMessageAction({
            cfg: slackConfig,
            action: "send",
