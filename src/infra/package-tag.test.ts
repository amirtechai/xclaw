...
describe("normalizePackageTagInput", () => {
  const packageNames = ["xclaw", "@xclaw/plugin"] as const;
  ...
  it("strips known package-name prefixes before returning the tag", () => {
    expect(normalizePackageTagInput("xclaw@beta", packageNames)).toBe("beta");
    expect(normalizePackageTagInput("@xclaw/plugin@2026.2.24", packageNames)).toBe("2026.2.24");
    expect(normalizePackageTagInput("xclaw@   ", packageNames)).toBeNull();
  });

  it("treats exact known package names as an empty tag", () => {
    expect(normalizePackageTagInput("xclaw", packageNames)).toBeNull();
    expect(normalizePackageTagInput(" @xclaw/plugin ", packageNames)).toBeNull();
  });
  ...
  it("returns trimmed raw values when no package prefix matches", () => {
    ...
    expect(normalizePackageTagInput("xclawer@beta", packageNames)).toBe("xclawer@beta");
  });
});
