...
export function triggerXClawRestart(): RestartAttempt {
  ...
  if (process.platform === "linux") {
    const unit = normalizeSystemdUnit(
      process.env.XCLAW_SYSTEMD_UNIT,
  ...
  const label =
    process.env.XCLAW_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.XCLAW_PROFILE);
