...
const DEFAULT_PACKAGE_NAME = "xclaw";
...
function normalizeTag(tag?: string) {
  return normalizePackageTagInput(tag, ["xclaw", DEFAULT_PACKAGE_NAME]) ?? "latest";
}
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "xclaw-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "xclaw.mjs");
    ...
    const doctorStep = await runStep(
      step("xclaw doctor", doctorArgv, gitRoot, { XCLAW_UPDATE_IN_PROGRESS: "1" }),
    );
