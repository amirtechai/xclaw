...
export const POSIX_XCLAW_TMP_DIR = "/tmp/xclaw";
const TMP_DIR_ACCESS_MODE = fs.constants.W_OK | fs.constants.X_OK;

type ResolvePreferredXClawTmpDirOptions = {
...
export function resolvePreferredXClawTmpDir(
  options: ResolvePreferredXClawTmpDirOptions = {},
): string {
  ...
  const fallback = (): string => {
    const base = tmpdir();
    const suffix = uid === undefined ? "xclaw" : `xclaw-${uid}`;
  ...
  const tryRepairWritableBits = (candidatePath: string): boolean => {
    try {
      ...
      warn(`[xclaw] tightened permissions on temp dir: ${candidatePath}`);
  ...
  const ensureTrustedFallbackDir = (): string => {
    ...
    if (state === "invalid") {
      ...
      throw new Error(`Unsafe fallback XClaw temp dir: ${fallbackPath}`);
    }
    try {
      mkdirSync(fallbackPath, { recursive: true, mode: 0o700 });
      chmodSync(fallbackPath, 0o700);
    } catch {
      throw new Error(`Unable to create fallback XClaw temp dir: ${fallbackPath}`);
    }
  ...
  const existingPreferredState = resolveDirState(POSIX_XCLAW_TMP_DIR);
  if (existingPreferredState === "available") {
    return POSIX_XCLAW_TMP_DIR;
  }
  if (existingPreferredState === "invalid") {
    if (tryRepairWritableBits(POSIX_XCLAW_TMP_DIR)) {
      return POSIX_XCLAW_TMP_DIR;
    }
  ...
  try {
    ...
    mkdirSync(POSIX_XCLAW_TMP_DIR, { recursive: true, mode: 0o700 });
    chmodSync(POSIX_XCLAW_TMP_DIR, 0o700);
    if (
      resolveDirState(POSIX_XCLAW_TMP_DIR) !== "available" &&
      !tryRepairWritableBits(POSIX_XCLAW_TMP_DIR)
    ) {
      return ensureTrustedFallbackDir();
    }
    return POSIX_XCLAW_TMP_DIR;
