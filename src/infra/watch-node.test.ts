...
describe("watch-node script", () => {
  it("wires chokidar watch to run-node with watched source/config paths", async () => {
    ...
    expect(watchOptions.ignored("extensions/voice-call/xclaw.plugin.json")).toBe(false);
    ...
    expect(spawn).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        ...
        env: expect.objectContaining({
          PATH: "/usr/bin",
          XCLAW_WATCH_MODE: "1",
          XCLAW_WATCH_SESSION: "1700000000000-4242",
  ...
  it("ignores test-only changes and restarts on non-test source changes", async () => {
    ...
    watcher.emit("change", "extensions/voice-call/xclaw.plugin.json");
    await new Promise((resolve) => setImmediate(resolve));
