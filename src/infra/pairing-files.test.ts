...
describe("pairing file helpers", () => {
  it("resolves pairing file paths from explicit base dirs", () => {
    expect(resolvePairingPaths("/tmp/xclaw-state", "devices")).toEqual({
      dir: path.join("/tmp/xclaw-state", "devices"),
      pendingPath: path.join("/tmp/xclaw-state", "devices", "pending.json"),
      pairedPath: path.join("/tmp/xclaw-state", "devices", "paired.json"),
    });
  });
