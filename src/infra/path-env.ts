...
function candidateBinDirs(opts: EnsureXClawPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `xclaw` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "xclaw");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.XCLAW_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "xclaw"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `xclaw` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureXClawCliOnPath(opts: EnsureXClawPathOpts = {}) {
  if (isTruthyEnvValue(process.env.XCLAW_PATH_BOOTSTRAPPED)) {
    return;
  }
