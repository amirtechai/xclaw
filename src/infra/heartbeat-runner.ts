...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: XClawConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: XClawConfig) {
...
function resolveHeartbeatAgents(cfg: XClawConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: XClawConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: XClawConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: XClawConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
