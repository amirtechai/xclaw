...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__xclaw_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "xclaw-root");
...
describe("resolveXClawPackageRoot", () => {
  let resolveXClawPackageRoot: typeof import("./xclaw-root.js").resolveXClawPackageRoot;
  let resolveXClawPackageRootSync: typeof import("./xclaw-root.js").resolveXClawPackageRootSync;

  beforeEach(async () => {
    vi.resetModules();
    ({ resolveXClawPackageRoot, resolveXClawPackageRootSync } =
      await import("./xclaw-root.js"));
    state.entries.clear();
  ...
  it("resolves package root from .bin argv1", async () => {
    ...
    const argv1 = path.join(project, "node_modules", ".bin", "xclaw");
    const pkgRoot = path.join(project, "node_modules", "xclaw");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "xclaw" }));

    expect(resolveXClawPackageRootSync({ argv1 })).toBe(pkgRoot);
  });

  it("resolves package root via symlinked argv1", async () => {
    const project = fx("symlink-scenario");
    const bin = path.join(project, "bin", "xclaw");
    const realPkg = path.join(project, "real-pkg");
    state.realpaths.set(abs(bin), abs(path.join(realPkg, "xclaw.mjs")));
    setFile(path.join(realPkg, "package.json"), JSON.stringify({ name: "xclaw" }));

    expect(resolveXClawPackageRootSync({ argv1: bin })).toBe(realPkg);
  });
  ...
  it("prefers moduleUrl candidates", async () => {
    ...
    expect(resolveXClawPackageRootSync({ moduleUrl })).toBe(pkgRoot);
  });

  it("falls through from a non-xclaw moduleUrl candidate to cwd", async () => {
    const wrongPkgRoot = fx("moduleurl-fallthrough", "wrong");
    const cwdPkgRoot = fx("moduleurl-fallthrough", "cwd");
    setFile(path.join(wrongPkgRoot, "package.json"), JSON.stringify({ name: "not-xclaw" }));
    setFile(path.join(cwdPkgRoot, "package.json"), JSON.stringify({ name: "xclaw" }));
    const moduleUrl = pathToFileURL(path.join(wrongPkgRoot, "dist", "index.js")).toString();

    expect(resolveXClawPackageRootSync({ moduleUrl, cwd: cwdPkgRoot })).toBe(cwdPkgRoot);
    await expect(resolveXClawPackageRoot({ moduleUrl, cwd: cwdPkgRoot })).resolves.toBe(
      cwdPkgRoot,
    );
  });
  ...
  it("ignores invalid moduleUrl values and falls back to cwd", async () => {
    ...
    expect(resolveXClawPackageRootSync({ moduleUrl: "not-a-file-url", cwd: pkgRoot })).toBe(
      pkgRoot,
    );
    await expect(
      resolveXClawPackageRoot({ moduleUrl: "not-a-file-url", cwd: pkgRoot }),
    ).resolves.toBe(pkgRoot);
  });

  it("returns null for non-xclaw package roots", async () => {
    const pkgRoot = fx("not-xclaw");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "not-xclaw" }));

    expect(resolveXClawPackageRootSync({ cwd: pkgRoot })).toBeNull();
  });

  it("falls back from a symlinked argv1 to the node_modules package root", () => {
    const project = fx("symlink-node-modules-fallback");
    const argv1 = path.join(project, "node_modules", ".bin", "xclaw");
    state.realpaths.set(abs(argv1), abs(path.join(project, "versions", "current", "xclaw.mjs")));
    const pkgRoot = path.join(project, "node_modules", "xclaw");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "xclaw" }));
  ...
  it("async resolver matches sync behavior", async () => {
    ...
    await expect(resolveXClawPackageRoot({ cwd: pkgRoot })).resolves.toBe(pkgRoot);
  });

  it("async resolver returns null when no package roots exist", async () => {
    await expect(resolveXClawPackageRoot({ cwd: fx("missing") })).resolves.toBeNull();
  });
});
