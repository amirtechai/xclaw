...
describe("resolveEffectiveHomeDir", () => {
  it.each([
    {
      name: "prefers XCLAW_HOME over HOME and USERPROFILE",
      env: {
...
describe("resolveRequiredHomeDir", () => {
  ...
  it("returns a fully resolved path for XCLAW_HOME", () => {
    const result = resolveRequiredHomeDir(
      { XCLAW_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      () => "/fallback",
    );
    expect(result).toBe(path.resolve("/custom/home"));
  });

  it("returns cwd when XCLAW_HOME is tilde-only and no fallback home exists", () => {
    expect(
      resolveRequiredHomeDir({ XCLAW_HOME: "~" } as NodeJS.ProcessEnv, () => {
        throw new Error("no home");
      }),
    ).toBe(process.cwd());
  });
...
describe("resolveOsHomeDir", () => {
  it("ignores XCLAW_HOME and uses HOME", () => {
    expect(
      resolveOsHomeDir(
        {
          XCLAW_HOME: "/srv/xclaw-home",
  ...
  it.each([
    {
      ...
      input: "~/x",
      opts: {
        env: { XCLAW_HOME: "/srv/xclaw-home" } as NodeJS.ProcessEnv,
      },
      expected: `${path.resolve("/srv/xclaw-home")}/x`,
    },
    {
      name: "expands exact ~ using explicit home",
      input: "~",
      opts: { home: " /srv/xclaw-home " },
      expected: "/srv/xclaw-home",
    },
    {
...
describe("resolveHomeRelativePath", () => {
  ...
  it("expands tilde paths using the resolved home directory", () => {
    ...
    ).toBe(path.resolve("/srv/xclaw-home/docs"));
  });
...
describe("resolveOsHomeRelativePath", () => {
  it("expands tilde paths using the OS home instead of XCLAW_HOME", () => {
    expect(
