...
function assertValidGatewayStartupConfigSnapshot(
  ...
  const doctorHint = options.includeDoctorHint
    ? `\nRun "${formatCliCommand("xclaw doctor")}" to repair, then retry.`
    : "";
...
export async function startGatewayServer(
  ...
  const minimalTestGateway =
    process.env.VITEST === "1" && process.env.XCLAW_TEST_MINIMAL_GATEWAY === "1";

  // Ensure all default port derivations (browser/canvas) see the actual runtime port.
  process.env.XCLAW_GATEWAY_PORT = String(port);
  logAcceptedEnvOption({
    key: "XCLAW_RAW_STREAM",
    description: "raw stream logging enabled",
  });
  logAcceptedEnvOption({
    key: "XCLAW_RAW_STREAM_PATH",
  ...
  if (authBootstrap.generatedToken) {
    if (authBootstrap.persistedGeneratedToken) {
      ...
      log.warn(
        "Gateway auth token was missing. Generated a runtime token for this startup without changing config; restart will generate a different token. Persist one with `xclaw config set gateway.auth.mode token` and `xclaw config set gateway.auth.token <token>`.",
      );
    }
  }
  ...
  // WARNING: auth mode "none" disables all authentication — only use in trusted environments.
  if (resolvedAuth.mode === "none") {
    log.warn(
      "WARNING: gateway auth mode is set to 'none' — all authentication is DISABLED. " +
      "Unauthenticated requests will be accepted from any client. " +
      "Do NOT use this mode in production or on untrusted networks.",
    );
  }
  ...
  const transcriptUnsub = minimalTestGateway
    ...
    : onSessionTranscriptUpdate((update) => {
        ...
        const sessionSnapshot = sessionRow
          ...
          : {};
        const message = attachXClawTranscriptMeta(update.message, {
