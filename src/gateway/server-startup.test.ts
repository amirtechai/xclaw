...
vi.mock("../agents/agent-paths.js", () => ({
  resolveXClawAgentDir: () => "/tmp/agent",
}));

vi.mock("../agents/models-config.js", () => ({
  ensureXClawModelsJson: (config: unknown, agentDir: unknown) =>
    ensureXClawModelsJsonMock(config, agentDir),
}));
...
describe("gateway startup primary model warmup", () => {
  beforeEach(() => {
    ensureXClawModelsJsonMock.mockClear();
  ...
  it("prewarms an explicit configured primary model", async () => {
    ...
    expect(ensureXClawModelsJsonMock).toHaveBeenCalledWith(cfg, "/tmp/agent");
  ...
  it("skips warmup when no explicit primary model is configured", async () => {
    ...
    expect(ensureXClawModelsJsonMock).not.toHaveBeenCalled();
