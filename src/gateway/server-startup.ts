...
import type { loadXClawPlugins } from "../plugins/loader.js";
...
async function prewarmConfiguredPrimaryModel(params: {
  ...
  try {
    await ensureXClawModelsJson(params.cfg, agentDir);
...
export async function startGatewaySidecars(params: {
  cfg: ReturnType<typeof loadConfig>;
  pluginRegistry: ReturnType<typeof loadXClawPlugins>;
  ...
  // Start XClaw browser control server (unless disabled via config).
  ...
  // Tests can opt out via XCLAW_SKIP_CHANNELS (or legacy XCLAW_SKIP_PROVIDERS).
  const skipChannels =
    isTruthyEnvValue(process.env.XCLAW_SKIP_CHANNELS) ||
    isTruthyEnvValue(process.env.XCLAW_SKIP_PROVIDERS);
  if (!skipChannels) {
    try {
    ...
    params.logChannels.info(
      "skipping channel start (XCLAW_SKIP_CHANNELS=1 or XCLAW_SKIP_PROVIDERS=1)",
    );
  }
