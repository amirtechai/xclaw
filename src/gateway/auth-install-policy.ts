...
type GatewayInstallAuthMode = NonNullable<NonNullable<XClawConfig["gateway"]>["auth"]>["mode"];
...
function hasConfiguredGatewayPasswordForInstall(cfg: XClawConfig): boolean {
...
function hasDurableGatewayPasswordEnvForInstall(
  ...
  return Boolean(
    durableServiceEnv.XCLAW_GATEWAY_PASSWORD?.trim() ||
