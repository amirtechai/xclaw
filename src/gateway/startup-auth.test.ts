...
const mocks = vi.hoisted(() => ({
  writeConfigFile: vi.fn(async (_cfg: XClawConfig) => {}),
}));
...
describe("ensureGatewayStartupAuth", () => {
  async function expectEphemeralGeneratedTokenWhenOverridden(cfg: XClawConfig) {
  ...
  async function expectNoTokenGeneration(cfg: XClawConfig, mode: string) {
    const result = await ensureGatewayStartupAuth({
      cfg,
      env: {} as NodeJS.ProcessEnv,
      persist: true,
  ...
  function createMissingGatewayTokenSecretRefConfig(): XClawConfig {
    return {
      gateway: {
        auth: {
          mode: "token",
  ...
  it("resolves env-template gateway.auth.token before env-token short-circuiting", async () => {
    await expectResolvedToken({
      ...
      env: {
        XCLAW_GATEWAY_TOKEN: "resolved-token",
      } as NodeJS.ProcessEnv,
      expectedToken: "resolved-token",
      expectedConfiguredToken: "${XCLAW_GATEWAY_TOKEN}",
    });
  });

  it("uses XCLAW_GATEWAY_TOKEN without resolving configured token SecretRef", async () => {
    await expectResolvedToken({
      cfg: createMissingGatewayTokenSecretRefConfig(),
      env: {
        XCLAW_GATEWAY_TOKEN: "token-from-env",
      } as NodeJS.ProcessEnv,
