...
import { createXClawTools } from "../agents/xclaw-tools.js";
...
export async function handleToolsInvokeHttpRequest(
  ...
  const messageChannel = normalizeMessageChannel(
    getHeader(req, "x-xclaw-message-channel") ?? "",
  );
  const accountId = getHeader(req, "x-xclaw-account-id")?.trim() || undefined;
  const agentTo = getHeader(req, "x-xclaw-message-to")?.trim() || undefined;
  const agentThreadId = getHeader(req, "x-xclaw-thread-id")?.trim() || undefined;
  ...
  const allTools = createXClawTools({
