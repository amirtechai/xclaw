...
export function resolveHooksConfig(cfg: XClawConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: XClawConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-xclaw-token"] === "string"
      ? req.headers["x-xclaw-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
...
export function resolveHookIdempotencyKey(params: {
  ...
  return (
    ...
    resolveOptionalHookIdempotencyKey(params.headers?.["x-xclaw-idempotency-key"]) ||
