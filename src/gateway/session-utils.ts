...
import { type XClawConfig, loadConfig } from "../config/config.js";
...
function listConfiguredAgentIds(cfg: XClawConfig): string[] {
...
export function listAgentsForGateway(cfg: XClawConfig): {
  defaultId: string;
  mainKey: string;
  scope: SessionScope;
...
function resolveDefaultStoreAgentId(cfg: XClawConfig): string {
...
function resolveSessionStoreAgentId(cfg: XClawConfig, canonicalKey: string): string {
...
export function loadCombinedSessionStoreForGateway(cfg: XClawConfig): {
  storePath: string;
  store: Record<string, SessionEntry>;
} {
  const storeConfig = cfg.session?.store;
...
export function getSessionDefaults(cfg: XClawConfig): GatewaySessionsDefaults {
