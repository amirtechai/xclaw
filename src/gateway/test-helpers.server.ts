...
const GATEWAY_TEST_ENV_KEYS = [
  ...
  "XCLAW_STATE_DIR",
  "XCLAW_CONFIG_PATH",
  "XCLAW_SKIP_BROWSER_CONTROL_SERVER",
  "XCLAW_SKIP_GMAIL_WATCHER",
  "XCLAW_SKIP_CANVAS_HOST",
  "XCLAW_BUNDLED_PLUGINS_DIR",
  "XCLAW_SKIP_CHANNELS",
  "XCLAW_SKIP_PROVIDERS",
  "XCLAW_SKIP_CRON",
  "XCLAW_TEST_MINIMAL_GATEWAY",
] as const;
...
async function persistTestSessionConfig(): Promise<void> {
  ...
  if (process.env.XCLAW_CONFIG_PATH) {
    configPaths.add(process.env.XCLAW_CONFIG_PATH);
  }
  if (process.env.XCLAW_STATE_DIR) {
    configPaths.add(path.join(process.env.XCLAW_STATE_DIR, "xclaw.json"));
  }
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "xclaw-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.XCLAW_STATE_DIR = path.join(tempHome, ".xclaw");
  delete process.env.XCLAW_CONFIG_PATH;
}

function applyGatewaySkipEnv() {
  process.env.XCLAW_SKIP_BROWSER_CONTROL_SERVER = "1";
  process.env.XCLAW_SKIP_GMAIL_WATCHER = "1";
  process.env.XCLAW_SKIP_CANVAS_HOST = "1";
  process.env.XCLAW_SKIP_CHANNELS = "1";
  process.env.XCLAW_SKIP_PROVIDERS = "1";
  process.env.XCLAW_SKIP_CRON = "1";
  process.env.XCLAW_TEST_MINIMAL_GATEWAY = "1";
  process.env.XCLAW_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "xclaw-test-no-bundled-extensions")
    : "xclaw-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  applyGatewaySkipEnv();
  const stateDir = process.env.XCLAW_STATE_DIR;
  if (stateDir) {
  ...
  if (options.uniqueConfigRoot) {
    const suiteRoot = path.join(tempHome, ".xclaw-test-suite");
    ...
    tempConfigRoot = path.join(tempHome, ".xclaw-test");
    await fs.rm(tempConfigRoot, {
      recursive: true,
      force: true,
      maxRetries: 20,
  ...
  tempControlUiRoot = path.join(tempHome, ".xclaw-test-control-ui");
  ...
  await fs.writeFile(
    ...
    "<!doctype html><title>xclaw-test-control-ui</title>\n",
    "utf-8",
  );
...
const CONNECT_CHALLENGE_NONCE_KEY = "__xclawTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__xclawTestConnectChallengeTracked";
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["XCLAW_GATEWAY_TOKEN"]);
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.XCLAW_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const safe =
    `${params.clientId}-${params.clientMode}-${params.platform}-${params.deviceFamily ?? "none"}-${params.role}`
      ...
      .toLowerCase();
  const suiteRoot = process.env.XCLAW_STATE_DIR ?? process.env.HOME ?? os.tmpdir();
  ...
  const defaultPassword =
    opts?.skipDefaultAuth === true
      ...
      : typeof (testState.gatewayAuth as { password?: unknown } | undefined)?.password === "string"
        ...
        : process.env.XCLAW_GATEWAY_PASSWORD;
