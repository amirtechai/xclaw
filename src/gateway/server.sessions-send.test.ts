...
const { createXClawTools } = await import("../agents/xclaw-tools.js");
...
type SessionSendTool = ReturnType<typeof createXClawTools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createXClawTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["XCLAW_GATEWAY_PORT", "XCLAW_GATEWAY_TOKEN"]);
  gatewayPort = await getFreePort();
  testState.gatewayAuth = { mode: "token", token: gatewayToken };
  process.env.XCLAW_GATEWAY_PORT = String(gatewayPort);
  process.env.XCLAW_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send label lookup", () => {
  it(
    ...
    async () => {
      ...
      if (!configPath) {
        throw new Error("XCLAW_CONFIG_PATH missing in gateway test environment");
      }
