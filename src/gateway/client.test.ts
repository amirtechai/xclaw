...
function expectSecurityConnectError(
  ...
  expect(error.message).toContain("xclaw doctor --fix");
...
describe("GatewayClient security checks", () => {
  ...
  it("allows ws:// to private addresses only with XCLAW_ALLOW_INSECURE_PRIVATE_WS=1", () => {
  ...
  it("allows ws:// hostnames with XCLAW_ALLOW_INSECURE_PRIVATE_WS=1", () => {
    process.env.XCLAW_ALLOW_INSECURE_PRIVATE_WS = "1";
    const onConnectError = vi.fn();
    const client = new GatewayClient({
      url: "ws://xclaw-gateway.ai:18789",
      onConnectError,
    });
