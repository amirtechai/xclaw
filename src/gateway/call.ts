...
export function buildGatewayConnectionDetails(
  ...
  const envUrlOverride = cliUrlOverride
    ? undefined
    : trimToUndefined(process.env.XCLAW_GATEWAY_URL);
  ...
  const allowPrivateWs = process.env.XCLAW_ALLOW_INSECURE_PRIVATE_WS === "1";
  ...
  if (!isSecureWebSocketUrl(url, { allowPrivateWs })) {
    throw new Error(
      [
        ...
        allowPrivateWs
          ? undefined
          : "Break-glass (trusted private networks only): set XCLAW_ALLOW_INSECURE_PRIVATE_WS=1",
        "Doctor: xclaw doctor --fix",
...
function gatewaySecretInputPathCanWin(params: {
  ...
  const sentinel = `__XCLAW_GATEWAY_SECRET_REF_PROBE_${params.path.replaceAll(".", "_")}__`;
