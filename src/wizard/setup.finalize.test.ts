...
const resolveExistingKey = vi.hoisted(() =>
  vi.fn<(config: XClawConfig, provider: string) => string | undefined>(() => undefined),
);
const hasExistingKey = vi.hoisted(() =>
  vi.fn<(config: XClawConfig, provider: string) => boolean>(() => false),
);
...
const listConfiguredWebSearchProviders = vi.hoisted(() =>
  vi.fn<(params?: { config?: XClawConfig }) => PluginWebSearchProviderEntry[]>(() => []),
);
...
describe("finalizeSetupWizard", () => {
  ...
  it("resolves gateway password SecretRef for probe and TUI", async () => {
    ...
    process.env.XCLAW_GATEWAY_PASSWORD = "resolved-gateway-password"; // pragma: allowlist secret
