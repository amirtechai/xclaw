...
function buildCoreToolGroupMap() {
  ...
  const xclawTools = CORE_TOOL_DEFINITIONS.filter((tool) => tool.includeInXClawGroup).map(
    (tool) => tool.id,
  );
  return {
    "group:xclaw": xclawTools,
