...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.XCLAW_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".xclaw", `workspace-${profile}`);
  }
  return path.join(home, ".xclaw", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".xclaw";
