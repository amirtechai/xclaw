    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.acpxRoot, "xclaw.plugin.json"),
      },
      {
        id: "helper",
        name: "Helper",
        ...
        manifestPath: path.join(params.helperRoot, "xclaw.plugin.json"),
      },
    ],
  };
}
    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.pluginRoot, "xclaw.plugin.json"),
      },
    ],
  };
}
...
async function setupAcpxAndHelperRegistry() {
  const workspaceDir = await tempDirs.make("xclaw-");
  const acpxRoot = await tempDirs.make("xclaw-acpx-plugin-");
  const helperRoot = await tempDirs.make("xclaw-helper-plugin-");
...
async function setupPluginOutsideSkills() {
  const workspaceDir = await tempDirs.make("xclaw-");
  const pluginRoot = await tempDirs.make("xclaw-plugin-");
  const outsideDir = await tempDirs.make("xclaw-outside-");
...
describe("resolvePluginSkillDirs", () => {
  ...
  it("resolves Claude bundle command roots through the normal plugin skill path", async () => {
    const workspaceDir = await tempDirs.make("xclaw-");
    const pluginRoot = await tempDirs.make("xclaw-claude-bundle-");
