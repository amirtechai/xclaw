...
type AgentEntry = NonNullable<NonNullable<XClawConfig["agents"]>["list"]>[number];
...
export function listAgentEntries(cfg: XClawConfig): AgentEntry[] {
...
export function listAgentIds(cfg: XClawConfig): string[] {
...
export function resolveDefaultAgentId(cfg: XClawConfig): string {
  const agents = listAgentEntries(cfg);
  if (agents.length === 0) {
    return DEFAULT_AGENT_ID;
  }
...
function resolveAgentEntry(cfg: XClawConfig, agentId: string): AgentEntry | undefined {
...
export function resolveAgentModelPrimary(cfg: XClawConfig, agentId: string): string | undefined {
...
export function resolveAgentWorkspaceDir(cfg: XClawConfig, agentId: string) {
  const id = normalizeAgentId(agentId);
