...
function resolveAttachmentLimits(config: XClawConfig): AttachmentLimits {
...
export async function materializeSubagentAttachments(params: {
  ...
  const absRootDir = path.join(childWorkspaceDir, ".xclaw", "attachments");
  const relDir = path.posix.join(".xclaw", "attachments", attachmentId);
