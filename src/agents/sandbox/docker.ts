...
import { markXClawExecEnv } from "../../infra/xclaw-exec-env.js";
...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "xclaw.sandbox=1");
  args.push("--label", `xclaw.sessionKey=${params.scopeKey}`);
  args.push("--label", `xclaw.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `xclaw.configHash=${params.configHash}`);
  }
  ...
  for (const [key, value] of Object.entries(markXClawExecEnv(envSanitization.allowed))) {
...
function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`xclaw sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`xclaw sandbox recreate --agent ${agentId}`);
  }
