...
function findTool(name: string, config: XClawConfig) {
  const allTools = createXClawTools({ config, sandboxed: true });
...
async function prepareAndActivate(params: { config: XClawConfig; env?: NodeJS.ProcessEnv }) {
...
describe("xclaw tools runtime web metadata wiring", () => {
