...
import { planXClawModelsJson } from "./models-config.plan.js";
...
async function buildModelsJsonFingerprint(params: {
  config: XClawConfig;
  sourceConfigForSecrets: XClawConfig;
  agentDir: string;
}): Promise<string> {
...
function resolveModelsConfigInput(config?: XClawConfig): {
  config: XClawConfig;
  sourceConfigForSecrets: XClawConfig;
} {
...
export async function ensureXClawModelsJson(
  config?: XClawConfig,
  agentDirOverride?: string,
): Promise<{ agentDir: string; wrote: boolean }> {
  const resolved = resolveModelsConfigInput(config);
  const cfg = resolved.config;
  const agentDir = agentDirOverride?.trim() ? agentDirOverride.trim() : resolveXClawAgentDir();
  ...
  const pending = withModelsJsonWriteLock(targetPath, async () => {
    ...
    const plan = await planXClawModelsJson({
      cfg,
      sourceConfigForSecrets: resolved.sourceConfigForSecrets,
      agentDir,
      env,
