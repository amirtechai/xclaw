...
describe("subagent registry persistence", () => {
  const envSnapshot = captureEnv(["XCLAW_STATE_DIR"]);
  ...
  const writePersistedRegistry = async (
    ...
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "xclaw-subagent-"));
  ...
  it("uses isolated temp state when XCLAW_STATE_DIR is unset in tests", async () => {
    ...
    expect(registryPath).toContain(path.join(os.tmpdir(), "xclaw-test-state"));
  });
});
