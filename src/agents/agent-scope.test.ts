...
describe("resolveAgentConfig", () => {
  ...
  it("resolves explicit and effective model primary separately", () => {
    ...
    const cfgWithObjectDefault: XClawConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgNoDefaults: XClawConfig = {
      agents: {
        list: [{ id: "main" }],
      },
    };
  ...
  it("supports per-agent model primary+fallbacks", () => {
    ...
    const cfgNoOverride: XClawConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgInheritDefaults: XClawConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-5.4"],
  ...
  it("computes whether any model fallbacks are configured via shared helper", () => {
    const cfgDefaultsOnly: XClawConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-5.4"],
    ...
    const cfgAgentOverrideOnly: XClawConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: [],
  ...
  it("should normalize agent id", () => {
    ...
    expect(result?.workspace).toBe("~/xclaw");
  });

  it("uses XCLAW_HOME for default agent workspace", () => {
    const home = path.join(path.sep, "srv", "xclaw-home");
    vi.stubEnv("XCLAW_HOME", home);

    const workspace = resolveAgentWorkspaceDir({} as XClawConfig, "main");
    expect(workspace).toBe(path.join(path.resolve(home), ".xclaw", "workspace"));
  });

  it("uses XCLAW_HOME for default agentDir", () => {
    const home = path.join(path.sep, "srv", "xclaw-home");
    vi.stubEnv("XCLAW_HOME", home);
    // Clear state dir so it falls back to XCLAW_HOME
    vi.stubEnv("XCLAW_STATE_DIR", "");

    const agentDir = resolveAgentDir({} as XClawConfig, "main");
    expect(agentDir).toBe(path.join(path.resolve(home), ".xclaw", "agents", "main", "agent"));
  });
});

describe("resolveAgentIdByWorkspacePath", () => {
  it("returns the most specific workspace match for a directory", () => {
    const workspaceRoot = `/tmp/xclaw-agent-scope-${Date.now()}-root`;
  ...
  it("returns undefined when directory has no matching workspace", () => {
    ...
    expect(
      resolveAgentIdByWorkspacePath(cfg, `/tmp/xclaw-agent-scope-${Date.now()}-unrelated`),
    ).toBeUndefined();
  });

  it("matches workspace paths through symlink aliases", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "xclaw-agent-scope-"));
