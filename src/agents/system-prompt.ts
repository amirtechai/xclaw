...
function buildMessagingSection(params: {
  ...
  return [
    ...
    "- Never use exec/curl for provider messaging; XClaw handles all routing internally.",
...
function buildDocsSection(params: { docsPath?: string; isMinimal: boolean; readToolName: string }) {
  ...
  return [
    "## Documentation",
    `XClaw docs: ${docsPath}`,
    "Mirror: https://docs.xclaw.ai",
    "Source: https://github.com/xclaw/xclaw",
...
export function buildAgentSystemPrompt(params: {
  ...
  const coreToolSummaries: Record<string, string> = {
    ...
    gateway: "Restart, apply config, or run updates on the running XClaw process",
    agents_list: acpSpawnRuntimeEnabled
      ? 'List XClaw agent ids allowed for sessions_spawn when runtime="subagent" (not ACP harness ids)'
      : "List XClaw agent ids allowed for sessions_spawn",
  ...
  if (promptMode === "none") {
    return "You are a personal assistant running inside XClaw.";
  }

  const lines = [
    "You are a personal assistant running inside XClaw.",
    "",
    "## Tooling",
    ...
    toolLines.length > 0
      ...
      : [
          ...
          "- browser: control XClaw's dedicated browser",
          "- canvas: present/eval/snapshot the Canvas",
          "- nodes: list/describe/notify/camera/screen on paired nodes",
          "- cron: manage cron jobs and wake events (use for reminders; when scheduling a reminder, write the systemEvent text as something that will read like a reminder when it fires, and mention that it is a reminder depending on the time gap between setting and firing; include recent context in reminder text if appropriate)",
    ...
    "",
    ...safetySection,
    "## XClaw CLI Quick Reference",
    "XClaw is controlled via subcommands. Do not invent commands.",
    ...
    hasGateway && !isMinimal ? "## XClaw Self-Update" : "",
    hasGateway && !isMinimal
      ? [
          ...
          "After restart, XClaw pings the last active session automatically.",
        ].join("\n")
      : "",
    hasGateway && !isMinimal ? "" : "",
    "",
    ...
    "These user-editable files are loaded by XClaw and included below in Project Context.",
    "",
  ...
  if (!isMinimal && heartbeatPrompt) {
    lines.push(
      ...
      "HEARTBEAT_OK",
      'XClaw treats a leading/trailing "HEARTBEAT_OK" as a heartbeat ack (and may discard it).',
