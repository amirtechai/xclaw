...
let configuredConfig: XClawConfig | undefined;
...
function isLikelyXClawCliProcess(argv: string[] = process.argv): boolean {
...
function shouldEagerWarmContextWindowCache(argv: string[] = process.argv): boolean {
  // Keep this gate tied to the real XClaw CLI entrypoints.
  //
  ...
  // built plugin-sdk can call ensureXClawModelsJson(), which cascades into
  // plugin discovery and breaks dist/source singleton assumptions.
  if (!isLikelyXClawCliProcess(argv)) {
    return false;
  }
...
function primeConfiguredContextWindows(): XClawConfig | undefined {
  if (configuredConfig) {
...
function ensureContextWindowCacheLoaded(): Promise<void> {
  ...
  loadPromise = (async () => {
    try {
      await (await loadModelsConfigRuntime()).ensureXClawModelsJson(cfg);
    } catch {
