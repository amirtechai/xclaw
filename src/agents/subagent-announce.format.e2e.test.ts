...
let configOverride: XClawConfig = {
  session: {
    mainKey: "main",
    scope: "per-sender",
  },
...
function setConfigOverride(next: XClawConfig): void {
  configOverride = next;
...
describe("subagent announce formatting", () => {
  ...
  beforeAll(async () => {
    ...
    // See: https://github.com/xclaw/xclaw/issues/31298
    previousFastTestEnv = process.env.XCLAW_TEST_FAST;
  ...
  afterAll(() => {
    ...
    if (previousFastTestEnv === undefined) {
      delete process.env.XCLAW_TEST_FAST;
      return;
    }
    process.env.XCLAW_TEST_FAST = previousFastTestEnv;
  });

  beforeEach(() => {
    // XCLAW_TEST_FAST is set in beforeAll before module import
  ...
  it("sends instructional message to main agent with status and findings", async () => {
    ...
    expect(msg).toContain("XClaw runtime context (internal):");
