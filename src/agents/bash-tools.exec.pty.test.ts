...
test("exec sets XCLAW_SHELL in pty mode", async () => {
  const tool = createExecTool({ allowBackground: false, security: "full", ask: "off" });
  const result = await tool.execute("toolcall-xclaw-shell", {
    command: "node -e \"process.stdout.write(process.env.XCLAW_SHELL || '')\"",
    pty: true,
  });
