...
import { resolvePreferredXClawTmpDir } from "../infra/tmp-xclaw-dir.js";
...
async function withOutsideHardlinkInXClawTmp<T>(
  params: {
    xClawTmpDir: string;
  ...
  const hardlinkPath = path.join(params.xClawTmpDir, makeTmpProbePath(params.hardlinkPrefix));
  const symlinkPath = params.symlinkPrefix
    ? path.join(params.xClawTmpDir, makeTmpProbePath(params.symlinkPrefix))
    : undefined;
  try {
    if (isPathInside(params.xClawTmpDir, outsideFile)) {
      return;
    }
    await fs.writeFile(outsideFile, "secret", "utf8");
    await fs.mkdir(params.xClawTmpDir, { recursive: true });
    try {
...
describe("resolveSandboxedMediaSource", () => {
  const xClawTmpDir = resolvePreferredXClawTmpDir();

  // Group 1: /tmp paths (the bug fix)
  it.each([
    {
      name: "absolute paths under preferred XClaw tmp root",
      media: path.join(xClawTmpDir, "image.png"),
      expected: path.join(xClawTmpDir, "image.png"),
    },
    {
      name: "file:// URLs pointing to preferred XClaw tmp root",
      media: pathToFileURL(path.join(xClawTmpDir, "photo.png")).href,
      expected: path.join(xClawTmpDir, "photo.png"),
    },
    {
      name: "nested paths under preferred XClaw tmp root",
      media: path.join(xClawTmpDir, "subdir", "deep", "file.png"),
      expected: path.join(xClawTmpDir, "subdir", "deep", "file.png"),
    },
  ...
  it.each([
    ...
    },
    {
      ...
      media: path.join(xClawTmpDir, "..", "etc", "passwd"),
      expected: /sandbox/i,
    },
    {
      name: "absolute paths under host tmp outside xclaw tmp root",
      media: path.join(os.tmpdir(), "outside-xclaw", "passwd"),
      expected: /sandbox/i,
    },
    {
  ...
  it("rejects symlinked XClaw tmp paths escaping tmp root", async () => {
    if (process.platform === "win32") {
      return;
    }
    const outsideTmpTarget = path.resolve(process.cwd(), "package.json");
    if (isPathInside(xClawTmpDir, outsideTmpTarget)) {
      return;
    }

    await withSandboxRoot(async (sandboxDir) => {
      await fs.access(outsideTmpTarget);
      await fs.mkdir(xClawTmpDir, { recursive: true });
      const symlinkPath = path.join(xClawTmpDir, `tmp-link-escape-${process.pid}`);
  ...
  it("rejects hardlinked XClaw tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInXClawTmp(
      {
        xClawTmpDir,
  ...
  it("rejects symlinked XClaw tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInXClawTmp(
