...
const originalBundledDir = process.env.XCLAW_BUNDLED_PLUGINS_DIR;
...
async function setupBundledDiffsPlugin() {
  const bundledPluginsDir = await createTempDir("xclaw-bundled-");
  const workspaceDir = await createTempDir("xclaw-workspace-");
...
async function resolveBundledDiffsSkillEntries(config?: XClawConfig) {
  const { bundledPluginsDir, workspaceDir } = await setupBundledDiffsPlugin();
  process.env.XCLAW_BUNDLED_PLUGINS_DIR = bundledPluginsDir;
...
afterEach(async () => {
  process.env.XCLAW_BUNDLED_PLUGINS_DIR = originalBundledDir;
  clearPluginManifestRegistryCache();
  await Promise.all(
