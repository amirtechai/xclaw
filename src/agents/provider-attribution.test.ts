...
describe("provider attribution", () => {
  it("resolves the canonical XClaw product and runtime version", () => {
    const identity = resolveProviderAttributionIdentity({
      XCLAW_VERSION: "2026.3.99",
    });
  ...
  it("returns a documented OpenRouter attribution policy", () => {
    ...
    expect(policy).toEqual({
      ...
      reviewNote: "Documented app attribution headers. Verified in XClaw runtime wrapper.",
      product: "XClaw",
      version: "2026.3.22",
      headers: {
        "HTTP-Referer": "https://xclaw.ai",
        "X-OpenRouter-Title": "XClaw",
  ...
  it("returns a hidden-spec OpenAI attribution policy", () => {
    expect(resolveProviderAttributionPolicy("openai", { XCLAW_VERSION: "2026.3.22" })).toEqual({
      provider: "openai",
      enabledByDefault: true,
      ...
      headers: {
        originator: "xclaw",
        "User-Agent": "xclaw/2026.3.22",
      },
    });
    expect(resolveProviderAttributionHeaders("openai", { XCLAW_VERSION: "2026.3.22" })).toEqual({
      originator: "xclaw",
      "User-Agent": "xclaw/2026.3.22",
    });
  });
  ...
  it("returns a hidden-spec OpenAI Codex attribution policy", () => {
    expect(
      resolveProviderAttributionPolicy("openai-codex", { XCLAW_VERSION: "2026.3.22" }),
    ).toEqual({
  ...
  it("lists the current attribution support matrix", () => {
    expect(
      listProviderAttributionPolicies({ XCLAW_VERSION: "2026.3.22" }).map((policy) => [
        policy.provider,
