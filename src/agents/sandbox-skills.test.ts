...
vi.mock("./sandbox/docker.js", () => ({
  ensureSandboxContainer: vi.fn(async () => "xclaw-sbx-test"),
}));
...
describe("sandbox skill mirroring", () => {
  ...
  const runContext = async (workspaceAccess: "none" | "ro") => {
    const bundledDir = await fs.mkdtemp(path.join(os.tmpdir(), "xclaw-bundled-skills-"));
    await fs.mkdir(bundledDir, { recursive: true });

    process.env.XCLAW_BUNDLED_SKILLS_DIR = bundledDir;
