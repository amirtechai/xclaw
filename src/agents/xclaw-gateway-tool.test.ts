...
let createXClawTools: typeof import("./xclaw-tools.js").createXClawTools;

async function loadFreshXClawToolsModuleForTest() {
  vi.resetModules();
  vi.doMock("./tools/gateway.js", () => createGatewayToolModuleMocks());
  ({ createXClawTools } = await import("./xclaw-tools.js"));
}
...
describe("gateway tool", () => {
  beforeEach(async () => {
    await loadFreshXClawToolsModuleForTest();
  });
  ...
  it("schedules SIGUSR1 restart", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "xclaw-test-"));

    try {
      await withEnvAsync(
        { XCLAW_STATE_DIR: stateDir, XCLAW_PROFILE: "isolated" },
        async () => {
          const tool = requireGatewayTool();
          ...
          expect(parsed.payload?.doctorHint).toBe(
            "Run: xclaw --profile isolated doctor --non-interactive",
          );
  ...
  it("passes config.apply through gateway call", async () => {
    ...
    const raw = '{\n  agents: { defaults: { workspace: "~/xclaw" } }\n}\n';
