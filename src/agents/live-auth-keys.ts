...
const GOOGLE_LIVE_SINGLE_KEY = "XCLAW_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "XCLAW_LIVE_ANTHROPIC_KEY",
    listVar: "XCLAW_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "XCLAW_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `XCLAW_LIVE_${base}_KEY`;
