...
async function resolveAllowedTmpMediaPath(params: {
  ...
  const xClawTmpDir = path.resolve(resolvePreferredXClawTmpDir());
  if (!isPathInside(xClawTmpDir, resolved)) {
    return undefined;
  }
  await assertNoTmpAliasEscape({ filePath: resolved, tmpRoot: xClawTmpDir });
  return resolved;
}
