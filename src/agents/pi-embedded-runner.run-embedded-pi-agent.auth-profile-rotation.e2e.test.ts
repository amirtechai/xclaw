...
const installRunEmbeddedMocks = () => {
  ...
  vi.doMock("./models-config.js", async (importOriginal) => {
    ...
    return {
      ...mod,
      ensureXClawModelsJson: vi.fn(async () => ({ wrote: false })),
    };
  });
};
...
const makeConfig = (opts?: { fallbacks?: string[]; apiKey?: string }): XClawConfig =>
  ({
    agents: {
      defaults: {
        model: {
...
const makeAgentOverrideOnlyFallbackConfig = (agentId: string): XClawConfig =>
  ({
    agents: {
      defaults: {
        model: {
...
const makeCopilotConfig = (): XClawConfig =>
  ({
    models: {
      providers: {
        "github-copilot": {
...
async function withTimedAgentWorkspace<T>(
  ...
  vi.useFakeTimers();
  try {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "xclaw-agent-"));
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "xclaw-workspace-"));
    const now = Date.now();
...
describe("runEmbeddedPiAgent auth profile rotation", () => {
  ...
  it("logs structured failover decision metadata for overloaded assistant rotation", async () => {
    ...
    setLoggerOverrideFn({
      level: "trace",
      consoleLevel: "silent",
      file: path.join(os.tmpdir(), `xclaw-auth-rotation-${Date.now()}.log`),
    });
