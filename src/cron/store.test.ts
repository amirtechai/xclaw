...
const { makeStorePath } = createCronStoreHarness({ prefix: "xclaw-cron-store-" });
...
describe("resolveCronStorePath", () => {
  ...
  it("uses XCLAW_HOME for tilde expansion", () => {
    vi.stubEnv("XCLAW_HOME", "/srv/xclaw-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/xclaw-home", "cron", "jobs.json"));
  });
});
