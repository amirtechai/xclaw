...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeHelper(fn, { prefix: "xclaw-cron-submodel-" });
}

async function writeSessionStore(home: string) {
  const dir = path.join(home, ".xclaw", "sessions");
...
function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<XClawConfig> = {},
): XClawConfig {
  const base: XClawConfig = {
    agents: {
      defaults: {
...
async function runSubagentModelCase(params: {
  home: string;
  cfgOverrides?: Partial<XClawConfig>;
