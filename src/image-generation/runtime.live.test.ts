...
const REQUIRE_PROFILE_KEYS = isTruthyEnvValue(process.env.XCLAW_LIVE_REQUIRE_PROFILE_KEYS);
...
function withPluginsEnabled(cfg: XClawConfig): XClawConfig {
  return {
    ...cfg,
    plugins: {
      ...cfg.plugins,
...
describeLive("image generation live (provider sweep)", () => {
  it("generates images for every configured image-generation variant with available auth", async () => {
    ...
    const providerFilter = parseCsvFilter(process.env.XCLAW_LIVE_IMAGE_GENERATION_PROVIDERS);
    const caseFilter = parseCaseFilter(process.env.XCLAW_LIVE_IMAGE_GENERATION_CASES);
    const envModelMap = parseProviderModelMap(process.env.XCLAW_LIVE_IMAGE_GENERATION_MODELS);
