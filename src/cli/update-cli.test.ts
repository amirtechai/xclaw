...
import type { XClawConfig, ConfigFileSnapshot } from "../config/types.xclaw.js";
...
vi.mock("../infra/xclaw-root.js", () => ({
  resolveXClawPackageRoot: vi.fn(),
}));
...
const { resolveXClawPackageRoot } = await import("../infra/xclaw-root.js");
...
describe("update-cli", () => {
  const fixtureRoot = "/tmp/xclaw-update-tests";
  let fixtureCount = 0;
  ...
  const baseConfig = {} as XClawConfig;
  const baseSnapshot: ConfigFileSnapshot = {
    path: "/tmp/xclaw-config.json",
    exists: true,
    raw: "{}",
    parsed: {},
    resolved: baseConfig,
  ...
  const mockPackageInstallStatus = (root: string) => {
    vi.mocked(resolveXClawPackageRoot).mockResolvedValue(root);
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = createCaseDir("xclaw-update");
    setTty(false);
  ...
  const setupUpdatedRootRefresh = (params?: {
    ...
    const root = createCaseDir("xclaw-updated-root");
  ...
  beforeEach(() => {
    ...
    vi.mocked(resolveXClawPackageRoot).mockResolvedValue(process.cwd());
    ...
    readPackageName.mockResolvedValue("xclaw");
    ...
    prepareRestartScript.mockResolvedValue("/tmp/xclaw-restart-test.sh");
    runRestartScript.mockResolvedValue(undefined);
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners: [{ pid: 4242, command: "xclaw-gateway" }],
      hints: [],
    });
  ...
  it("parses update status --json as the subcommand option", async () => {
    ...
    await program.parseAsync(["node", "xclaw", "update", "status", "--json"]);
  ...
  it.each([
    ...
    },
    {
      ...
      prepare: async () => {
        vi.mocked(readConfigFileSnapshot).mockResolvedValue({
          ...baseSnapshot,
          config: { update: { channel: "beta" } } as XClawConfig,
        });
      },
    ...
    async ({ mode, options, prepare, expectedChannel, expectedTag, expectedPersistedChannel }) => {
      ...
      if (expectedChannel !== undefined) {
        ...
        expect(runGatewayUpdate).not.toHaveBeenCalled();
        expect(runCommandWithTimeout).toHaveBeenCalledWith(
          ["npm", "i", "-g", "xclaw@latest", "--no-fund", "--no-audit", "--loglevel=error"],
          expect.any(Object),
        );
      }
    ...
    for (const scenario of [
      {
        ...
        run: async () => {
          mockPackageInstallStatus(createCaseDir("xclaw-update"));
          await updateCommand({ tag: "next" });
        },
        expectedSpec: "xclaw@next",
      },
      {
        name: "main shorthand",
        run: async () => {
        ...
        expectedSpec: "github:xclaw/xclaw#main",
      },
      {
        name: "explicit git package spec",
        run: async () => {
          mockPackageInstallStatus(createCaseDir("xclaw-update"));
          await updateCommand({ yes: true, tag: "github:xclaw/xclaw#main" });
        },
        expectedSpec: "github:xclaw/xclaw#main",
      },
      {
        name: "XCLAW_UPDATE_PACKAGE_SPEC override",
        run: async () => {
          mockPackageInstallStatus(createCaseDir("xclaw-update"));
          await withEnvAsync(
        ...
        expectedSpec: "http://10.211.55.2:8138/xclaw-next.tgz",
      },
    ]) {
      vi.clearAllMocks();
      readPackageName.mockResolvedValue("xclaw");
  ...
  it("fails package updates when the installed correction version does not match the requested target", async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({ name: "xclaw", version: "2026.3.23" }),
      "utf-8",
    );
  ...
  it("prepends portable Git PATH for package updates on Windows", async () => {
    ...
    const tempDir = createCaseDir("xclaw-update");
    const localAppData = createCaseDir("xclaw-localappdata");
  ...
  it.each([
    ...
    },
    {
      ...
      invoke: async () => {
        await withEnvAsync(
          {
            XCLAW_STATE_DIR: "./state",
            XCLAW_CONFIG_PATH: "./config/xclaw.json",
          },
          async () => {
            await updateCommand({});
          },
      ...
      expectedOptions: (root: string) =>
        expect.objectContaining({
          ...
          env: expect.objectContaining({
            XCLAW_STATE_DIR: path.resolve("./state"),
            XCLAW_CONFIG_PATH: path.resolve("./config/xclaw.json"),
          }),
          timeoutMs: 60_000,
        }),
      assertExtra: () => {
    ...
    },
    {
      ...
      expectedOptions: (_root: string, context?: { originalCwd: string }) =>
        expect.objectContaining({
          ...
          env: expect.objectContaining({
            XCLAW_STATE_DIR: path.resolve(context?.originalCwd ?? process.cwd(), "./state"),
          }),
          timeoutMs: 60_000,
        }),
      assertExtra: () => {
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    try {
      await withEnvAsync({ XCLAW_UPDATE_IN_PROGRESS: undefined }, async () => {
        vi.mocked(runGatewayUpdate).mockResolvedValue(makeOkUpdateResult());
        vi.mocked(runDaemonRestart).mockResolvedValue(true);
        vi.mocked(doctorCommand).mockResolvedValue(undefined);
        vi.mocked(defaultRuntime.log).mockClear();
        ...
        expect(process.env.XCLAW_UPDATE_IN_PROGRESS).toBeUndefined();
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = createCaseDir("xclaw-update-wizard");
    await withEnvAsync({ XCLAW_GIT_DIR: tempDir }, async () => {
      setTty(true);
  ...
  it("uses ~/xclaw as the default dev checkout directory", async () => {
    const homedirSpy = vi.spyOn(os, "homedir").mockReturnValue("/tmp/oc-home");
    await withEnvAsync({ XCLAW_GIT_DIR: undefined }, async () => {
      expect(resolveGitInstallDir()).toBe(path.posix.join("/tmp/oc-home", "xclaw"));
    });
