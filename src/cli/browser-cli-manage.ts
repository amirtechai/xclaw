...
async function runBrowserToggle(
  ...
  const name = status.profile ?? "xclaw";
...
export function registerBrowserManageCommands(
        ...
        defaultRuntime.log(
          profiles
            .map((p) => {
              ...
              const driver = p.driver !== "xclaw" ? ` [${p.driver}]` : "";
  ...
  browser
    ...
    .option("--driver <driver>", "Profile driver (xclaw|existing-session). Default: xclaw")
    .action(
      async (
        opts: {
          name: string;
