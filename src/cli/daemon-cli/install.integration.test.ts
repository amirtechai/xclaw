...
describe("runDaemonInstall integration", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await makeTempWorkspace("xclaw-daemon-install-int-");
    configPath = path.join(tempHome, "xclaw.json");
    process.env.HOME = tempHome;
    process.env.XCLAW_STATE_DIR = tempHome;
  ...
  it("auto-mints token when no source exists without embedding it into service env", async () => {
    ...
    expect(installEnv?.XCLAW_GATEWAY_TOKEN).toBeUndefined();
  });
});
