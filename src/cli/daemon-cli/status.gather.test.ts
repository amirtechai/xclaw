...
>(async (_env?: NodeJS.ProcessEnv) => ({
  ...
  environment: {
    XCLAW_STATE_DIR: "/tmp/xclaw-daemon",
    XCLAW_CONFIG_PATH: "/tmp/xclaw-daemon/xclaw.json",
  },
}));
...
const resolveStateDir = vi.fn(
  (env: NodeJS.ProcessEnv) => env.XCLAW_STATE_DIR ?? "/tmp/xclaw-cli",
);
const resolveConfigPath = vi.fn((env: NodeJS.ProcessEnv, stateDir: string) => {
  return env.XCLAW_CONFIG_PATH ?? `${stateDir}/xclaw.json`;
});
...
vi.mock("../../config/config.js", () => ({
  createConfigIO: ({ configPath }: { configPath: string }) => {
    const isDaemon = configPath.includes("/xclaw-daemon/");
    return {
...
describe("gatherDaemonStatus", () => {
  ...
  beforeEach(() => {
    ...
    process.env.XCLAW_STATE_DIR = "/tmp/xclaw-cli";
    process.env.XCLAW_CONFIG_PATH = "/tmp/xclaw-cli/xclaw.json";
  ...
  it("reuses command environment when reading runtime status", async () => {
    ...
    serviceReadRuntime.mockImplementationOnce(async (env?: NodeJS.ProcessEnv) => ({
      status: env?.XCLAW_GATEWAY_PORT === "19001" ? "running" : "unknown",
      detail: env?.XCLAW_GATEWAY_PORT ?? "missing-port",
    }));
  ...
  it("keeps remote probe auth strict when remote token is missing", async () => {
    ...
    process.env.XCLAW_GATEWAY_PASSWORD = "env-password"; // pragma: allowlist secret
  ...
  it("surfaces stale gateway listener pids from restart health inspection", async () => {
    inspectGatewayRestart.mockResolvedValueOnce({
      ...
      portUsage: {
        port: 19001,
        status: "busy",
        listeners: [{ pid: 9000, ppid: 8999, commandLine: "xclaw-gateway" }],
        hints: [],
      },
      healthy: false,
