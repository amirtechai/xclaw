...
const CLI_PREFIX_RE = /^(?:pnpm|npm|bunx|npx)\s+xclaw\b|^xclaw\b/;
const CONTAINER_FLAG_RE = /(?:^|\s)--container(?:\s|=|$)/;
const PROFILE_FLAG_RE = /(?:^|\s)--profile(?:\s|=|$)/;
const DEV_FLAG_RE = /(?:^|\s)--dev(?:\s|$)/;
const UPDATE_COMMAND_RE =
  /^(?:pnpm|npm|bunx|npx)\s+xclaw\b.*(?:^|\s)update(?:\s|$)|^xclaw\b.*(?:^|\s)update(?:\s|$)/;
...
export function formatCliCommand(
  ...
  const container = env.XCLAW_CONTAINER_HINT?.trim();
  const profile = normalizeProfileName(env.XCLAW_PROFILE);
