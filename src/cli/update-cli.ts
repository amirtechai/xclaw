...
export function registerUpdateCli(program: Command) {
  ...
  const update = program
    .command("update")
    .description("Update XClaw and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["xclaw update", "Update a source checkout (git)"],
        ["xclaw update --channel beta", "Switch to beta channel (git + npm)"],
        ["xclaw update --channel dev", "Switch to dev channel (git + npm)"],
        ["xclaw update --tag beta", "One-off update to a dist-tag or version"],
        ["xclaw update --tag main", "One-off package install from GitHub main"],
        ["xclaw update --dry-run", "Preview actions without changing anything"],
        ["xclaw update --no-restart", "Update without restarting the service"],
        ["xclaw update --json", "Output result as JSON"],
        ["xclaw update --yes", "Non-interactive (accept downgrade prompts)"],
        ["xclaw update wizard", "Interactive update wizard"],
        ["xclaw --update", "Shorthand for xclaw update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run xclaw update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.xclaw.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["xclaw update status", "Show channel + version status."],
          ["xclaw update status --json", "JSON output."],
          ["xclaw update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.xclaw.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
