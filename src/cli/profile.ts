...
function resolveProfileStateDir(
  ...
  return path.join(resolveRequiredHomeDir(env as NodeJS.ProcessEnv, homedir), `.xclaw${suffix}`);
}
...
export function applyCliProfileEnv(params: {
  ...
  env.XCLAW_PROFILE = profile;

  const stateDir = env.XCLAW_STATE_DIR?.trim() || resolveProfileStateDir(profile, env, homedir);
  if (!env.XCLAW_STATE_DIR?.trim()) {
    env.XCLAW_STATE_DIR = stateDir;
  }

  if (!env.XCLAW_CONFIG_PATH?.trim()) {
    env.XCLAW_CONFIG_PATH = path.join(stateDir, "xclaw.json");
  }

  if (profile === "dev" && !env.XCLAW_GATEWAY_PORT?.trim()) {
    env.XCLAW_GATEWAY_PORT = "19001";
  }
}
