    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["xclaw nodes status", "List known nodes with live status."],
          ["xclaw nodes pairing pending", "Show pending node pairing requests."],
          ['xclaw nodes run --node <id> --raw "uname -a"', "Run a shell command on a node."],
          ["xclaw nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.xclaw.ai/cli/nodes")}\n`,
    );
