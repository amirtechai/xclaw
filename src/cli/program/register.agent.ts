...
${formatHelpExamples([
  ['xclaw agent --to +15555550123 --message "status update"', "Start a new session."],
  ['xclaw agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'xclaw agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'xclaw agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['xclaw agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'xclaw agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.xclaw.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.xclaw.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['xclaw agents set-identity --agent main --name "XClaw" --emoji "🦞"', "Set name + emoji."],
  ["xclaw agents set-identity --agent main --avatar avatars/xclaw.png", "Set avatar path."],
  [
    "xclaw agents set-identity --workspace ~/.xclaw/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "xclaw agents set-identity --identity-file ~/.xclaw/workspace/IDENTITY.md --agent main",
