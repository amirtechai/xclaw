...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "xclaw-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "xclaw-bundled-skills-test-"));
    process.env.XCLAW_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
