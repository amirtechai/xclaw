...
describe("npm-resolution helpers", () => {
  it("keeps original spec when pin is disabled", () => {
    const result = resolvePinnedNpmSpec({
      rawSpec: "@xclaw/plugin-alpha@latest",
      pin: false,
      resolvedSpec: "@xclaw/plugin-alpha@1.2.3",
    });
    expect(result).toEqual({
      recordSpec: "@xclaw/plugin-alpha@latest",
    });
  });
  ...
  it("returns pinned spec notice when resolved spec is available", () => {
    ...
    expect(result).toEqual({
      recordSpec: "@xclaw/plugin-alpha@1.2.3",
      pinNotice: "Pinned npm install record to @xclaw/plugin-alpha@1.2.3.",
    });
  });
  ...
  it("maps npm resolution metadata to install fields", () => {
    expect(
      ...
      resolvedName: "@xclaw/plugin-alpha",
  ...
  it("builds common npm install record fields", () => {
    expect(
      buildNpmInstallRecordFields({
        spec: "@xclaw/plugin-alpha@1.2.3",
        installPath: "/tmp/xclaw/extensions/alpha",
        version: "1.2.3",
        resolution: {
          name: "@xclaw/plugin-alpha",
          version: "1.2.3",
  ...
  it("resolves pinned install record and emits pin notice", () => {
    ...
    expect(logs).toEqual(["Pinned npm install record to @xclaw/plugin-alpha@1.2.3."]);
  ...
  it("resolves pinned install record for CLI and formats warning output", () => {
    ...
    const record = resolvePinnedNpmInstallRecordForCli(
      "@xclaw/plugin-alpha@latest",
      true,
      "/tmp/xclaw/extensions/alpha",
      "1.2.3",
      undefined,
      (message) => logs.push(message),
    ...
    expect(record).toEqual({
      source: "npm",
      spec: "@xclaw/plugin-alpha@latest",
      installPath: "/tmp/xclaw/extensions/alpha",
      version: "1.2.3",
