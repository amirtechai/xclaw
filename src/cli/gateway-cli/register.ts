      ...
      .addHelpText(
        ...
        () =>
          `\n${theme.heading("Examples:")}\n${formatHelpExamples([
            ["xclaw gateway run", "Run the gateway in the foreground."],
            ["xclaw gateway status", "Show service status and probe reachability."],
            ["xclaw gateway discover", "Find local and wide-area gateway beacons."],
            ["xclaw gateway call health", "Call a gateway RPC method directly."],
          ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/gateway", "docs.xclaw.ai/cli/gateway")}\n`,
      ),
  );
