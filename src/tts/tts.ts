...
export function resolveTtsConfig(cfg: XClawConfig): ResolvedTtsConfig {
...
export function resolveTtsPrefsPath(config: ResolvedTtsConfig): string {
  ...
  const envPath = process.env.XCLAW_TTS_PREFS?.trim();
  if (envPath) {
...
export function buildTtsSystemPromptHint(cfg: XClawConfig): string | undefined {
...
export function resolveTtsProviderOrder(primary: TtsProvider, cfg?: XClawConfig): TtsProvider[] {
...
export async function textToSpeech(params: {
  ...
  const tempRoot = resolvePreferredXClawTmpDir();
