...
function snapshotCurrentStateDirVars(): EnvSnapshot {
  return {
    xclaw: process.env.XCLAW_STATE_DIR,
  };
}

function expectStateDirVars(snapshot: EnvSnapshot) {
  expect(process.env.XCLAW_STATE_DIR).toBe(snapshot.xclaw);
}
...
describe("state-dir-env helpers", () => {
  it("set/snapshot/restore round-trips XCLAW_STATE_DIR", () => {
    const prev = snapshotCurrentStateDirVars();
    const snapshot = snapshotStateDirEnv();

    setStateDirEnv("/tmp/xclaw-state-dir-test");
    expect(process.env.XCLAW_STATE_DIR).toBe("/tmp/xclaw-state-dir-test");
  ...
  it("withStateDirEnv sets env for callback and cleans up temp root", async () => {
    ...
    await withStateDirEnv("xclaw-state-dir-env-", async ({ tempRoot, stateDir }) => {
      capturedTempRoot = tempRoot;
      capturedStateDir = stateDir;
      expect(process.env.XCLAW_STATE_DIR).toBe(stateDir);
  ...
  it("withStateDirEnv restores env and cleans temp root when callback throws", async () => {
    ...
    let capturedStateDir = "";
    await expect(
      withStateDirEnv("xclaw-state-dir-env-", async ({ tempRoot, stateDir }) => {
        capturedTempRoot = tempRoot;
        capturedStateDir = stateDir;
