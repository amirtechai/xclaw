  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          xclaw: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  ...
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.XCLAW_GATEWAY_PORT;
    process.env.XCLAW_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.XCLAW_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.XCLAW_GATEWAY_PASSWORD;
