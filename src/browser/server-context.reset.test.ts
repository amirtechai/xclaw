...
function localXClawProfile(): Parameters<typeof createProfileResetOps>[0]["profile"] {
  return {
    name: "xclaw",
...
function createLocalXClawResetOps(
  params: Omit<Parameters<typeof createProfileResetOps>[0], "profile">,
) {
  return createProfileResetOps({ profile: localXClawProfile(), ...params });
}
...
function createStatelessResetOps(profile: Parameters<typeof createProfileResetOps>[0]["profile"]) {
  return createProfileResetOps({
    ...
    resolveXClawUserDataDir: (name: string) => `/tmp/${name}`,
  });
}
...
describe("createProfileResetOps", () => {
  ...
  it("stops local browser, closes playwright connection, and trashes profile dir", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "xclaw-reset-"));
    const profileDir = path.join(tempRoot, "xclaw");
    ...
    const ops = createLocalXClawResetOps({
      getProfileState,
      stopRunningBrowser,
      isHttpReachable,
      resolveXClawUserDataDir: () => profileDir,
    });
  ...
  it("forces playwright disconnect when loopback cdp is occupied by non-owned process", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "xclaw-reset-no-own-"));
    const profileDir = path.join(tempRoot, "xclaw");
    fs.mkdirSync(profileDir, { recursive: true });
