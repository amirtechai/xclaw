...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".xclaw-profile-decorated");
}
 ...
 */
export function decorateXClawProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_XCLAW_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_XCLAW_BROWSER_COLOR).toUpperCase();
