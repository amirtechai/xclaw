...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredXClawTmpDir: vi.fn(() => "/tmp/xclaw"),
}));
vi.mock("../infra/tmp-xclaw-dir.js", () => tmpDirMocks);
...
describe("pw-tools-core", () => {
  ...
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredXClawTmpDir.mockReturnValue("/tmp/xclaw");
  });

  async function withTempDir<T>(run: (tempDir: string) => Promise<T>): Promise<T> {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "xclaw-browser-download-test-"));
    try {
  ...
  async function expectAtomicDownloadSave(params: {
    ...
    expect(path.basename(String(savedPath))).toContain(".xclaw-output-");
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredXClawTmpDir.mockReturnValue("/tmp/xclaw-preferred");
    ...
    const expectedRootedDownloadsDir = path.resolve(
      path.join(path.sep, "tmp", "xclaw-preferred", "downloads"),
    );
    const expectedDownloadsTail = `${path.join("tmp", "xclaw-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredXClawTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.resolve(path.join(path.sep, "tmp", "xclaw-preferred", "downloads")),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "xclaw-preferred", "downloads")}${path.sep}`),
    );
  });
