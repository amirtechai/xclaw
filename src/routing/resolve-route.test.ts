...
describe("resolveAgentRoute", () => {
  const resolveDiscordGuildRoute = (cfg: XClawConfig) =>
    resolveAgentRoute({
      cfg,
      channel: "discord",
      accountId: "default",
  ...
  test("defaultAgentId is used when no binding matches", () => {
    const cfg: XClawConfig = {
      agents: {
        list: [{ id: "home", default: true, workspace: "~/xclaw-home" }],
      },
    };
    const route = resolveAgentRoute({
      cfg,
...
describe("role-based agent routing", () => {
  type DiscordBinding = NonNullable<XClawConfig["bindings"]>[number];
