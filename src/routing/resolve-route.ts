...
function listAgents(cfg: XClawConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

type AgentLookupCache = {
  agentsRef: XClawConfig["agents"] | undefined;
  byNormalizedId: Map<string, string>;
  fallbackDefaultAgentId: string;
};

const agentLookupCacheByCfg = new WeakMap<XClawConfig, AgentLookupCache>();

function resolveAgentLookupCache(cfg: XClawConfig): AgentLookupCache {
...
export function pickFirstExistingAgentId(cfg: XClawConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: XClawConfig["bindings"];
...
const evaluatedBindingsCacheByCfg = new WeakMap<XClawConfig, EvaluatedBindingsCache>();
const MAX_EVALUATED_BINDINGS_CACHE_KEYS = 2000;
const resolvedRouteCacheByCfg = new WeakMap<
  XClawConfig,
  {
    bindingsRef: XClawConfig["bindings"];
    agentsRef: XClawConfig["agents"];
    sessionRef: XClawConfig["session"];
...
function resolveRouteCacheForConfig(cfg: XClawConfig): Map<string, ResolvedAgentRoute> {
