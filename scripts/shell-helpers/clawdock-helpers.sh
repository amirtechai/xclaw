#!/usr/bin/env bash
# ClawDock - Docker helpers for XClaw
# Inspired by Simon Willison's "Running XClaw in Docker"
# https://til.simonwillison.net/llms/xclaw-docker
#
# Installation:
...
# Common paths to check for XClaw
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/xclaw"
  "${HOME}/workspace/xclaw"
  "${HOME}/projects/xclaw"
  "${HOME}/dev/xclaw"
  "${HOME}/code/xclaw"
  "${HOME}/src/xclaw"
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found XClaw at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/xclaw"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ XClaw not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/xclaw/xclaw.git ~/xclaw"
    echo "  cd ~/xclaw && ./scripts/docker/setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^XCLAW_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d xclaw-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart xclaw-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f xclaw-gateway
}
...
# Container Access
clawdock-shell() {
  _clawdock_compose exec xclaw-gateway \
    bash -c 'echo "alias xclaw=\"./xclaw.mjs\"" > /tmp/.bashrc_xclaw && bash --rcfile /tmp/.bashrc_xclaw'
}
...
clawdock-cli() {
  _clawdock_compose run --rm xclaw-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build xclaw-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "XCLAW_GATEWAY_TOKEN=$token" xclaw-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" xclaw-gateway \
    bash -c './xclaw.mjs config set gateway.remote.token "$TOKEN" && ./xclaw.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec xclaw-gateway \
    bash -c "./xclaw.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart xclaw-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm xclaw-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec xclaw-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'xclaw config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for XClaw${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (xclaw alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to xclaw project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.xclaw)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'xclaw channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'xclaw status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.xclaw.ai${_CLR_RESET}"
  echo ""
}
