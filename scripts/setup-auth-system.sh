#!/bin/bash
# Setup XClaw Auth Management System
...
echo "  2. Subscribe to a topic (e.g., 'xclaw-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  XClaw message: Send warning via XClaw itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/xclaw-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/xclaw-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/xclaw-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now xclaw-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status xclaw-auth-monitor.timer"
echo ""
echo "Quick commands:"
