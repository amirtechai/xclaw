...
- regenerates apps/ios/XClaw.xcodeproj via xcodegen
EOF
}
...
PUSH_RELAY_BASE_URL="${XCLAW_PUSH_RELAY_BASE_URL:-${IOS_PUSH_RELAY_BASE_URL:-}}"
...
validate_push_relay_base_url() {
  ...
  if [[ "${value}" =~ [[:space:]] ]]; then
    echo "Invalid XCLAW_PUSH_RELAY_BASE_URL: whitespace is not allowed." >&2
    exit 1
  fi

  if [[ "${value}" == *'$'* || "${value}" == *'('* || "${value}" == *')'* || "${value}" == *'='* ]]; then
    echo "Invalid XCLAW_PUSH_RELAY_BASE_URL: contains forbidden xcconfig characters." >&2
    exit 1
  fi

  if [[ ! "${value}" =~ ^https://[A-Za-z0-9.-]+(:([0-9]{1,5}))?(/[A-Za-z0-9._~!&*+,;:@%/-]*)?$ ]]; then
    echo "Invalid XCLAW_PUSH_RELAY_BASE_URL: expected https://host[:port][/path]." >&2
    exit 1
  fi

  local port="${BASH_REMATCH[2]:-}"
  if [[ -n "${port}" ]] && (( 10#${port} > 65535 )); then
    echo "Invalid XCLAW_PUSH_RELAY_BASE_URL: port must be between 1 and 65535." >&2
    exit 1
  fi
}
...
if [[ -z "${PUSH_RELAY_BASE_URL}" ]]; then
  echo "Missing XCLAW_PUSH_RELAY_BASE_URL (or IOS_PUSH_RELAY_BASE_URL) for beta relay registration." >&2
  exit 1
fi
...
PUSH_RELAY_BASE_URL_XCCONFIG="$(
  printf '%s' "${PUSH_RELAY_BASE_URL}" \
    | sed 's#//#$(XCLAW_URL_SLASH)$(XCLAW_URL_SLASH)#g'
)"
...
XCLAW_CODE_SIGN_STYLE = Automatic
XCLAW_DEVELOPMENT_TEAM = ${TEAM_ID}
XCLAW_IOS_SELECTED_TEAM = ${TEAM_ID}
XCLAW_APP_BUNDLE_ID = ai.xclaw.client
XCLAW_SHARE_BUNDLE_ID = ai.xclaw.client.share
XCLAW_ACTIVITY_WIDGET_BUNDLE_ID = ai.xclaw.client.activitywidget
XCLAW_WATCH_APP_BUNDLE_ID = ai.xclaw.client.watchkitapp
XCLAW_WATCH_EXTENSION_BUNDLE_ID = ai.xclaw.client.watchkitapp.extension
XCLAW_APP_PROFILE =
XCLAW_SHARE_PROFILE =
XCLAW_PUSH_TRANSPORT = relay
XCLAW_PUSH_DISTRIBUTION = official
XCLAW_URL_SLASH = /
XCLAW_PUSH_RELAY_BASE_URL = ${PUSH_RELAY_BASE_URL_XCCONFIG}
XCLAW_PUSH_APNS_ENVIRONMENT = production
EOF
