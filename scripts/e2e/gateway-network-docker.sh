...
IMAGE_NAME="xclaw-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="xclaw-net-e2e-$$"
GW_NAME="xclaw-gateway-e2e-$$"
...
docker run -d \
  --name "$GW_NAME" \
  --network "$NET_NAME" \
  -e "XCLAW_GATEWAY_TOKEN=$TOKEN" \
  -e "XCLAW_SKIP_CHANNELS=1" \
  -e "XCLAW_SKIP_GMAIL_WATCHER=1" \
  -e "XCLAW_SKIP_CRON=1" \
  -e "XCLAW_SKIP_CANVAS_HOST=1" \
  "$IMAGE_NAME" \
