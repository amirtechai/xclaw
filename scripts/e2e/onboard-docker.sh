...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  export PATH="/tmp/xclaw-bin:$PATH"
  mkdir -p /tmp/xclaw-bin
  cat > /tmp/xclaw-bin/trash <<'"'"'TRASH'"'"'
#!/usr/bin/env bash
set -euo pipefail
...
TRASH
  ...
  wait_for_log() {
    ...
    while true; do
	  ...
	  start_gateway() {
	    node "$XCLAW_ENTRY" gateway --port 18789 --bind loopback --allow-unconfigured > /tmp/gateway-e2e.log 2>&1 &
	    GATEWAY_PID="$!"
	  }
  ...
  run_wizard_cmd() {
    ...
    set_isolated_xclaw_env "$home_dir"

    input_fifo="$(mktemp -u "/tmp/xclaw-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/xclaw-onboard-${case_name}.log"
  ...
  run_wizard() {
    ...
    local validate_fn="${4:-}"
	    ...
	    run_wizard_cmd "$case_name" "$home_dir" "node \"$XCLAW_ENTRY\" onboard $ONBOARD_FLAGS" "$send_fn" true "$validate_fn"
	  }

  make_home() {
    mktemp -d "/tmp/xclaw-e2e-$1.XXXXXX"
  }

  set_isolated_xclaw_env() {
    local home_dir="$1"
    export HOME="$home_dir"
    export XCLAW_HOME="$home_dir"
    export XCLAW_STATE_DIR="$home_dir/.xclaw"
    export XCLAW_CONFIG_PATH="$XCLAW_STATE_DIR/xclaw.json"
    mkdir -p "$XCLAW_STATE_DIR"
  }
  ...
  run_case_local_basic() {
    ...
    workspace_dir="$XCLAW_STATE_DIR/workspace"
    config_path="$XCLAW_CONFIG_PATH"
    sessions_dir="$XCLAW_STATE_DIR/agents/main/sessions"
...
NODE
  ...
  run_case_remote_non_interactive() {
    ...
    set_isolated_xclaw_env "$home_dir"
	    ...
	    node "$XCLAW_ENTRY" onboard --non-interactive --accept-risk \
	      --mode remote \
...
NODE
  ...
  run_case_channels() {
	    ...
	    run_wizard_cmd channels "$home_dir" "node \"$XCLAW_ENTRY\" configure --section channels" send_channels_flow
...
JSON
	    ...
	    run_wizard_cmd skills "$home_dir" "node \"$XCLAW_ENTRY\" configure --section skills" send_skills_flow
