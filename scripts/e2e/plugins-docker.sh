...
if [ -f dist/index.mjs ]; then
  XCLAW_ENTRY="dist/index.mjs"
elif [ -f dist/index.js ]; then
  XCLAW_ENTRY="dist/index.js"
else
...
home_dir=$(mktemp -d "/tmp/xclaw-plugins-e2e.XXXXXX")
...
start_gateway() {
  local log_file="$1"
  : > "$log_file"
  node "$XCLAW_ENTRY" gateway --port 18789 --bind loopback --allow-unconfigured \
    >"$log_file" 2>&1 &
  gateway_pid=$!
...
wait_for_gateway_health() {
  for _ in $(seq 1 120); do
    if node "$XCLAW_ENTRY" gateway health \
...
run_gateway_chat_json() {
  ...
  node - <<'NODE' "$XCLAW_ENTRY" "$session_key" "$message" "$output_file" "$timeout_ms"
...
JS
  cat > "$dir/xclaw.plugin.json" <<'JSON'
{
  "id": "placeholder",
  "configSchema": {
    "type": "object",
...
JSON
  node - <<'NODE' "$dir/xclaw.plugin.json" "$id"
const fs = require("node:fs");
...
mkdir -p "$HOME/.xclaw/extensions/demo-plugin"

cat > "$HOME/.xclaw/extensions/demo-plugin/index.js" <<'JS'
module.exports = {
  id: "demo-plugin",
  name: "Demo Plugin",
...
JS
cat > "$HOME/.xclaw/extensions/demo-plugin/xclaw.plugin.json" <<'JSON'
{
  "id": "demo-plugin",
  "configSchema": {
    "type": "object",
...
node "$XCLAW_ENTRY" plugins list --json > /tmp/plugins.json
...
pack_dir="$(mktemp -d "/tmp/xclaw-plugin-pack.XXXXXX")"
mkdir -p "$pack_dir/package"
cat > "$pack_dir/package/package.json" <<'JSON'
{
  "name": "@xclaw/demo-plugin-tgz",
  "version": "0.0.1",
  "xclaw": { "extensions": ["./index.js"] }
}
JSON
...
JS
cat > "$pack_dir/package/xclaw.plugin.json" <<'JSON'
{
  "id": "demo-plugin-tgz",
  "configSchema": {
    "type": "object",
...
node "$XCLAW_ENTRY" plugins install /tmp/demo-plugin-tgz.tgz
node "$XCLAW_ENTRY" plugins list --json > /tmp/plugins2.json
...
dir_plugin="$(mktemp -d "/tmp/xclaw-plugin-dir.XXXXXX")"
cat > "$dir_plugin/package.json" <<'JSON'
{
  "name": "@xclaw/demo-plugin-dir",
  "version": "0.0.1",
  "xclaw": { "extensions": ["./index.js"] }
}
JSON
...
JS
cat > "$dir_plugin/xclaw.plugin.json" <<'JSON'
{
  "id": "demo-plugin-dir",
  "configSchema": {
    "type": "object",
...
node "$XCLAW_ENTRY" plugins install "$dir_plugin"
node "$XCLAW_ENTRY" plugins list --json > /tmp/plugins3.json
...
file_pack_dir="$(mktemp -d "/tmp/xclaw-plugin-filepack.XXXXXX")"
mkdir -p "$file_pack_dir/package"
cat > "$file_pack_dir/package/package.json" <<'JSON'
{
  "name": "@xclaw/demo-plugin-file",
  "version": "0.0.1",
  "xclaw": { "extensions": ["./index.js"] }
}
JSON
...
JS
cat > "$file_pack_dir/package/xclaw.plugin.json" <<'JSON'
{
  "id": "demo-plugin-file",
  "configSchema": {
    "type": "object",
...
node "$XCLAW_ENTRY" plugins install "file:$file_pack_dir/package"
node "$XCLAW_ENTRY" plugins list --json > /tmp/plugins4.json
...
bundle_root="$HOME/.xclaw/extensions/claude-bundle-e2e"
...
const configPath = path.join(process.env.HOME, ".xclaw", "xclaw.json");
...
gateway_log="/tmp/xclaw-plugin-command-e2e.log"
start_gateway "$gateway_log"
wait_for_gateway_health

echo "Testing /plugin install with auto-restart..."
slash_install_dir="$(mktemp -d "/tmp/xclaw-plugin-slash-install.XXXXXX")"
cat > "$slash_install_dir/package.json" <<'JSON'
{
  "name": "@xclaw/slash-install-plugin",
  "version": "0.0.1",
  "xclaw": { "extensions": ["./index.js"] }
}
JSON
...
JS
cat > "$slash_install_dir/xclaw.plugin.json" <<'JSON'
{
  "id": "slash-install-plugin",
  "configSchema": {
    "type": "object",
...
cat > "$HOME/.claude/plugins/known_marketplaces.json" <<JSON
{
  "claude-fixtures": {
    ...
    "source": {
      "type": "github",
      "repo": "xclaw/fixture-marketplace"
    }
  }
}
JSON

node "$XCLAW_ENTRY" plugins marketplace list claude-fixtures --json > /tmp/marketplace-list.json
...
node "$XCLAW_ENTRY" plugins install marketplace-shortcut@claude-fixtures
node "$XCLAW_ENTRY" plugins install marketplace-direct --marketplace claude-fixtures
node "$XCLAW_ENTRY" plugins list --json > /tmp/plugins-marketplace.json
...
write_fixture_plugin \
  ...
  "Marketplace Shortcut"
node "$XCLAW_ENTRY" plugins update marketplace-shortcut --dry-run
node "$XCLAW_ENTRY" plugins update marketplace-shortcut
node "$XCLAW_ENTRY" plugins list --json > /tmp/plugins-marketplace-updated.json
