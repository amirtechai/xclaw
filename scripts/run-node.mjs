...
const extensionRestartMetadataFiles = new Set(["xclaw.plugin.json", "package.json"]);
...
const shouldBuild = (deps) => {
  if (deps.env.XCLAW_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message, deps) => {
  if (deps.env.XCLAW_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[xclaw] ${message}\n`);
};

const runXClaw = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["xclaw.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
