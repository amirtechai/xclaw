...
export function copyBundledPluginMetadata(params = {}) {
  ...
  for (const dirent of fs.readdirSync(extensionsRoot, { withFileTypes: true })) {
    ...
    const manifestPath = path.join(pluginDir, "xclaw.plugin.json");
    ...
    const distManifestPath = path.join(distPluginDir, "xclaw.plugin.json");
    ...
    if (packageJson.xclaw && "extensions" in packageJson.xclaw) {
      packageJson.xclaw = {
        ...packageJson.xclaw,
        extensions: rewritePackageExtensions(packageJson.xclaw.extensions),
        ...(typeof packageJson.xclaw.setupEntry === "string"
          ? { setupEntry: rewritePackageEntry(packageJson.xclaw.setupEntry) }
          : {}),
      };
    }
