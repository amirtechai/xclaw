...
# XClaw Installer for macOS and Linux
# Usage: curl -fsSL --proto '=https' --tlsv1.2 https://xclaw.ai/install.sh | bash
...
DEFAULT_TAGLINE="All your chats, one XClaw."
...
GUM_VERSION="${XCLAW_GUM_VERSION:-0.17.0}"
GUM=""
GUM_STATUS="skipped"
GUM_REASON=""
...
print_installer_banner() {
    if [[ -n "$GUM" ]]; then
        ...
        title="$("$GUM" style --foreground "#ff4d4d" --bold "🦞 XClaw Installer")"
...
detect_os_or_die() {
    ...
    if [[ "$OS" == "unknown" ]]; then
        ...
        echo "For Windows, use: iwr -useb https://xclaw.ai/install.ps1 | iex"
        exit 1
    fi
...
show_install_plan() {
    ...
    ui_kv "Requested version" "$XCLAW_VERSION"
    if [[ "$USE_BETA" == "1" ]]; then
...
show_footer_links() {
    local faq_url="https://docs.xclaw.ai/start/faq"
    if [[ -n "$GUM" ]]; then
        local content
...
cleanup_npm_xclaw_paths() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || "$npm_root" != *node_modules* ]]; then
        return 1
    fi
    rm -rf "$npm_root"/.xclaw-* "$npm_root"/xclaw 2>/dev/null || true
}

extract_xclaw_conflict_path() {
    local log="$1"
    local path=""
...
cleanup_xclaw_bin_conflict() {
    ...
    if [[ -n "$npm_bin" && "$bin_path" != "$npm_bin/xclaw" ]]; then
        case "$bin_path" in
            "/opt/homebrew/bin/xclaw"|"/usr/local/bin/xclaw")
                ;;
            *)
                return 1
                ;;
    ...
    if [[ -L "$bin_path" ]]; then
        ...
        if [[ "$target" == *"/node_modules/xclaw/"* ]]; then
            rm -f "$bin_path"
            ui_info "Removed stale xclaw symlink at ${bin_path}"
            return 0
        fi
        return 1
    fi
    ...
    if mv "$bin_path" "$backup"; then
        ui_info "Moved existing xclaw binary to ${backup}"
        return 0
    fi
    return 1
}
...
run_npm_global_install() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        printf -v log_quoted '%q' "$log"
        run_with_spinner "Installing XClaw package" bash -c "${cmd_quoted}>${log_quoted} 2>&1"
        return $?
    fi
...
install_xclaw_npm() {
    ...
    if ! run_npm_global_install "$spec" "$log"; then
        ...
        if auto_install_build_tools_for_npm_failure "$log"; then
            ...
            if run_npm_global_install "$spec" "$log"; then
                ui_success "XClaw npm package installed"
                return 0
            fi
        fi
        ...
        if grep -q "ENOTEMPTY: directory not empty, rename .*xclaw" "$log"; then
        ...
        if grep -q "EEXIST" "$log"; then
            local conflict=""
            conflict="$(extract_xclaw_conflict_path "$log" || true)"
            if [[ -n "$conflict" ]] && cleanup_xclaw_bin_conflict "$conflict"; then
                if run_npm_global_install "$spec" "$log"; then
                    ui_success "XClaw npm package installed"
                    return 0
                fi
            ...
            ui_error "npm failed because an xclaw binary already exists"
            if [[ -n "$conflict" ]]; then
...
pick_tagline() {
    ...
    if [[ -n "${XCLAW_TAGLINE_INDEX:-}" ]]; then
        if [[ "${XCLAW_TAGLINE_INDEX}" =~ ^[0-9]+$ ]]; then
            local idx=$((XCLAW_TAGLINE_INDEX % count))
            echo "${TAGLINES[$idx]}"
            return
        fi
    fi
...
NO_ONBOARD=${XCLAW_NO_ONBOARD:-0}
NO_PROMPT=${XCLAW_NO_PROMPT:-0}
DRY_RUN=${XCLAW_DRY_RUN:-0}
INSTALL_METHOD=${XCLAW_INSTALL_METHOD:-}
XCLAW_VERSION=${XCLAW_VERSION:-latest}
USE_BETA=${XCLAW_BETA:-0}
GIT_DIR_DEFAULT="${HOME}/xclaw"
GIT_DIR=${XCLAW_GIT_DIR:-$GIT_DIR_DEFAULT}
GIT_UPDATE=${XCLAW_GIT_UPDATE:-1}
SHARP_IGNORE_GLOBAL_LIBVIPS="${SHARP_IGNORE_GLOBAL_LIBVIPS:-1}"
NPM_LOGLEVEL="${XCLAW_NPM_LOGLEVEL:-error}"
NPM_SILENT_FLAG="--silent"
VERBOSE="${XCLAW_VERBOSE:-0}"
VERIFY_INSTALL="${XCLAW_VERIFY_INSTALL:-0}"
XCLAW_BIN=""
PNPM_CMD=()
HELP=0
...
print_usage() {
    cat <<EOF
XClaw installer (macOS + Linux)

Usage:
  curl -fsSL --proto '=https' --tlsv1.2 https://xclaw.ai/install.sh | bash -s -- [options]
...
Options:
  ...
  --git-dir, --dir <path>             Checkout directory (default: ~/xclaw)
...
Environment variables:
  XCLAW_INSTALL_METHOD=git|npm
  XCLAW_VERSION=latest|next|main|<semver>|<spec>
  XCLAW_BETA=0|1
  XCLAW_GIT_DIR=...
  XCLAW_GIT_UPDATE=0|1
  XCLAW_NO_PROMPT=1
  XCLAW_VERIFY_INSTALL=1
  XCLAW_DRY_RUN=1
  XCLAW_NO_ONBOARD=1
  XCLAW_VERBOSE=1
  XCLAW_NPM_LOGLEVEL=error|warn|notice  Default: error (hide npm deprecation noise)
  SHARP_IGNORE_GLOBAL_LIBVIPS=0|1    Default: 1 (avoid sharp building against global libvips)

Examples:
  curl -fsSL --proto '=https' --tlsv1.2 https://xclaw.ai/install.sh | bash
  curl -fsSL --proto '=https' --tlsv1.2 https://xclaw.ai/install.sh | bash -s -- --no-onboard
  curl -fsSL --proto '=https' --tlsv1.2 https://xclaw.ai/install.sh | bash -s -- --no-onboard --verify
  curl -fsSL --proto '=https' --tlsv1.2 https://xclaw.ai/install.sh | bash -s -- --version main
  curl -fsSL --proto '=https' --tlsv1.2 https://xclaw.ai/install.sh | bash -s -- --install-method git --no-onboard
EOF
}
...
choose_install_method_interactive() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        header="Detected XClaw checkout in: ${detected_checkout}
...
${WARN}→${NC} Detected a XClaw source checkout in: ${INFO}${detected_checkout}${NC}
Choose install method:
...
detect_xclaw_checkout() {
    ...
    if ! grep -q '"name"[[:space:]]*:[[:space:]]*"xclaw"' "$dir/package.json" 2>/dev/null; then
        return 1
    fi
    echo "$dir"
    return 0
...
print_homebrew_admin_fix() {
    ...
    echo "Then retry:"
    echo "  curl -fsSL https://xclaw.ai/install.sh | bash"
}
...
ensure_xclaw_bin_link() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || ! -d "$npm_root/xclaw" ]]; then
        return 1
    fi
    local npm_bin=""
    ...
    mkdir -p "$npm_bin"
    if [[ ! -x "${npm_bin}/xclaw" ]]; then
        ln -sf "$npm_root/xclaw/dist/entry.js" "${npm_bin}/xclaw"
        ui_info "Created xclaw bin link at ${npm_bin}/xclaw"
    fi
    return 0
}

# Check for existing XClaw installation
check_existing_xclaw() {
    if [[ -n "$(type -P xclaw 2>/dev/null || true)" ]]; then
        ui_info "Existing XClaw installation detected, upgrading"
        return 0
    fi
    return 1
}
...
warn_shell_path_missing_dir() {
    ...
    echo "  This can make xclaw show as \"command not found\" in new terminals."
...
warn_xclaw_not_found() {
    ui_warn "Installed, but xclaw is not discoverable on PATH in this shell"
    echo "  Try: hash -r (bash) or rehash (zsh), then retry."
    local t=""
    t="$(type -t xclaw 2>/dev/null || true)"
    if [[ "$t" == "alias" || "$t" == "function" ]]; then
        ui_warn "Found a shell ${t} named xclaw; it may shadow the real binary"
    fi
    if command -v nodenv &> /dev/null; then
...
resolve_xclaw_bin() {
    refresh_shell_command_cache
    local resolved=""
    resolved="$(type -P xclaw 2>/dev/null || true)"
    ...
    local npm_bin=""
    npm_bin="$(npm_global_bin_dir || true)"
    if [[ -n "$npm_bin" && -x "${npm_bin}/xclaw" ]]; then
        echo "${npm_bin}/xclaw"
        return 0
    fi
...
install_xclaw_from_git() {
    local repo_dir="$1"
    local repo_url="https://github.com/xclaw/xclaw.git"

    if [[ -d "$repo_dir/.git" ]]; then
        ui_info "Installing XClaw from git checkout: ${repo_dir}"
    else
        ui_info "Installing XClaw from GitHub (${repo_url})"
    fi
    ...
    if [[ ! -d "$repo_dir" ]]; then
        run_quiet_step "Cloning XClaw" git clone "$repo_url" "$repo_dir"
    fi
    ...
    run_quiet_step "Building XClaw" run_pnpm -C "$repo_dir" build

    ensure_user_local_bin_on_path

    cat > "$HOME/.local/bin/xclaw" <<EOF
#!/usr/bin/env bash
set -euo pipefail
exec node "${repo_dir}/dist/entry.js" "\$@"
EOF
    chmod +x "$HOME/.local/bin/xclaw"
    ui_success "XClaw wrapper installed to \$HOME/.local/bin/xclaw"
...
resolve_beta_version() {
    local beta=""
    beta="$(npm view xclaw dist-tags.beta 2>/dev/null || true)"
...
install_xclaw() {
    local package_name="xclaw"
    if [[ "$USE_BETA" == "1" ]]; then
        local beta_version=""
        beta_version="$(resolve_beta_version || true)"
        if [[ -n "$beta_version" ]]; then
            XCLAW_VERSION="$beta_version"
            ui_info "Beta tag detected (${beta_version})"
            package_name="xclaw"
        else
            XCLAW_VERSION="latest"
            ui_info "No beta tag found; using latest"
        fi
    fi

    if [[ -z "${XCLAW_VERSION}" ]]; then
        XCLAW_VERSION="latest"
    fi

    local resolved_version=""
    if can_resolve_registry_package_version "${XCLAW_VERSION}"; then
        resolved_version="$(npm view "${package_name}@${XCLAW_VERSION}" version 2>/dev/null || true)"
    fi
    if [[ -n "$resolved_version" ]]; then
        ui_info "Installing XClaw v${resolved_version}"
    else
        ui_info "Installing XClaw (${XCLAW_VERSION})"
    fi
    local install_spec=""
    install_spec="$(resolve_package_install_spec "${package_name}" "${XCLAW_VERSION}")"

    if ! install_xclaw_npm "${install_spec}"; then
        ui_warn "npm install failed; retrying"
        cleanup_npm_xclaw_paths
        install_xclaw_npm "${install_spec}"
    fi

    if [[ "${XCLAW_VERSION}" == "latest" && "${package_name}" == "xclaw" ]]; then
        if ! resolve_xclaw_bin &> /dev/null; then
            ui_warn "npm install xclaw@latest failed; retrying xclaw@next"
            cleanup_npm_xclaw_paths
            install_xclaw_npm "xclaw@next"
        fi
    fi

    ensure_xclaw_bin_link || true

    ui_success "XClaw installed"
}

# Run doctor for migrations (safe, non-interactive)
run_doctor() {
    ui_info "Running doctor to migrate settings"
    local claw="${XCLAW_BIN:-}"
    if [[ -z "$claw" ]]; then
        claw="$(resolve_xclaw_bin || true)"
    fi
    if [[ -z "$claw" ]]; then
        ui_info "Skipping doctor (xclaw not on PATH yet)"
        warn_xclaw_not_found
        return 0
    fi
...
resolve_workspace_dir() {
    local profile="${XCLAW_PROFILE:-default}"
    if [[ "${profile}" != "default" ]]; then
        echo "${HOME}/.xclaw/workspace-${profile}"
    else
        echo "${HOME}/.xclaw/workspace"
    fi
}
...
run_bootstrap_onboarding_if_needed() {
    ...
    local config_path="${XCLAW_CONFIG_PATH:-$HOME/.xclaw/xclaw.json}"
    ...
    if [[ ! -r /dev/tty || ! -w /dev/tty ]]; then
        ui_info "BOOTSTRAP.md found but no TTY; run xclaw onboard to finish setup"
        return
    fi
    ...
    if [[ -z "$claw" ]]; then
        ui_info "BOOTSTRAP.md found but xclaw not on PATH; skipping onboarding"
        warn_xclaw_not_found
        return
    fi

    "$claw" onboard || {
        ui_error "Onboarding failed; run xclaw onboard to retry"
        return
    }
}
...
resolve_xclaw_version() {
    local version=""
    local raw_version_output=""
    local claw="${XCLAW_BIN:-}"
    if [[ -z "$claw" ]] && command -v xclaw &> /dev/null; then
        claw="$(command -v xclaw)"
    fi
    if [[ -n "$claw" ]]; then
        raw_version_output=$("$claw" --version 2>/dev/null | head -n 1 | tr -d '\r')
        version="$(extract_xclaw_semver "$raw_version_output")"
        if [[ -z "$version" ]]; then
    ...
    if [[ -z "$version" ]]; then
        local npm_root=""
        npm_root=$(npm root -g 2>/dev/null || true)
        if [[ -n "$npm_root" && -f "$npm_root/xclaw/package.json" ]]; then
            version=$(node -e "console.log(require('${npm_root}/xclaw/package.json').version)" 2>/dev/null || true)
        fi
    fi
    echo "$version"
}
...
verify_installation() {
    ...
    if [[ -z "$claw" ]]; then
        ui_error "Install verify failed: xclaw not on PATH yet"
        warn_xclaw_not_found
        return 1
    fi

    run_quiet_step "Checking XClaw version" "$claw" --version || return 1

    if is_gateway_daemon_loaded "$claw"; then
        run_quiet_step "Checking gateway service" "$claw" gateway status --deep || {
            ui_error "Install verify failed: gateway service unhealthy"
            ui_info "Run: xclaw gateway status --deep"
            return 1
        }
    else
...
main() {
    ...
    detected_checkout="$(detect_xclaw_checkout "$PWD" || true)"

    if [[ -z "$INSTALL_METHOD" && -n "$detected_checkout" ]]; then
        if ! is_promptable; then
            ui_info "Found XClaw checkout but no TTY; defaulting to npm install"
            INSTALL_METHOD="npm"
        else
            ...
            case "$selected_method" in
                ...
                *)
                    ...
                    echo "Re-run with: --install-method git|npm (or set XCLAW_INSTALL_METHOD)."
                    exit 2
                    ;;
            esac
        fi
    ...
    if [[ "$INSTALL_METHOD" == "git" ]]; then
        ...
        install_xclaw_from_git "$repo_dir"
    else
        # Clean up git wrapper if switching to npm
        if [[ -x "$HOME/.local/bin/xclaw" ]]; then
            ui_info "Removing git wrapper (switching to npm)"
            rm -f "$HOME/.local/bin/xclaw"
    ...
    XCLAW_BIN="$(resolve_xclaw_bin || true)"
    ...
    installed_version=$(resolve_xclaw_version)

    echo ""
    if [[ -n "$installed_version" ]]; then
        ui_celebrate "🦞 XClaw installed successfully (${installed_version})!"
    else
        ui_celebrate "🦞 XClaw installed successfully!"
    fi
    ...
    if [[ "$INSTALL_METHOD" == "git" && -n "$final_git_dir" ]]; then
        ...
        ui_kv "Wrapper" "$HOME/.local/bin/xclaw"
        ui_kv "Update command" "xclaw update --restart"
        ui_kv "Switch to npm" "curl -fsSL --proto '=https' --tlsv1.2 https://xclaw.ai/install.sh | bash -s -- --install-method npm"
        ...
        if [[ -r /dev/tty && -w /dev/tty ]]; then
            ...
            ui_info "Running xclaw doctor"
            local doctor_ok=0
            if (( ${#doctor_args[@]} )); then
                XCLAW_UPDATE_IN_PROGRESS=1 "$claw" doctor "${doctor_args[@]}" </dev/tty && doctor_ok=1
            else
                XCLAW_UPDATE_IN_PROGRESS=1 "$claw" doctor </dev/tty && doctor_ok=1
            fi
            if (( doctor_ok )); then
                ui_info "Updating plugins"
                XCLAW_UPDATE_IN_PROGRESS=1 "$claw" plugins update --all || true
            else
                ui_warn "Doctor failed; skipping plugin updates"
            fi
        else
            ui_info "No TTY; run xclaw doctor and xclaw plugins update --all manually"
        fi
    else
        if [[ "$NO_ONBOARD" == "1" || "$skip_onboard" == "true" ]]; then
            ui_info "Skipping onboard (requested); run xclaw onboard later"
        else
            local config_path="${XCLAW_CONFIG_PATH:-$HOME/.xclaw/xclaw.json}"
            if [[ -f "${config_path}" || -f "$HOME/.clawdbot/clawdbot.json" || -f "$HOME/.moltbot/moltbot.json" || -f "$HOME/.moldbot/moldbot.json" ]]; then
            ...
            if [[ -r /dev/tty && -w /dev/tty ]]; then
                ...
                if [[ -z "$claw" ]]; then
                    ui_info "Skipping onboarding (xclaw not on PATH yet)"
                    warn_xclaw_not_found
                    return 0
                fi
                exec </dev/tty
            ...
            ui_info "No TTY; run xclaw onboard to finish setup"
            return 0
        fi
    fi

    if command -v xclaw &> /dev/null; then
        local claw="${XCLAW_BIN:-}"
        if [[ -z "$claw" ]]; then
            claw="$(resolve_xclaw_bin || true)"
        fi
        if [[ -n "$claw" ]] && is_gateway_daemon_loaded "$claw"; then
            if [[ "$DRY_RUN" == "1" ]]; then
                ui_info "Gateway daemon detected; would restart (xclaw daemon restart)"
            else
                ui_info "Gateway daemon detected; restarting"
                if XCLAW_UPDATE_IN_PROGRESS=1 "$claw" daemon restart >/dev/null 2>&1; then
                    ui_success "Gateway restarted"
                else
                    ui_warn "Gateway restart failed; try: xclaw daemon restart"
                fi
            fi
        fi
    fi
...
if [[ "${XCLAW_INSTALL_SH_NO_RUN:-0}" != "1" ]]; then
    parse_args "$@"
    configure_verbose
    main
fi
