...
#
# Suggested cron: */30 * * * * /home/admin/xclaw/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send XClaw notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., xclaw-alerts)
...
STATE_FILE="$HOME/.xclaw/auth-monitor-state"
...
send_notification() {
    ...
    # Send via XClaw if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use xclaw
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via XClaw to $NOTIFY_PHONE..."
            xclaw send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! XClaw is down. Run: ssh l36 '~/xclaw/scripts/mobile-reauth.sh'" "urgent"
    exit 1
