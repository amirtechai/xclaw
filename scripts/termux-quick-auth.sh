...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/xclaw/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "XClaw Re-Auth" -c "After getting key, run: ssh $SERVER '~/xclaw/scripts/mobile-reauth.sh'" --id xclaw-auth
        ;;
    *)
