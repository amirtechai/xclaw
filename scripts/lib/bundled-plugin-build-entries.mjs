...
function collectPluginSourceEntries(packageJson) {
  let packageEntries = Array.isArray(packageJson?.xclaw?.extensions)
    ? packageJson.xclaw.extensions.filter(
        (entry) => typeof entry === "string" && entry.trim().length > 0,
      )
    : [];
  const setupEntry =
    typeof packageJson?.xclaw?.setupEntry === "string" &&
    packageJson.xclaw.setupEntry.trim().length > 0
      ? packageJson.xclaw.setupEntry
      : undefined;
  if (setupEntry) {
...
export function listBundledPluginPackArtifacts(params = {}) {
  ...
  for (const { id, hasPackageJson, sourceEntries } of entries) {
    artifacts.add(`dist/extensions/${id}/xclaw.plugin.json`);
    if (hasPackageJson) {
