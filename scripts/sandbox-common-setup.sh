...
BASE_IMAGE="${BASE_IMAGE:-xclaw-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-xclaw-sandbox-common:bookworm-slim}"
...
XCLAW_DOCKER_BUILD_USE_BUILDX="${XCLAW_DOCKER_BUILD_USE_BUILDX:-0}"
XCLAW_DOCKER_BUILD_CACHE_FROM="${XCLAW_DOCKER_BUILD_CACHE_FROM:-}"
XCLAW_DOCKER_BUILD_CACHE_TO="${XCLAW_DOCKER_BUILD_CACHE_TO:-}"
...
if [ "${XCLAW_DOCKER_BUILD_USE_BUILDX}" = "1" ]; then
  build_cmd=(docker buildx build --load)
  if [ -n "${XCLAW_DOCKER_BUILD_CACHE_FROM}" ]; then
    build_cmd+=(--cache-from "${XCLAW_DOCKER_BUILD_CACHE_FROM}")
  fi
  if [ -n "${XCLAW_DOCKER_BUILD_CACHE_TO}" ]; then
    build_cmd+=(--cache-to "${XCLAW_DOCKER_BUILD_CACHE_TO}")
  fi
fi
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=xclaw.sandbox=1)
NOTE
