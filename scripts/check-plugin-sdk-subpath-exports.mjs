...
function parsePluginSdkSubpath(specifier) {
  if (!specifier.startsWith("xclaw/plugin-sdk/")) {
    return null;
  }
  const subpath = specifier.slice("xclaw/plugin-sdk/".length);
  return subpath || null;
}
...
async function main() {
  ...
  if (violations.length === 0) {
    console.log("OK: all referenced xclaw/plugin-sdk/<subpath> imports are exported.");
    return;
  }

  console.error(
    "Rule: every referenced xclaw/plugin-sdk/<subpath> must exist in the public package exports.",
  );
