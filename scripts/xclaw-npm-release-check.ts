...
const EXPECTED_REPOSITORY_URL = "https://github.com/xclaw/xclaw";
...
export function collectReleasePackageMetadataErrors(pkg: PackageJson): string[] {
  ...
  if (pkg.name !== "xclaw") {
    errors.push(`package.json name must be "xclaw"; found "${pkg.name ?? ""}".`);
  }
  ...
  if (pkg.bin?.xclaw !== "xclaw.mjs") {
    errors.push(
      `package.json bin.xclaw must be "xclaw.mjs"; found "${pkg.bin?.xclaw ?? ""}".`,
    );
  }
...
function main(): number {
  ...
  if (errors.length > 0) {
    for (const error of errors) {
      console.error(`xclaw-npm-release-check: ${error}`);
    }
    return 1;
  }
  ...
  console.log(
    `xclaw-npm-release-check: validated ${channel} release ${pkg.version} (${dayDistance} day UTC delta).`,
  );
  return 0;
}
