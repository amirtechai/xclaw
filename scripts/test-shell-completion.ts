 ...
 * `xclaw update`. Use it to verify the completion installation flow
...
\`xclaw update\`, \`xclaw doctor\`, and \`xclaw onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# XClaw Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`xclaw completion --install\` later to enable.`));
    return;
  }
