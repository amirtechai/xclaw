...
function buildCatalogEntry(packageJson) {
  ...
  const manifest = isRecord(packageJson.xclaw) ? packageJson.xclaw : null;
...
export function buildOfficialChannelCatalog(params = {}) {
  ...
  entries.sort((left, right) => {
    const leftId = trimString(left.xclaw?.channel?.id) || left.name;
    const rightId = trimString(right.xclaw?.channel?.id) || right.name;
