...
SMOKE_PREVIOUS_VERSION="${XCLAW_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${XCLAW_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="xclaw"
PACKAGE_NAME="${XCLAW_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (XCLAW_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${XCLAW_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${XCLAW_INSTALL_LATEST_OUT:-}"
fi
