...
const warningFilterKey = Symbol.for("xclaw.warning-filter");
...
const { clearPluginCommands, getPluginCommandSpecs, loadXClawPlugins, matchPluginCommand } =
  await import(pathToFileURL(smokeEntryPath).href);

assert.equal(typeof loadXClawPlugins, "function", "built loader export missing");
...
const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "xclaw-build-smoke-"));
...
fs.writeFileSync(
  ...
  [
    "import sdk from 'xclaw/plugin-sdk';",
...
const registry = loadXClawPlugins({
  ...
  env: {
    ...process.env,
    XCLAW_BUNDLED_PLUGINS_DIR: path.join(tempRoot, "dist-runtime", "extensions"),
