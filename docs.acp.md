...
This document describes how the XClaw ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`xclaw acp` exposes an ACP agent over stdio and forwards prompts to a running
XClaw Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
runtime. It is designed to route IDE prompts into an existing XClaw Gateway
...
- ACP client spawns `xclaw acp` and speaks ACP messages over stdio.
...
`xclaw acp` resolves the Gateway URL and auth from CLI flags or config:
