...
import { resetInboundDedupe } from "xclaw/plugin-sdk/reply-runtime";
import type { MsgContext } from "xclaw/plugin-sdk/reply-runtime";
import type { GetReplyOptions, ReplyPayload } from "xclaw/plugin-sdk/reply-runtime";
import { createReplyDispatcher } from "xclaw/plugin-sdk/reply-runtime";
import type { MockFn } from "xclaw/plugin-sdk/testing";
import { beforeEach, vi } from "vitest";
import type { TelegramBotDeps } from "./bot-deps.js";

type AnyMock = ReturnType<typeof vi.fn>;
type AnyAsyncMock = ReturnType<typeof vi.fn>;
type LoadConfigFn = typeof import("xclaw/plugin-sdk/config-runtime").loadConfig;
type ResolveStorePathFn = typeof import("xclaw/plugin-sdk/config-runtime").resolveStorePath;
type TelegramBotRuntimeForTest = NonNullable<
  Parameters<typeof import("./bot.js").setTelegramBotRuntimeForTest>[0]
>;
type DispatchReplyWithBufferedBlockDispatcherFn =
  typeof import("xclaw/plugin-sdk/reply-runtime").dispatchReplyWithBufferedBlockDispatcher;
...
const { sessionStorePath } = vi.hoisted(() => ({
  sessionStorePath: `/tmp/xclaw-telegram-${process.pid}-${process.env.VITEST_POOL_ID ?? "0"}.json`,
}));
...
vi.mock("xclaw/plugin-sdk/web-media", () => ({
  loadWebMedia,
}));
vi.mock("xclaw/plugin-sdk/web-media.js", () => ({
  loadWebMedia,
}));
...
vi.doMock("xclaw/plugin-sdk/config-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("xclaw/plugin-sdk/config-runtime")>();
  return {
    ...actual,
    loadConfig,
...
vi.doMock("xclaw/plugin-sdk/conversation-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("xclaw/plugin-sdk/conversation-runtime")>();
  return {
    ...actual,
...
vi.doMock("xclaw/plugin-sdk/conversation-runtime.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("xclaw/plugin-sdk/conversation-runtime")>();
  return {
    ...actual,
    readChannelAllowFromStore,
...
function createModelsProviderDataFromConfig(cfg: XClawConfig): {
...
vi.doMock("xclaw/plugin-sdk/command-auth", async (importOriginal) => {
  const actual = await importOriginal<typeof import("xclaw/plugin-sdk/command-auth")>();
  return {
    ...actual,
...
vi.doMock("xclaw/plugin-sdk/command-auth.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("xclaw/plugin-sdk/command-auth")>();
  return {
    ...actual,
    listSkillCommandsForAgents: skillCommandListHoisted.listSkillCommandsForAgents,
...
vi.doMock("xclaw/plugin-sdk/reply-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("xclaw/plugin-sdk/reply-runtime")>();
  return {
    ...actual,
...
vi.doMock("xclaw/plugin-sdk/reply-runtime.js", async (importOriginal) => {
  const actual = await importOriginal<typeof import("xclaw/plugin-sdk/reply-runtime")>();
  return {
    ...actual,
    getReplyFromConfig: replySpyHoisted.replySpy,
...
vi.doMock("xclaw/plugin-sdk/infra-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("xclaw/plugin-sdk/infra-runtime")>();
  return {
    ...actual,
...
const DEFAULT_TELEGRAM_TEST_CONFIG: XClawConfig = {
  agents: {
    defaults: {
...
beforeEach(() => {
  ...
  buildModelsProviderData.mockImplementation(async (cfg: XClawConfig) => {
