...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): XClawConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as XClawConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): XClawConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as XClawConfig;
    await expect(
      handleTelegramAction(
        {
          action: "sendSticker",
...
describe("handleTelegramAction per-account gating", () => {
  ...
  async function expectAccountStickerSend(cfg: XClawConfig, accountId = "media") {
    await handleTelegramAction(
