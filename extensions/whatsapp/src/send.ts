...
import { generateSecureUuid } from "xclaw/plugin-sdk/infra-runtime";
...
import { getChildLogger } from "xclaw/plugin-sdk/text-runtime";
...
import { markdownToWhatsApp } from "xclaw/plugin-sdk/text-runtime";
import { toWhatsappJid } from "xclaw/plugin-sdk/text-runtime";
...
export async function sendPollWhatsApp(
  to: string,
  poll: PollInput,
  options: { verbose: boolean; accountId?: string; cfg?: XClawConfig },
): Promise<{ messageId: string; toJid: string }> {
  const correlationId = generateSecureUuid();
  const startedAt = Date.now();
  const { listener: active } = requireActiveWebListener(options.accountId);
