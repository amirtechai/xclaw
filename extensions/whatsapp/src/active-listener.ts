...
import { resolveGlobalSingleton } from "xclaw/plugin-sdk/text-runtime";
...
const WHATSAPP_ACTIVE_LISTENER_STATE_KEY = Symbol.for("xclaw.whatsapp.activeListenerState");
...
export function requireActiveWebListener(accountId?: string | null): {
  ...
  if (!listener) {
    throw new Error(
      `No active WhatsApp Web listener (account: ${id}). Start the gateway, then link WhatsApp with: ${formatCliCommand(`xclaw channels login --channel whatsapp --account ${id}`)}.`,
    );
  }
