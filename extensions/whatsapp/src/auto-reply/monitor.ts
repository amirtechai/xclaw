import { resolveInboundDebounceMs } from "xclaw/plugin-sdk/channel-inbound";
import { formatCliCommand } from "xclaw/plugin-sdk/cli-runtime";
import { waitForever } from "xclaw/plugin-sdk/cli-runtime";
import { hasControlCommand } from "xclaw/plugin-sdk/command-auth";
import { loadConfig } from "xclaw/plugin-sdk/config-runtime";
import { formatDurationPrecise } from "xclaw/plugin-sdk/infra-runtime";
...
import { getReplyFromConfig } from "xclaw/plugin-sdk/reply-runtime";
import { resolveAgentRoute } from "xclaw/plugin-sdk/routing";
import { logVerbose } from "xclaw/plugin-sdk/runtime-env";
import { registerUnhandledRejectionHandler } from "xclaw/plugin-sdk/runtime-env";
import { getChildLogger } from "xclaw/plugin-sdk/runtime-env";
import { defaultRuntime, type RuntimeEnv } from "xclaw/plugin-sdk/runtime-env";
...
export async function monitorWebChannel(
  ...
  while (true) {
    ...
    if (loggedOut) {
      ...
      runtime.error(
        `WhatsApp session logged out. Run \`${formatCliCommand("xclaw channels login --channel web")}\` to relink.`,
      );
      await closeListener();
      break;
    }
    ...
    if (isNonRetryableWebCloseStatus(statusCode)) {
      ...
      runtime.error(
        `WhatsApp Web connection closed (status ${statusCode}: session conflict). Resolve conflicting WhatsApp Web sessions, then relink with \`${formatCliCommand("xclaw channels login --channel web")}\`. Stopping web monitoring.`,
      );
      await closeListener();
      break;
    }
