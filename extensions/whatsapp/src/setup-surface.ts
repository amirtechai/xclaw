...
import type { ChannelSetupWizard } from "xclaw/plugin-sdk/setup";
import { formatCliCommand, formatDocsLink } from "xclaw/plugin-sdk/setup-tools";
...
function mergeWhatsAppConfig(
  cfg: XClawConfig,
  patch: Partial<NonNullable<NonNullable<XClawConfig["channels"]>["whatsapp"]>>,
...
function setWhatsAppDmPolicy(cfg: XClawConfig, dmPolicy: DmPolicy): XClawConfig {
  return mergeWhatsAppConfig(cfg, { dmPolicy });
}

function setWhatsAppAllowFrom(cfg: XClawConfig, allowFrom?: string[]): XClawConfig {
  return mergeWhatsAppConfig(cfg, { allowFrom }, { unsetOnUndefined: ["allowFrom"] });
}

function setWhatsAppSelfChatMode(cfg: XClawConfig, selfChatMode: boolean): XClawConfig {
  return mergeWhatsAppConfig(cfg, { selfChatMode });
}

async function detectWhatsAppLinked(cfg: XClawConfig, accountId: string): Promise<boolean> {
...
async function promptWhatsAppOwnerAllowFrom(params: {
  ...
  await prompter.note(
    "We need the sender/owner number so XClaw can allowlist you.",
    "WhatsApp number",
  );
...
async function promptWhatsAppDmAccess(params: {
  ...
  const phoneMode = await params.prompter.select({
    ...
    options: [
      ...
      { value: "separate", label: "Separate phone just for XClaw" },
    ],
  });
...
export const whatsappSetupWizard: ChannelSetupWizard = {
  ...
  finalize: async ({ cfg, accountId, forceAllowFrom, prompter, runtime }) => {
    ...
    if (wantsLink) {
      ...
      await prompter.note(
        `Run \`${formatCliCommand("xclaw channels login")}\` later to link WhatsApp.`,
        "WhatsApp",
      );
    }
