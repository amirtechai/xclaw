...
import { VERSION } from "xclaw/plugin-sdk/cli-runtime";
import { danger, success } from "xclaw/plugin-sdk/runtime-env";
import { getChildLogger, toPinoLikeLogger } from "xclaw/plugin-sdk/runtime-env";
import { ensureDir, resolveUserPath } from "xclaw/plugin-sdk/text-runtime";
      ...
      try {
        ...
        if (connection === "close") {
          ...
          if (status === LOGGED_OUT_STATUS) {
            console.error(
              danger(
                `WhatsApp session logged out. Run: ${formatCliCommand("xclaw channels login")}`,
              ),
            );
          }
        }
