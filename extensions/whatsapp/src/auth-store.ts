...
import { info, success } from "xclaw/plugin-sdk/runtime-env";
...
import { resolveOAuthDir } from "xclaw/plugin-sdk/state-paths";
import type { WebChannel } from "xclaw/plugin-sdk/text-runtime";
import { jidToE164, resolveUserPath } from "xclaw/plugin-sdk/text-runtime";
...
export async function pickWebChannel(
  ...
  if (!hasWeb) {
    throw new Error(
      `No WhatsApp Web session found. Run \`${formatCliCommand("xclaw channels login --channel whatsapp --verbose")}\` to link.`,
    );
  }
  return choice;
}
