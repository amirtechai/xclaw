...
Official Voice Call plugin for **XClaw**.
...
Docs: `https://docs.xclaw.ai/plugins/voice-call`
Plugin system: `https://docs.xclaw.ai/plugin`

## Install (local dev)

### Option A: install via XClaw (recommended)
...
```bash
mkdir -p ~/.xclaw/extensions
cp -R extensions/voice-call ~/.xclaw/extensions/voice-call
cd ~/.xclaw/extensions/voice-call && pnpm install
```
...
- advanced webhook, streaming, and tunnel notes: `https://docs.xclaw.ai/plugins/voice-call`

## Stale call reaper

See the plugin docs for recommended ranges and production examples:
`https://docs.xclaw.ai/plugins/voice-call#stale-call-reaper`

## TTS for calls

Voice Call uses the core `messages.tts` configuration for
streaming speech on calls. Override examples and provider caveats live here:
`https://docs.xclaw.ai/plugins/voice-call#tts-for-calls`

## CLI

```bash
xclaw voicecall call --to "+15555550123" --message "Hello from XClaw"
xclaw voicecall continue --call-id <id> --message "Any questions?"
xclaw voicecall speak --call-id <id> --message "One moment"
xclaw voicecall end --call-id <id>
xclaw voicecall status --call-id <id>
xclaw voicecall tail
xclaw voicecall expose --mode funnel
```
