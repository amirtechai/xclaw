...
export async function runMatrixStartupMaintenance(params: {
  ...
  try {
    ...
    if (deviceHealth.staleXClawDevices.length > 0) {
      params.logger.warn(
        `matrix: stale XClaw devices detected for ${params.auth.userId}: ${deviceHealth.staleXClawDevices.map((device) => device.deviceId).join(", ")}. Run 'xclaw matrix devices prune-stale --account ${params.effectiveAccountId}' to keep encrypted-room trust healthy.`,
      );
    }
  } catch (err) {
