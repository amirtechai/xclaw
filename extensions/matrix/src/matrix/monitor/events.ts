...
function formatMatrixSelfDecryptionHint(accountId: string): string {
  return (
    ...
    "This usually means another Matrix device did not share the room key, or another XClaw runtime is using the same account. " +
    `Check 'xclaw matrix verify status --verbose --account ${accountId}' and 'xclaw matrix devices list --account ${accountId}'.`
  );
}
