...
`xclaw.invoke --each`) without adding new XClaw code per workflow.
...
This extension depends on XClaw internal modules (the embedded agent runner).
It is intended to ship as a **bundled** XClaw extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.xclaw/extensions` as a standalone plugin directory.
