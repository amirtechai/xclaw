import type { SlackSlashCommandConfig } from "xclaw/plugin-sdk/config-runtime";
...
export function resolveSlackSlashCommandConfig(
  ...
  const normalizedName = normalizeSlackSlashCommandName(raw?.name?.trim() || "xclaw");
  const name = normalizedName || "xclaw";
  return {
