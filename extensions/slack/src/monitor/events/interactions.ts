...
// Prefix for XClaw-generated action IDs to scope our handler
const XCLAW_ACTION_PREFIX = "xclaw:";
...
export function registerSlackInteractionEvents(params: { ctx: SlackMonitorContext }) {
  ...
  const modalMatcher = new RegExp(`^${XCLAW_ACTION_PREFIX}`);

  // Handle XClaw modal submissions with callback_ids scoped by our prefix.
  registerModalLifecycleHandler({
