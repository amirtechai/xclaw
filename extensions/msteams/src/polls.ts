...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["xclawPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["xclaw", "pollId"]) ??
    readNestedString(value, ["xclaw", "poll", "id"]) ??
    readNestedString(value, ["data", "xclawPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "xclaw", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        msteams: {
          type: "messageBack",
          text: "xclaw poll vote",
          displayText: "Vote recorded",
          value: { xclawPollId: pollId, pollId },
        },
      },
    ],
  };
