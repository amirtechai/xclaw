...
import type { XClawPluginApi } from "./api.js";
...
const pluginApiMocks = vi.hoisted(() => ({
  ...
  resolvePreferredXClawTmpDir: vi.fn(() => path.join(os.tmpdir(), "xclaw-device-pair-tests")),
}));
...
vi.mock("./api.js", () => {
  return {
    ...
    resolvePreferredXClawTmpDir: pluginApiMocks.resolvePreferredXClawTmpDir,
...
function createApi(params?: {
  runtime?: XClawPluginApi["runtime"];
  ...
  return createTestPluginApi({
    ...
    runtime: (params?.runtime ?? {}) as XClawPluginApi["runtime"],
...
function registerPairCommand(params?: {
  ...
  let command: XClawPluginCommandDefinition | undefined;
...
function createChannelRuntime(
  ...
  } as unknown as XClawPluginApi["runtime"];
}
...
describe("device-pair /pair qr", () => {
  beforeEach(async () => {
    ...
    await fs.mkdir(pluginApiMocks.resolvePreferredXClawTmpDir(), { recursive: true });
  });

  afterEach(async () => {
    await fs.rm(pluginApiMocks.resolvePreferredXClawTmpDir(), { recursive: true, force: true });
  });
  ...
  it("returns an inline QR image for webchat surfaces", async () => {
    ...
    expect(text).toContain("Scan this QR code with the XClaw iOS app:");
    expect(text).toContain("![XClaw pairing QR](data:image/png;base64,ZmFrZXBuZw==)");
  ...
  it.each([
    ...
    expect(caption).toContain("Scan this QR code with the XClaw iOS app:");
