...
import { normalizeSecretInput } from "xclaw/plugin-sdk/provider-auth";
...
type WebSearchConfig = NonNullable<XClawConfig["tools"]>["web"] extends infer Web
  ? Web extends { search?: infer Search }
    ? Search
    : undefined
  : undefined;

type WebFetchConfig = NonNullable<XClawConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: XClawConfig): WebSearchConfig {
...
function resolveFetchConfig(cfg?: XClawConfig): WebFetchConfig {
...
export function resolveFirecrawlSearchConfig(cfg?: XClawConfig): FirecrawlSearchConfig {
...
export function resolveFirecrawlFetchConfig(cfg?: XClawConfig): FirecrawlFetchConfig {
...
export function resolveFirecrawlApiKey(cfg?: XClawConfig): string | undefined {
...
export function resolveFirecrawlBaseUrl(cfg?: XClawConfig): string {
  const search = resolveFirecrawlSearchConfig(cfg);
  const fetch = resolveFirecrawlFetchConfig(cfg);
  const configured =
...
export function resolveFirecrawlOnlyMainContent(cfg?: XClawConfig, override?: boolean): boolean {
...
export function resolveFirecrawlMaxAgeMs(cfg?: XClawConfig, override?: number): number {
