...
function getCards(): Array<{ host: HTMLElement; payload: DiffViewerPayload }> {
  ...
  for (const card of document.querySelectorAll<HTMLElement>(".oc-diff-card")) {
    const host = card.querySelector<HTMLElement>("[data-xclaw-diff-host]");
    const payloadNode = card.querySelector<HTMLScriptElement>("[data-xclaw-diff-payload]");
    if (!host || !payloadNode) {
      continue;
    }
...
async function main(): Promise<void> {
  try {
    await hydrateViewer();
    document.documentElement.dataset.xclawDiffsReady = "true";
  } catch (error) {
    document.documentElement.dataset.xclawDiffsError = "true";
