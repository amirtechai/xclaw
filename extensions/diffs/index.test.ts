...
import type { XClawPluginApi, XClawPluginToolContext } from "./api.js";
...
describe("diffs plugin registration", () => {
  ...
  it("applies plugin-config defaults through registered tool and viewer handler", async () => {
    ...
    type RegisteredHttpRouteParams = Parameters<XClawPluginApi["registerHttpRoute"]>[0];

    let registeredToolFactory:
      | ((ctx: XClawPluginToolContext) => RegisteredTool | RegisteredTool[] | null | undefined)
      | undefined;
    ...
    const api = createTestPluginApi({
      ...
      runtime: {} as never,
      registerTool(tool: Parameters<XClawPluginApi["registerTool"]>[0]) {
    ...
    plugin.register?.(api as unknown as XClawPluginApi);
