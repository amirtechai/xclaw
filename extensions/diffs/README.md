...
Read-only diff viewer plugin for **XClaw** agents.
...
Set plugin-wide defaults in `~/.xclaw/xclaw.json`:
...
Before:
XClaw supports plugins.

After:
XClaw supports plugins and hosted diff views.
```
...
- Artifacts are ephemeral and stored in the plugin temp subfolder (`$TMPDIR/xclaw-diffs`).
