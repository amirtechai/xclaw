...
import { createScopedDmSecurityResolver } from "xclaw/plugin-sdk/channel-config-helpers";
import { createPairingPrefixStripper } from "xclaw/plugin-sdk/channel-pairing";
import { createOpenProviderConfiguredRouteWarningCollector } from "xclaw/plugin-sdk/channel-policy";
import { resolveTargetsWithOptionalToken } from "xclaw/plugin-sdk/channel-targets";
...
function hasDiscordExecApprovalDmRoute(cfg: XClawConfig): boolean {
    ...
    base: {
      ...
      agentPrompt: {
        messageToolHints: () => [
          ...
          "- Forms: add `components.modal` (title, fields). XClaw adds a trigger button and routes submissions as new messages.",
        ],
      },
      messaging: {
