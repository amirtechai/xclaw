import { DEFAULT_ACCOUNT_ID } from "xclaw/plugin-sdk/account-id";
...
import { createEnvPatchedAccountSetupAdapter } from "xclaw/plugin-sdk/setup-adapter-runtime";
import type {
  ChannelSetupAdapter,
  ChannelSetupDmPolicy,
  ChannelSetupWizard,
} from "xclaw/plugin-sdk/setup-runtime";
import { createStandardChannelSetupStatus } from "xclaw/plugin-sdk/setup-runtime";
import { formatDocsLink } from "xclaw/plugin-sdk/setup-tools";
import {
...
export function createDiscordSetupWizardBase(handlers: {
  ...
  return {
    ...
    credentials: [
      {
        ...
        inspect: ({ cfg, accountId }: { cfg: XClawConfig; accountId: string }) => {
          const account = inspectDiscordSetupAccount({ cfg, accountId });
          return {
    ...
    groupAccess: createAccountScopedGroupAccessSection({
      ...
      currentPolicy: ({ cfg, accountId }: { cfg: XClawConfig; accountId: string }) =>
        resolveDiscordSetupAccountConfig({ cfg, accountId }).config.groupPolicy ?? "allowlist",
      currentEntries: ({ cfg, accountId }: { cfg: XClawConfig; accountId: string }) =>
        Object.entries(
      ...
      updatePrompt: ({ cfg, accountId }: { cfg: XClawConfig; accountId: string }) =>
    ...
    disable: (cfg: XClawConfig) => setSetupChannelEnabled(cfg, channel, false),
