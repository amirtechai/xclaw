...
function normalizeDenyList(cfg: XClawPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: XClawPluginApi["config"]): string[] {
...
export default definePluginEntry({
  ...
  register(api: XClawPluginApi) {
    ...
    const timerService: XClawPluginService = {
