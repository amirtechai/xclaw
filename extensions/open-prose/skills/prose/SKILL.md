...
metadata: { "xclaw": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == XClaw `sessions_spawn`
- **File I/O** == XClaw `read`/`write`
- **Remote fetch** == XClaw `web_fetch` (or `exec` with curl when POST is required)
