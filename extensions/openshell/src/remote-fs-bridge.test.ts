...
describe("openshell remote fs bridge", () => {
  it("writes, reads, renames, and removes files without local host paths", async () => {
    const workspaceDir = await makeTempDir("xclaw-openshell-remote-local-");
    const remoteWorkspaceDir = await makeTempDir("xclaw-openshell-remote-workspace-");
    const remoteAgentDir = await makeTempDir("xclaw-openshell-remote-agent-");
