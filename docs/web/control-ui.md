...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/xclaw`)
...
If the page fails to load, start the Gateway first: `xclaw gateway`.
...
created. Re-run `xclaw devices list` before approval.

Once approved, the device is remembered and won't require re-approval unless
you revoke it with `xclaw devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.xclaw/xclaw.json` (`config.get`, `config.set`)
...
- Stop:
  ...
  - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop xclaw`, `please stop`) to abort out-of-band
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. XClaw
...
XClaw **blocks** Control UI connections without device identity.
