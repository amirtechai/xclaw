...
```bash
xclaw tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `xclaw tui --deliver`
...
- Local shell commands receive `XCLAW_SHELL=tui-local` in their environment.
...
- If your terminal uses a light background and auto-detection is wrong, set `XCLAW_THEME=light` before launching `xclaw tui`.
- To force the original dark palette instead, set `XCLAW_THEME=dark`.
...
- Check the Gateway logs: `xclaw logs --follow`.
- Confirm the agent can run: `xclaw status` and `xclaw models status`.
...
- No agents in picker: check `xclaw agents list` and your routing config.
