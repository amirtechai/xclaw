...
XClaw supports Perplexity Search API as a `web_search` provider.
It returns structured results with `title`, `url`, and `snippet` fields.

For compatibility, XClaw also supports legacy Perplexity Sonar/OpenRouter setups.
...
**Via config:** run `xclaw configure --section web`. It stores the key in
`~/.xclaw/xclaw.json` under `plugins.entries.perplexity.config.webSearch.apiKey`.
...
`~/.xclaw/.env` (or your service environment). See [Env vars](/help/faq#env-vars-and-env-loading).
