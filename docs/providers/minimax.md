---
summary: "Use MiniMax models in XClaw"
read_when:
  - You want MiniMax models in XClaw
...
XClaw's MiniMax provider defaults to **MiniMax M2.7** and keeps
...
```bash
xclaw plugins enable minimax  # skip if already loaded.
xclaw gateway restart  # restart if gateway is already running
xclaw onboard --auth-choice minimax-portal
```
...
See [MiniMax plugin README](https://github.com/xclaw/xclaw/tree/main/extensions/minimax) for details.
...
Configure manually via `xclaw.json`:
...
- Use `xclaw models list` and `xclaw models set minimax/MiniMax-M2.7` to switch.
...
- Running `xclaw configure` and selecting a **MiniMax** auth option, or
