---
summary: "Use Venice AI privacy-focused models in XClaw"
read_when:
  - You want privacy-focused inference in XClaw
...
```bash
xclaw onboard --auth-choice venice-api-key
```
...
```bash
xclaw agent --model venice/kimi-k2-5 --message "Hello, are you working?"
```

## Model Selection

After setup, XClaw shows all available Venice models. Pick based on your needs:
...
```bash
xclaw models set venice/kimi-k2-5
xclaw models set venice/claude-opus-4-6
```

List all available models:

```bash
xclaw models list | grep venice
```

## Configure via `xclaw configure`

1. Run `xclaw configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
XClaw automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
xclaw agent --model venice/kimi-k2-5 --message "Quick health check"

# Use Claude Opus via Venice (anonymized)
xclaw agent --model venice/claude-opus-4-6 --message "Summarize this task"

# Use uncensored model
xclaw agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
xclaw agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
xclaw agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
The Venice model catalog updates dynamically. Run `xclaw models list` to see currently available models. Some models may be temporarily offline.
