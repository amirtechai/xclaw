---
summary: "Run XClaw with SGLang (OpenAI-compatible self-hosted server)"
read_when:
  - You want to run XClaw against a local SGLang server
...
XClaw can connect to SGLang using the `openai-completions` API.

XClaw can also **auto-discover** available models from SGLang when you opt
...
define `models.providers.sglang`, XClaw will query:
