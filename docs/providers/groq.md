...
read_when:
  - You want to use Groq with XClaw
...
(Llama, Gemma, Mistral, and more) using custom LPU hardware. XClaw connects
...
media-understanding provider, XClaw uses Groq's `whisper-large-v3-turbo`
...
available to that process (for example, in `~/.xclaw/.env` or via
`env.shellEnv`).

## Available models

Groq's model catalog changes frequently. Run `xclaw models list | grep groq`
