---
summary: "Use OpenCode Zen and Go catalogs with XClaw"
read_when:
...
OpenCode exposes two hosted catalogs in XClaw:

- `opencode/...` for the **Zen** catalog
- `opencode-go/...` for the **Go** catalog

Both catalogs use the same OpenCode API key. XClaw keeps the runtime provider ids
...
xclaw onboard --opencode-zen-api-key "$OPENCODE_API_KEY"
```

### Go catalog

```bash
xclaw onboard --auth-choice opencode-go
xclaw onboard --opencode-go-api-key "$OPENCODE_API_KEY"
```
