---
summary: "Use Synthetic's Anthropic-compatible API in XClaw"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. XClaw registers it as the
...
```bash
xclaw onboard --auth-choice synthetic-api-key
```
...
Note: XClaw's Anthropic client appends `/v1` to the base URL, so use
