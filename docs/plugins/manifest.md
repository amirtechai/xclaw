...
read_when:
  - You are building an XClaw plugin
  - You need to ship a plugin config schema or debug plugin validation errors
title: "Plugin Manifest"
---

# Plugin manifest (xclaw.plugin.json)

This page is for the **native XClaw plugin manifest** only.
...
XClaw auto-detects those bundle layouts too, but they are not validated
against the `xclaw.plugin.json` schema described here.

For compatible bundles, XClaw currently reads bundle metadata plus declared
skill roots, Claude command roots, Claude bundle `settings.json` defaults, and
supported hook packs when the layout matches XClaw runtime expectations.

Every native XClaw plugin **must** ship a `xclaw.plugin.json` file in the
**plugin root**. XClaw uses this manifest to validate configuration
...
`xclaw.plugin.json` is the metadata XClaw reads before it loads your
plugin code.
...
| `providerAuthEnvVars` | No       | `Record<string, string[]>`       | Cheap provider-auth env metadata that XClaw can inspect without loading plugin code.                                      |
...
XClaw reads this before provider runtime loads.
...
| `choiceLabel`      | No       | `string`                                        | User-facing label. If omitted, XClaw falls back to `choiceId`.                                        |
...
| File                   | Use it for                                                                                                         |
| ---------------------- | ------------------------------------------------------------------------------------------------------------------ |
| `xclaw.plugin.json` | Discovery, config validation, auth-choice metadata, and UI hints that must exist before plugin code runs           |
| `package.json`         | npm metadata, dependency installation, and the `xclaw` block used for entrypoints and setup or catalog metadata |

If you are unsure where a piece of metadata belongs, use this rule:

- if XClaw must know it before loading plugin code, put it in `xclaw.plugin.json`
...
- The manifest is **required for native XClaw plugins**, including local filesystem loads.
