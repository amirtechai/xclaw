...
**Import:** `xclaw/plugin-sdk/plugin-entry`
...
| `kind`         | `string`                                                         | No       | —                   |
| `configSchema` | `XClawPluginConfigSchema \| () => XClawPluginConfigSchema` | No       | Empty object schema |
| `register`     | `(api: XClawPluginApi) => void`                               | Yes      | —                   |

- `id` must match your `xclaw.plugin.json` manifest.
- `kind` is for exclusive slots: `"memory"` or `"context-engine"`.
- `configSchema` can be a function for lazy evaluation.

## `defineChannelPluginEntry`

**Import:** `xclaw/plugin-sdk/core`
...
| `registerFull` | `(api: XClawPluginApi) => void`                               | No       | —                   |
...
XClaw loads this instead of the full entry when a channel is disabled,
...
XClaw classifies loaded plugins by their registration behavior:
...
Use `xclaw plugins inspect <id>` to see a plugin's shape.
