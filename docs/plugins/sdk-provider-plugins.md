...
summary: "Step-by-step guide to building a model provider plugin for XClaw"
read_when:
  - You are building a new model provider plugin
  - You want to add an OpenAI-compatible proxy or custom LLM to XClaw
...
(LLM) to XClaw. By the end you will have a provider with a model catalog,
API key auth, and dynamic model resolution.

<Info>
  If you have not built any XClaw plugin before, read
...
<Steps>
  <Step title="Package and manifest">
    ...
    The manifest declares `providerAuthEnvVars` so XClaw can detect
  ...
  <Step title="Register the provider">
    ...
    `xclaw onboard --acme-ai-api-key <key>` and select
    ...
    ```typescript
    import { defineSingleProviderPluginEntry } from "xclaw/plugin-sdk/provider-entry";
    ...
    `xclaw/plugin-sdk/provider-onboard`. The narrowest helpers are
  ...
  <Step title="Add runtime hooks (as needed)">
    ...
    <Accordion title="All available provider hooks">
      XClaw calls hooks in this order. Most providers only use 2-3:
  ...
  <Step title="Add extra capabilities (optional)">
    ...
    XClaw classifies this as a **hybrid-capability** plugin. This is the
...
```
extensions/acme-ai/
├── package.json              # xclaw.providers metadata
├── xclaw.plugin.json      # Manifest with providerAuthEnvVars
