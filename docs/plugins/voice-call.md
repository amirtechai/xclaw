...
read_when:
  - You want to place an outbound voice call from XClaw
...
Voice calls for XClaw via a plugin. Supports outbound notifications and
...
- Use `xclaw voicecall ...` or the `voice_call` tool
...
```bash
xclaw plugins install ./extensions/voice-call
...
xclaw voicecall start --to "+15555550123"   # alias for call
...
xclaw voicecall latency                     # summarize turn latency from logs
