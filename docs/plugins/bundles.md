---
summary: "Install and use Codex, Claude, and Cursor bundles as XClaw plugins"
read_when:
  - You want to install a Codex, Claude, or Cursor-compatible bundle
  - You need to understand how XClaw maps bundle content into native features
...
XClaw can install plugins from three external ecosystems: **Codex**, **Claude**,
and **Cursor**. These are called **bundles** — content and metadata packs that
XClaw maps into native features like skills, hooks, and MCP tools.

<Info>
  Bundles are **not** the same as native XClaw plugins. Native plugins run
...
of requiring authors to rewrite them as native XClaw plugins, XClaw
...
## What XClaw maps from bundles

Not every bundle feature runs in XClaw today. Here is what works and what
...
| ------------- | ---------------------------------------------------------------------------------------------------- | -------------- |
| Skill content | Bundle skill roots load as normal XClaw skills                                                    | All formats    |
| Commands      | `commands/` and `.cursor/commands/` treated as skill roots                                           | Claude, Cursor |
| Hook packs    | XClaw-style `HOOK.md` + `handler.ts` layouts                                                      | Codex          |
...
These are recognized and shown in diagnostics, but XClaw does not run them:
...
<AccordionGroup>
  <Accordion title="Codex bundles">
    ...
    Codex bundles fit XClaw best when they use skill roots and XClaw-style
...
XClaw checks for native plugin format first:

1. `xclaw.plugin.json` or valid `package.json` with `xclaw.extensions` — treated as **native plugin**
2. Bundle markers (`.codex-plugin/`, `.claude-plugin/`, or default Claude/Cursor layout) — treated as **bundle**

If a directory contains both, XClaw uses the native path. This prevents
...
- XClaw does **not** load arbitrary bundle runtime modules in-process
...
<AccordionGroup>
  <Accordion title="Bundle is detected but capabilities do not run">
    Run `xclaw plugins inspect <id>`. If a capability is listed but marked as
  ...
  <Accordion title="Claude settings do not apply">
    Only embedded Pi settings from `settings.json` are supported. XClaw does
  ...
  <Accordion title="Claude hooks do not execute">
    ...
    XClaw hook-pack layout or ship a native plugin.
  </Accordion>
</AccordionGroup>
