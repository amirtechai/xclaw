...
summary: "Create your first XClaw plugin in minutes"
read_when:
  - You want to create a new XClaw plugin
  - You need a quick-start for plugin development
  - You are adding a new channel, provider, tool, or other capability to XClaw
---

# Building Plugins

Plugins extend XClaw with new capabilities: channels, model providers, speech,
image generation, web search, agent tools, or any combination.

You do not need to add your plugin to the XClaw repository. Publish to
[ClawHub](/tools/clawhub) or npm and users install with
`xclaw plugins install <package-name>`. XClaw tries ClawHub first and
...
<CardGroup cols={3}>
  <Card title="Channel plugin" icon="messages-square" href="/plugins/sdk-channel-plugins">
    Connect XClaw to a messaging platform (Discord, IRC, etc.)
  </Card>
...
<Steps>
  <Step title="Create the package and manifest">
    ...
    ```json xclaw.plugin.json
    {
      "id": "my-plugin",
      "name": "My Plugin",
      "description": "Adds a custom tool to XClaw",
      "configSchema": {
        "type": "object",
  ...
  <Step title="Test and publish">
    ...
    **External plugins:** publish to [ClawHub](/tools/clawhub) or npm, then install:
    ...
    XClaw checks ClawHub first, then falls back to npm.
...
Always import from focused `xclaw/plugin-sdk/<subpath>` paths:
...
<Check>**package.json** has correct `xclaw` metadata</Check>
<Check>**xclaw.plugin.json** manifest is present and valid</Check>
