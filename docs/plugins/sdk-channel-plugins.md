...
summary: "Step-by-step guide to building a messaging channel plugin for XClaw"
read_when:
  - You are building a new messaging channel plugin
  - You want to connect XClaw to a messaging platform
  - You need to understand the ChannelPlugin adapter surface
---

# Building Channel Plugins

This guide walks through building a channel plugin that connects XClaw to a
...
Channel plugins do not need their own send/edit/react tools. XClaw keeps one
...
<Steps>
    ...
    ```json package.json
    {
      ...
      "xclaw": {
        ...
        "channel": {
          "id": "acme-chat",
          "label": "Acme Chat",
          "blurb": "Connect XClaw to Acme Chat."
        }
      }
    }
    ```
  ...
  <Step title="Wire the entry point">
    ...
    ```typescript index.ts
    import { defineChannelPluginEntry } from "xclaw/plugin-sdk/core";
  ...
  <Step title="Add a setup entry">
    ...
    XClaw loads this instead of the full entry when the channel is disabled
  ...
  <Step title="Handle inbound messages">
    ...
    XClaw. The typical pattern is a webhook that verifies the request and
    ...
    registerFull(api) {
      api.registerHttpRoute({
        ...
        handler: async (req, res) => {
          ...
          // Your inbound handler dispatches the message to XClaw.
...
├── package.json              # xclaw.channel metadata
├── xclaw.plugin.json      # Manifest with config schema
