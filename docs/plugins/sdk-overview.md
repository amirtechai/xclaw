...
read_when:
  ...
  - You want a reference for all registration methods on XClawPluginApi
...
The `register(api)` callback receives an `XClawPluginApi` object with these
methods:
...
| `api.config`             | `XClawConfig`          | Current config snapshot                                   |
...
<Warning>
  Never import your own plugin through `xclaw/plugin-sdk/<your-plugin>`
