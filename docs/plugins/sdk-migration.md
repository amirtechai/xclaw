...
read_when:
  - You see the XCLAW_PLUGIN_SDK_COMPAT_DEPRECATED warning
  - You see the XCLAW_EXTENSION_API_DEPRECATED warning
  - You are updating a plugin to the modern plugin architecture
  - You maintain an external XClaw plugin
---

# Plugin SDK Migration

XClaw has moved from a broad backwards-compatibility layer to a modern plugin
...
- **`xclaw/plugin-sdk/compat`** — a single import that re-exported dozens of
  helpers. It was introduced to keep older hook-based plugins working while the
  new plugin architecture was being built.
- **`xclaw/extension-api`** — a bridge that gave plugins direct access to
...
The modern plugin SDK fixes this: each import path (`xclaw/plugin-sdk/\<subpath\>`)
...
<Steps>
  <Step title="Find deprecated imports">
    ...
    grep -r "xclaw/extension-api" my-plugin/
    ```
  ...
  <Step title="Replace with focused imports">
    ...
    import { runEmbeddedPiAgent } from "xclaw/extension-api";
...
```bash
XCLAW_SUPPRESS_PLUGIN_SDK_COMPAT_WARNING=1 xclaw gateway run
XCLAW_SUPPRESS_EXTENSION_API_WARNING=1 xclaw gateway run
```
