...
service via the external `xclaw` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `ai.xclaw.gateway`
  (or `ai.xclaw.<profile>` when using `--profile`/`XCLAW_PROFILE`; legacy `com.xclaw.*` is supported).
