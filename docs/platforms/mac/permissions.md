...
- Same path: run the app from a fixed location (for XClaw, `dist/XClaw.app`).
...
```bash
sudo tccutil reset Accessibility ai.xclaw.mac
sudo tccutil reset ScreenCapture ai.xclaw.mac
...
Workaround: move files into the XClaw workspace (`~/.xclaw/workspace`) if you want to avoid per-folder grants.
