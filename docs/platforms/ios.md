...
- Network path:
  ...
  - Tailnet via unicast DNS-SD (example domain: `xclaw.internal.`), **or**
...
Run `xclaw devices list` again before approval.

4. Verify connection:

```bash
xclaw nodes status
xclaw gateway call node.list --params "{}"
```
...
- `XCLAW_APNS_RELAY_BASE_URL` still works as a temporary env override for the gateway.
...
- Only genuine XClaw iOS builds distributed through Apple can use the hosted relay.
...
- To allow hosted relay usage only for official/TestFlight XClaw builds.
...
```bash
export XCLAW_APNS_TEAM_ID="TEAMID"
export XCLAW_APNS_KEY_ID="KEYID"
export XCLAW_APNS_PRIVATE_KEY_P8="$(cat /path/to/AuthKey_KEYID.p8)"
```
...
The Gateway advertises `_xclaw-gw._tcp` on `local.`. The iOS app lists these automatically.

### Tailnet (cross-network)

If mDNS is blocked, use a unicast DNS-SD zone (choose a domain; example: `xclaw.internal.`) and Tailscale split DNS.
...
```bash
xclaw nodes invoke --node "iOS Node" --command canvas.navigate --params '{"url":"http://<gateway-host>:18789/__xclaw__/canvas/"}'
```

Notes:

- The Gateway canvas host serves `/__xclaw__/canvas/` and `/__xclaw__/a2ui/`.
...
```bash
xclaw nodes invoke --node "iOS Node" --command canvas.eval --params '{"javaScript":"(() => { const {ctx} = window.__xclaw; ctx.clearRect(0,0,innerWidth,innerHeight); ctx.lineWidth=6; ctx.strokeStyle=\"#ff2d55\"; ctx.beginPath(); ctx.moveTo(40,40); ctx.lineTo(innerWidth-40, innerHeight-40); ctx.stroke(); return \"ok\"; })()"}'
```

```bash
xclaw nodes invoke --node "iOS Node" --command canvas.snapshot --params '{"maxWidth":900,"format":"jpeg"}'
```
...
- Pairing prompt never appears: run `xclaw devices list` and approve manually.
