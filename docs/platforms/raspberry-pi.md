---
summary: "XClaw on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up XClaw on a Raspberry Pi
  - Running XClaw on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi (Platform)"
---

# XClaw on Raspberry Pi

## Goal

Run a persistent, always-on XClaw Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
# View logs
journalctl -u xclaw -f
```

## 9) Access the XClaw Dashboard
...
```bash
ssh user@gateway-host 'xclaw dashboard --no-open'
```
...
(or `XCLAW_GATEWAY_TOKEN`) into Control UI settings.
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/xclaw-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF' # pragma: allowlist secret
export NODE_COMPILE_CACHE=/var/tmp/xclaw-compile-cache
mkdir -p /var/tmp/xclaw-compile-cache
export XCLAW_NO_RESPAWN=1
EOF
source ~/.bashrc
```
...
- `XCLAW_NO_RESPAWN=1` avoids extra startup cost from CLI self-respawn.
- First run warms the cache; later runs benefit most.

### systemd startup tuning (optional)

If this Pi is mostly running XClaw, add a service drop-in to reduce restart
jitter and keep startup env stable:

```bash
sudo systemctl edit xclaw
```

```ini
[Service]
Environment=XCLAW_NO_RESPAWN=1
Environment=NODE_COMPILE_CACHE=/var/tmp/xclaw-compile-cache
Restart=always
RestartSec=2
TimeoutStartSec=90
```
...
If possible, keep XClaw state/cache on SSD-backed storage to avoid SD-card
...
Most XClaw features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled xclaw
...
```bash
# Check logs
journalctl -u xclaw --no-pager -n 100

# Common fix: rebuild
cd ~/xclaw  # if using hackable install
npm run build
sudo systemctl restart xclaw
```
