...
XClaw supports **camera capture** for agent workflows:
...
```bash
xclaw nodes camera snap --node <id>               # default: both front + back (2 MEDIA lines)
xclaw nodes camera snap --node <id> --facing front
xclaw nodes camera clip --node <id> --duration 3000
xclaw nodes camera clip --node <id> --no-audio
```
...
- **Settings → General → Allow Camera** (`xclaw.cameraEnabled`)
  - Default: **off**
  - When off: camera requests return “Camera disabled by user”.

### CLI helper (node invoke)

Use the main `xclaw` CLI to invoke camera commands on the macOS node.

Examples:

```bash
xclaw nodes camera list --node <id>            # list camera ids
xclaw nodes camera snap --node <id>            # prints MEDIA:<path>
xclaw nodes camera snap --node <id> --max-width 1280
xclaw nodes camera snap --node <id> --delay-ms 2000
xclaw nodes camera snap --node <id> --device-id <id>
xclaw nodes camera clip --node <id> --duration 10s          # prints MEDIA:<path>
xclaw nodes camera clip --node <id> --duration-ms 3000      # prints MEDIA:<path> (legacy flag)
xclaw nodes camera clip --node <id> --device-id <id>
xclaw nodes camera clip --node <id> --no-audio
```

Notes:

- `xclaw nodes camera snap` defaults to `maxWidth=1600` unless overridden.
...
```bash
xclaw nodes screen record --node <id> --duration 10s --fps 15   # prints MEDIA:<path>
```
