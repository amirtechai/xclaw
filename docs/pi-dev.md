...
This guide summarizes a sane workflow for working on the pi integration in XClaw.
...
```bash
XCLAW_LIVE_TEST=1 pnpm test -- src/agents/pi-embedded-runner-extraparams.live.test.ts
```
...
- Trigger the agent directly:
  - `pnpm xclaw agent --message "Hello" --thinking low`
...
State lives under the XClaw state directory. Default is `~/.xclaw`. If `XCLAW_STATE_DIR` is set, use that directory instead.

To reset everything:

- `xclaw.json` for config
