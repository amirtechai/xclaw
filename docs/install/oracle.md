---
summary: "Host XClaw on Oracle Cloud's Always Free ARM tier"
read_when:
  - Setting up XClaw on Oracle Cloud
  - Looking for free VPS hosting for XClaw
...
Run a persistent XClaw Gateway on Oracle Cloud's **Always Free** ARM tier (up to 4 OCPU, 24 GB RAM, 200 GB storage) at no cost.
...
<Steps>
  ...
  <Step title="Install Tailscale">
    ...
    From now on, connect via Tailscale: `ssh ubuntu@xclaw`.
...
**Tailscale will not connect** -- Run `sudo tailscale up --ssh --hostname=xclaw --reset` to re-authenticate.

**Gateway will not start** -- Run `xclaw doctor --non-interactive` and check logs with `journalctl --user -u xclaw-gateway -n 50`.
...
- [Updating](/install/updating) -- keep XClaw up to date
