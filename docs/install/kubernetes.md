---
summary: "Deploy XClaw Gateway to a Kubernetes cluster with Kustomize"
read_when:
  - You want to run XClaw on a Kubernetes cluster
  - You want to test XClaw in a Kubernetes environment
title: "Kubernetes"
---

# XClaw on Kubernetes

A minimal starting point for running XClaw on Kubernetes — not a production-ready deployment. It covers the core resources and is meant to be adapted to your environment.

## Why not Helm?

XClaw is a single container with some config files. The interesting customization is in agent content (markdown files, skills, config overrides), not infrastructure templating. Kustomize handles overlays without the overhead of a Helm chart. If your deployment grows more complex, a Helm chart can be layered on top of these manifests.
...
kubectl port-forward svc/xclaw 18789:18789 -n xclaw
...
```bash
kubectl get secret xclaw-secrets -n xclaw -o jsonpath='{.data.XCLAW_GATEWAY_TOKEN}' | base64 -d
```
...
```
Namespace: xclaw (configurable via XCLAW_NAMESPACE)
├── Deployment/xclaw        # Single pod, init container + gateway
├── Service/xclaw           # ClusterIP on port 18789
├── PersistentVolumeClaim      # 10Gi for agent state and config
├── ConfigMap/xclaw-config  # xclaw.json + AGENTS.md
└── Secret/xclaw-secrets    # Gateway token + API keys
```
...
Edit `xclaw.json` in `scripts/k8s/manifests/configmap.yaml`. See [Gateway configuration](/gateway/configuration) for the full reference.
...
```bash
kubectl patch secret xclaw-secrets -n xclaw \
  -p '{"stringData":{"<PROVIDER>_API_KEY":"..."}}'
kubectl rollout restart deployment/xclaw -n xclaw
```

### Custom namespace

```bash
XCLAW_NAMESPACE=my-namespace ./scripts/k8s/deploy.sh
```
...
```yaml
image: ghcr.io/xclaw/xclaw:latest # or pin to a specific version from https://github.com/xclaw/xclaw/releases
```
...
└── manifests/
    ...
    ├── configmap.yaml          # xclaw.json + AGENTS.md
