---
summary: "Move (migrate) an XClaw install from one machine to another"
read_when:
  - You are moving XClaw to a new laptop/server
  - You want to preserve sessions, auth, and channel logins (WhatsApp, etc.)
title: "Migration Guide"
---

# Migrating XClaw to a New Machine

This guide moves an XClaw gateway to a new machine without redoing onboarding.

## What Gets Migrated

When you copy the **state directory** (`~/.xclaw/` by default) and your **workspace**, you preserve:

- **Config** -- `xclaw.json` and all gateway settings
...
<Tip>
Run `xclaw status` on the old machine to confirm your state directory path.
Custom profiles use `~/.xclaw-<profile>/` or a path set via `XCLAW_STATE_DIR`.
</Tip>
...
<Steps>
  <Step title="Stop the gateway and back up">
    ...
    xclaw gateway stop
    cd ~
    tar -czf xclaw-state.tgz .xclaw
    ```

    If you use multiple profiles (e.g. `~/.xclaw-work`), archive each separately.

  </Step>

  <Step title="Install XClaw on the new machine">
    [Install](/install) the CLI (and Node if needed) on the new machine.
    It is fine if onboarding creates a fresh `~/.xclaw/` -- you will overwrite it next.
  </Step>
...
<AccordionGroup>
  <Accordion title="Profile or state-dir mismatch">
    If the old gateway used `--profile` or `XCLAW_STATE_DIR` and the new one does not,
    channels will appear logged out and sessions will be empty.
    Launch the gateway with the **same** profile or state-dir you migrated, then rerun `xclaw doctor`.
  </Accordion>

  <Accordion title="Copying only xclaw.json">
  ...
  <Accordion title="Remote mode">
    ...
    Migrate the gateway host itself, not your local laptop. See [FAQ](/help/faq#where-does-xclaw-store-its-data).
  </Accordion>
...
- [ ] `xclaw status` shows the gateway running
