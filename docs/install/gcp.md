---
summary: "Run XClaw Gateway 24/7 on a GCP Compute Engine VM (Docker) with durable state"
read_when:
  - You want XClaw running 24/7 on GCP
  - You want a production-grade, always-on Gateway on your own VM
  - You want full control over persistence, binaries, and restart behavior
title: "GCP"
---

# XClaw on GCP Compute Engine (Docker, Production VPS Guide)

## Goal

Run a persistent XClaw Gateway on a GCP Compute Engine VM using Docker, with durable state, baked-in binaries, and safe restart behavior.

If you want "XClaw 24/7 for ~$5-12/mo", this is a reliable setup on Google Cloud.
...
- Start the XClaw Gateway in Docker
- Persist `~/.xclaw` + `~/.xclaw/workspace` on the host (survives restarts/rebuilds)
...
<Steps>
  ...
  <Step title="Create a GCP project">
    **CLI:**
    ...
    ```bash
    gcloud projects create my-xclaw-project --name="XClaw Gateway"
    gcloud config set project my-xclaw-project
    ```
  ...
  <Step title="Create the VM">
    ...
    **CLI:**
    ...
    ```bash
    gcloud compute instances create xclaw-gateway \
      --zone=us-central1-a \
  ...
  <Step title="SSH into the VM">
    **CLI:**
    ...
    ```bash
    gcloud compute ssh xclaw-gateway --zone=us-central1-a
    ```
  ...
  <Step title="Clone the XClaw repository">
    ```bash
  ...
  <Step title="Configure environment variables">
    ...
    ```bash
    XCLAW_IMAGE=xclaw:latest
    XCLAW_GATEWAY_TOKEN=change-me-now
    XCLAW_GATEWAY_BIND=lan
    XCLAW_GATEWAY_PORT=18789

    XCLAW_CONFIG_DIR=/home/$USER/.xclaw
    XCLAW_WORKSPACE_DIR=/home/$USER/.xclaw/workspace

    GOG_KEYRING_PASSWORD=change-me-now
    XDG_CONFIG_HOME=/home/node/.xclaw
    ```
    ...
    services:
      xclaw-gateway:
        ...
        environment:
          ...
          - NODE_ENV=production
          - TERM=xterm-256color
          - XCLAW_GATEWAY_BIND=${XCLAW_GATEWAY_BIND}
          - XCLAW_GATEWAY_PORT=${XCLAW_GATEWAY_PORT}
          - XCLAW_GATEWAY_TOKEN=${XCLAW_GATEWAY_TOKEN}
          - GOG_KEYRING_PASSWORD=${GOG_KEYRING_PASSWORD}
          - XDG_CONFIG_HOME=${XDG_CONFIG_HOME}
          - PATH=/home/linuxbrew/.linuxbrew/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin
        volumes:
          - ${XCLAW_CONFIG_DIR}:/home/node/.xclaw
          - ${XCLAW_WORKSPACE_DIR}:/home/node/.xclaw/workspace
        ports:
          # Recommended: keep the Gateway loopback-only on the VM; access via SSH tunnel.
          # To expose it publicly, remove the `127.0.0.1:` prefix and firewall accordingly.
          - "127.0.0.1:${XCLAW_GATEWAY_PORT}:18789"
        command:
          [
            "node",
            "dist/index.js",
  ...
  <Step title="GCP-specific launch notes">
    ...
    When binding to LAN (`XCLAW_GATEWAY_BIND=lan`), configure a trusted browser origin before continuing:

    ```bash
    docker compose run --rm xclaw-cli config set gateway.controlUi.allowedOrigins '["http://127.0.0.1:18789"]' --strict-json
    ```
  ...
  <Step title="Access from your laptop">
    ...
    ```bash
    gcloud compute ssh xclaw-gateway --zone=us-central1-a -- -L 18789:127.0.0.1:18789
    ```
...
```bash
# Stop the VM first
gcloud compute instances stop xclaw-gateway --zone=us-central1-a

# Change machine type
gcloud compute instances set-machine-type xclaw-gateway \
  --zone=us-central1-a \
  --machine-type=e2-small

# Start the VM
gcloud compute instances start xclaw-gateway --zone=us-central1-a
```
...
1. Create a service account:
   ...
   ```bash
   gcloud iam service-accounts create xclaw-deploy \
     --display-name="XClaw Deployment"
   ```

2. Grant Compute Instance Admin role (or narrower custom role):

   ```bash
   gcloud projects add-iam-policy-binding my-xclaw-project \
     --member="serviceAccount:xclaw-deploy@my-xclaw-project.iam.gserviceaccount.com" \
