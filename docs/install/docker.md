---
summary: "Optional Docker-based setup and onboarding for XClaw"
read_when:
...
- **Yes**: you want an isolated, throwaway gateway environment or to run XClaw on a host without local installs.
...
<Steps>
  <Step title="Build the image">
    ...
    ```bash
    export XCLAW_IMAGE="ghcr.io/xclaw/xclaw:latest"
    ./scripts/docker/setup.sh
    ```

    Pre-built images are published at the
    [GitHub Container Registry](https://github.com/xclaw/xclaw/pkgs/container/xclaw).
...
<Note>
Run `docker compose` from the repo root. If you enabled `XCLAW_EXTRA_MOUNTS`
or `XCLAW_HOME_VOLUME`, the setup script writes `docker-compose.extra.yml`;
...
| Variable                       | Purpose                                                          |
| ------------------------------ | ---------------------------------------------------------------- |
| `XCLAW_IMAGE`               | Use a remote image instead of building locally                   |
| `XCLAW_DOCKER_APT_PACKAGES` | Install extra apt packages during build (space-separated)        |
| `XCLAW_EXTENSIONS`          | Pre-install extension deps at build time (space-separated names) |
| `XCLAW_EXTRA_MOUNTS`        | Extra host bind mounts (comma-separated `source:target[:opts]`)  |
| `XCLAW_HOME_VOLUME`         | Persist `/home/node` in a named Docker volume                    |
| `XCLAW_SANDBOX`             | Opt in to sandbox bootstrap (`1`, `true`, `yes`, `on`)           |
| `XCLAW_DOCKER_SOCKET`       | Override Docker socket path                                      |
...
`scripts/docker/setup.sh` defaults `XCLAW_GATEWAY_BIND=lan` so host access to
...
Docker Compose bind-mounts `XCLAW_CONFIG_DIR` to `/home/node/.xclaw` and
`XCLAW_WORKSPACE_DIR` to `/home/node/.xclaw/workspace`, so those paths
...
and rolling file logs under `/tmp/xclaw/`.
...
```bash
mkdir -p ~/.clawdock && curl -sL https://raw.githubusercontent.com/xclaw/xclaw/main/scripts/shell-helpers/clawdock-helpers.sh -o ~/.clawdock/clawdock-helpers.sh
echo 'source ~/.clawdock/clawdock-helpers.sh' >> ~/.zshrc && source ~/.zshrc
```

Then use `clawdock-start`, `clawdock-stop`, `clawdock-dashboard`, etc. Run
`clawdock-help` for all commands.
See the [`ClawDock` Helper README](https://github.com/xclaw/xclaw/blob/main/scripts/shell-helpers/README.md).

<AccordionGroup>
  <Accordion title="Enable agent sandbox for Docker gateway">
    ```bash
    export XCLAW_SANDBOX=1
    ./scripts/docker/setup.sh
    ```
    ...
    ```bash
    export XCLAW_SANDBOX=1
    export XCLAW_DOCKER_SOCKET=/run/user/1000/docker.sock
    ./scripts/docker/setup.sh
    ```
  ...
  <Accordion title="Automation / CI (non-interactive)">
    ...
    ```bash
    docker compose run -T --rm xclaw-cli gateway probe
    docker compose run -T --rm xclaw-cli devices list --json
    ```

  </Accordion>

  <Accordion title="Shared-network security note">
    `xclaw-cli` uses `network_mode: "service:xclaw-gateway"` so CLI
    ...
    `no-new-privileges` on `xclaw-cli`.
  </Accordion>

  <Accordion title="Permissions and EACCES">
    The image runs as `node` (uid 1000). If you see permission errors on
    `/home/node/.xclaw`, make sure your host bind mounts are owned by uid 1000:
  ...
  <Accordion title="Power-user container options">
    ...
    1. **Persist `/home/node`**: `export XCLAW_HOME_VOLUME="xclaw_home"`
    2. **Bake system deps**: `export XCLAW_DOCKER_APT_PACKAGES="git curl jq"`
    ...
    4. **Persist browser downloads**: set
       ...
       `XCLAW_HOME_VOLUME` or `XCLAW_EXTRA_MOUNTS`.
...
<AccordionGroup>
  <Accordion title="Image missing or sandbox container not starting">
    ...
    [`scripts/sandbox-setup.sh`](https://github.com/xclaw/xclaw/blob/main/scripts/sandbox-setup.sh)
  ...
  <Accordion title="Custom tools not found in sandbox">
    XClaw runs commands with `sh -lc` (login shell), which sources
  ...
  <Accordion title="Gateway target shows ws://172.x.x.x or pairing errors from Docker CLI">
    ...
    ```bash
    docker compose run --rm xclaw-cli config set gateway.mode local
    docker compose run --rm xclaw-cli config set gateway.bind lan
    docker compose run --rm xclaw-cli devices list --url ws://127.0.0.1:18789
    ```
