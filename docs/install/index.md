---
summary: "Install XClaw — installer script, npm/pnpm, from source, Docker, and more"
read_when:
...
The fastest way to install. It detects your OS, installs Node if needed, installs XClaw, and launches onboarding.
...
Or skip the link and use `pnpm xclaw ...` from inside the repo. See [Setup](/start/setup) for full development workflows.
...
```bash
xclaw --version      # confirm the CLI is available
xclaw doctor         # check for config issues
xclaw gateway status # verify the Gateway is running
```

## Hosting and deployment

Deploy XClaw on a cloud server or VPS:
...
<CardGroup cols={3}>
  <Card title="Updating" href="/install/updating" icon="refresh-cw">
    Keep XClaw up to date.
  </Card>
  <Card title="Migrating" href="/install/migrating" icon="arrow-right">
    Move to a new machine.
  </Card>
  <Card title="Uninstall" href="/install/uninstall" icon="trash-2">
    Remove XClaw completely.
  </Card>
</CardGroup>

## Troubleshooting: `xclaw` not found

If the install succeeded but `xclaw` is not found in your terminal:
