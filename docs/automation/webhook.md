...
read_when:
  ...
  - Wiring external systems into XClaw
title: "Webhooks"
---
...
- `hooks.transformsDir` + `transform.module` loads a JS/TS module for custom logic.
  - `hooks.transformsDir` (if set) must stay within the transforms root under your XClaw config directory (typically `~/.xclaw/hooks/transforms`).
...
- `xclaw webhooks gmail setup` writes `hooks.gmail` config for `xclaw webhooks gmail run`.
