...
Hooks provide an extensible event-driven system for automating actions in response to agent commands and events. Hooks are automatically discovered from directories and can be inspected with `xclaw hooks`, while hook-pack installation and updates now go through `xclaw plugins`.
...
- **Webhooks**: external HTTP webhooks that let other systems trigger work in XClaw. See [Webhook Hooks](/automation/webhook) or use `xclaw webhooks` for Gmail helper commands.

Hooks can also be bundled inside plugins; see [Plugin hooks](/plugins/architecture#provider-runtime-hooks). `xclaw hooks list` shows both standalone hooks and plugin-managed hooks.
...
If you can write a small TypeScript function, you can write a hook. Managed and bundled hooks are trusted local code. Workspace hooks are discovered automatically, but XClaw keeps them disabled until you explicitly enable them via the CLI or config.
...
- Extend XClaw's behavior without modifying core code

## Getting Started

### Bundled Hooks

XClaw ships with four bundled hooks that are automatically discovered:

- **💾 session-memory**: Saves session context to your agent workspace (default `~/.xclaw/workspace/memory/`) when you issue `/new` or `/reset`
- **📎 bootstrap-extra-files**: Injects additional workspace bootstrap files from configured glob/path patterns during `agent:bootstrap`
- **📝 command-logger**: Logs all command events to `~/.xclaw/logs/commands.log`
...
```bash
xclaw hooks enable session-memory
```
...
```bash
xclaw hooks info session-memory
```

### Onboarding

During onboarding (`xclaw onboard`), you'll be prompted to enable recommended hooks. The wizard automatically discovers eligible hooks and presents them for selection.

### Trust Boundary

Hooks run inside the Gateway process. Treat bundled hooks, managed hooks, and `hooks.internal.load.extraDirs` as trusted local code. Workspace hooks under `<workspace>/hooks/` are repo-local code, so XClaw requires an explicit enable step before loading them.

## Hook Discovery

Hooks are automatically discovered from these directories, in order of increasing override precedence:

1. **Bundled hooks**: shipped with XClaw; located at `<xclaw>/dist/hooks/bundled/` for npm installs (or a sibling `hooks/bundled/` for compiled binaries)
2. **Plugin hooks**: hooks bundled inside installed plugins (see [Plugin hooks](/plugins/architecture#provider-runtime-hooks))
3. **Managed hooks**: `~/.xclaw/hooks/` (user-installed, shared across workspaces; can override bundled and plugin hooks). **Extra hook directories** configured via `hooks.internal.load.extraDirs` are also treated as managed hooks and share the same override precedence.
...
Hook packs are standard npm packages that export one or more hooks via `xclaw.hooks` in
`package.json`. Install them with:

```bash
xclaw plugins install <path-or-spec>
```
...
those to a prerelease, XClaw stops and asks you to opt in explicitly with a
...
Hook packs can ship dependencies; they will be installed under `~/.xclaw/hooks/<id>`.
Each `xclaw.hooks` entry must stay inside the package directory after symlink
resolution; entries that escape are rejected.

Security note: `xclaw plugins install` installs hook-pack dependencies with `npm install --ignore-scripts`
...
homepage: https://docs.xclaw.ai/automation/hooks#my-hook
metadata:
  { "xclaw": { "emoji": "🔗", "events": ["command:new"], "requires": { "bins": ["node"] } } }
---
...
The `metadata.xclaw` object supports:
...
These hooks are not event-stream listeners; they let plugins synchronously adjust tool results before XClaw persists them.
...
- **Managed hooks** (`~/.xclaw/hooks/`): Shared across workspaces; can override bundled and plugin hooks

### 2. Create Directory Structure

```bash
mkdir -p ~/.xclaw/hooks/my-hook
cd ~/.xclaw/hooks/my-hook
```
...
metadata: { "xclaw": { "emoji": "🎯", "events": ["command:new"] } }
---
...
xclaw hooks list --eligible

# Verbose output (show missing requirements)
xclaw hooks list --verbose

# JSON output
xclaw hooks list --json
```
...
# JSON output
xclaw hooks info session-memory --json
```
...
# Disable a hook
xclaw hooks disable command-logger
```
...
**Output**: `<workspace>/memory/YYYY-MM-DD-slug.md` (defaults to `~/.xclaw/workspace`)
...
```bash
xclaw hooks enable bootstrap-extra-files
```
...
**Output**: `~/.xclaw/logs/commands.log`
...
tail -n 20 ~/.xclaw/logs/commands.log

# Pretty-print with jq
cat ~/.xclaw/logs/commands.log | jq .

# Filter by action
grep '"action":"new"' ~/.xclaw/logs/commands.log | jq .
```

**Enable**:

```bash
xclaw hooks enable command-logger
```
...
```yaml
metadata: { "xclaw": { "events": ["command:new"] } } # Specific
```

Rather than:

```yaml
metadata: { "xclaw": { "events": ["command"] } } # General - more overhead
```
...
2. Verify HOOK.md format:
   ...
   ```bash
   cat ~/.xclaw/hooks/my-hook/HOOK.md
...
1. Create hook directory:
   ...
   ```bash
   mkdir -p ~/.xclaw/hooks/my-hook
   mv ./hooks/handlers/my-handler.ts ~/.xclaw/hooks/my-hook/handler.ts
   ```
...
- [Bundled Hooks README](https://github.com/xclaw/xclaw/tree/main/src/hooks/bundled)
