...
xclaw system heartbeat last
xclaw logs --follow
xclaw config get agents.defaults.heartbeat
xclaw channels status --probe
```
...
```bash
xclaw config get agents.defaults.heartbeat.activeHours
xclaw config get agents.defaults.heartbeat.activeHours.timezone
xclaw config get agents.defaults.userTimezone || echo "agents.defaults.userTimezone not set"
xclaw cron list
xclaw logs --follow
```
