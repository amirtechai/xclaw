...
XClaw can convert outbound replies into audio using ElevenLabs, Microsoft, or OpenAI.
It works anywhere XClaw can send audio; Telegram gets a round voice-note bubble.
...
XClaw defaults to Microsoft (unless disabled via
...
TTS config lives under `messages.tts` in `xclaw.json`.
...
- If `provider` is **unset**, XClaw prefers `openai` (if key), then `elevenlabs` (if key),
  otherwise `microsoft`.
...
`~/.xclaw/settings/tts.json`, override with `XCLAW_TTS_PREFS` or
...
- **Microsoft**: uses `microsoft.outputFormat` (default `audio-24khz-48kbitrate-mono-mp3`).
  ...
  - If the configured Microsoft output format fails, XClaw retries with MP3.
...
Discord note: `/tts` is a built-in Discord command, so XClaw registers
...
reply delivery. When the result is Telegram-compatible, XClaw marks it for
