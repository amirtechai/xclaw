...
- With Anthropic API-key auth profiles, XClaw seeds `cacheRetention: "short"` for Anthropic model refs when unset.
...
For `openrouter/anthropic/*` model refs, XClaw injects Anthropic `cache_control` on system/developer prompt blocks to improve prompt-cache reuse.
...
XClaw exposes dedicated cache-trace diagnostics for embedded agent runs.
...
diagnostics:
  cacheTrace:
    enabled: true
    filePath: "~/.xclaw/logs/cache-trace.jsonl" # optional
...
- `filePath`: `$XCLAW_STATE_DIR/logs/cache-trace.jsonl`
...
- `XCLAW_CACHE_TRACE=1` enables cache tracing.
- `XCLAW_CACHE_TRACE_FILE=/path/to/cache-trace.jsonl` overrides output path.
- `XCLAW_CACHE_TRACE_MESSAGES=0|1` toggles full message payload capture.
- `XCLAW_CACHE_TRACE_PROMPT=0|1` toggles prompt text capture.
- `XCLAW_CACHE_TRACE_SYSTEM=0|1` toggles system prompt capture.
