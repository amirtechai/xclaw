---
summary: "How XClaw builds prompt context and reports token usage + costs"
read_when:
...
XClaw tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

XClaw assembles its own system prompt on every run. It includes:
...
For images, XClaw downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from XClaw session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `xclaw status --usage` and `xclaw channels list` show
...
`cacheWrite`. If pricing is missing, XClaw shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. XClaw can
...
Anthropic's 1M context window is currently beta-gated. XClaw can inject the
...
XClaw skips the `context-1m-*` beta header because Anthropic currently
