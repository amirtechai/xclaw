...
XClaw features that can generate provider usage or paid API calls.
...
- `xclaw status --usage` and `xclaw channels list` show provider **usage windows**
...
XClaw can pick up credentials from:
...
`xclaw models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
