...
This is the full reference for `xclaw onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.xclaw/xclaw.json` exists, choose **Keep / Modify / Reset**.
    ...
    - If the config is invalid or contains legacy keys, the wizard stops and asks
      you to run `xclaw doctor` before continuing.
  ...
  <Step title="Model/Auth">
    ...
    - OAuth credentials live in `~/.xclaw/credentials/oauth.json`; auth profiles live in `~/.xclaw/agents/<agentId>/agent/auth-profiles.json` (API keys + OAuth).
    - More detail: [/concepts/oauth](/concepts/oauth)
    <Note>
    Headless/server tip: complete OAuth on a machine with a browser, then copy
    `~/.xclaw/credentials/oauth.json` (or `$XCLAW_STATE_DIR/credentials/oauth.json`) to the
    gateway host.
    </Note>
  </Step>
  <Step title="Workspace">
    - Default `~/.xclaw/workspace` (configurable).
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via `xclaw pairing approve <channel> <code>` or use allowlists.
  </Step>
  <Step title="Web search">
    ...
    - Configure later: `xclaw configure --section web`.
  </Step>
  ...
  <Step title="Health check">
    - Starts the Gateway (if needed) and runs `xclaw health`.
    - Tip: `xclaw status --deep` adds gateway health probes to status output (requires a reachable gateway).
  </Step>
...
```bash
export XCLAW_GATEWAY_TOKEN="your-token"
xclaw onboard --non-interactive \
  --mode local \
  --auth-choice skip \
  --gateway-auth token \
  --gateway-token-ref-env XCLAW_GATEWAY_TOKEN
```
...
- Stores it under `~/.xclaw/tools/signal-cli/<version>/`.
...
Typical fields in `~/.xclaw/xclaw.json`:
...
`xclaw agents add` writes `agents.list[]` and optional `bindings`.

WhatsApp credentials go under `~/.xclaw/credentials/whatsapp/<accountId>/`.
Sessions are stored under `~/.xclaw/agents/<agentId>/sessions/`.
