...
<Steps>
  ...
  <Step title="Use it">
    ...
    ```javascript
    await web_search({ query: "XClaw plugin SDK" });
    ```
...
If no `provider` is set, XClaw checks for API keys in this order and uses
the first one found:
...
<Note>
  ...
  XClaw resolves only the selected provider key -- non-selected SecretRefs
  stay inactive.
</Note>
...
<Tabs>
  <Tab title="Config file">
    Run `xclaw configure --section web` or set the key directly:
  ...
  <Tab title="Environment variable">
    ...
    For a gateway install, put it in `~/.xclaw/.env`.
