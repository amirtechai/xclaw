...
read_when:
  - Running XClaw Gateway in WSL2 while Chrome lives on Windows
...
- XClaw Gateway runs inside WSL2
- Chrome runs on Windows
- browser control must cross the WSL2/Windows boundary

It also covers the layered failure pattern from [issue #39369](https://github.com/xclaw/xclaw/issues/39369): several independent problems can show up at once, which makes the wrong layer look broken first.
...
- XClaw and Chrome are on the same machine
...
- XClaw points a browser profile at the address that is reachable from WSL2
...
If this fails on Windows, XClaw is not the problem yet.
...
Fix that before touching XClaw config.

### Layer 3: Configure the correct browser profile

For raw remote CDP, point XClaw at the address that is reachable from WSL2:
...
- test the same URL with `curl` before expecting XClaw to succeed
...
```bash
xclaw browser open https://example.com --browser-profile remote
xclaw browser tabs --browser-profile remote
```

Good result:

- the tab opens in Windows Chrome
- `xclaw browser tabs` returns the target
...
3. XClaw config: does `browser.profiles.<name>.cdpUrl` use that exact WSL2-reachable address?
...
- only then debug XClaw config or Control UI auth
