...
XClaw can use **Firecrawl** in three ways:
...
- Choosing Firecrawl in onboarding or `xclaw configure --section web` enables the bundled Firecrawl plugin automatically.
...
XClaw always uses `proxy: "auto"` plus `storeInCache: true` for Firecrawl requests.
