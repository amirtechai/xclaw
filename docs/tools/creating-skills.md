...
<Steps>
  <Step title="Create the skill directory">
    ...
    ```bash
    mkdir -p ~/.xclaw/workspace/skills/hello-world
    ```
  ...
  <Step title="Load the skill">
    Start a new session so XClaw picks up the skill:
  ...
  <Step title="Test it">
    ...
    ```bash
    xclaw agent --message "give me a greeting"
    ```
...
| `metadata.xclaw.os`              | No       | OS filter (`["darwin"]`, `["linux"]`, etc.) |
| `metadata.xclaw.requires.bins`   | No       | Required binaries on PATH                   |
| `metadata.xclaw.requires.config` | No       | Required config keys                        |
...
- **Test locally** — use `xclaw agent --message "..."` to test before sharing
...
| `~/.xclaw/skills/`           | Medium     | Shared (all agents)   |
| Bundled (shipped with XClaw) | Lowest     | Global                |
