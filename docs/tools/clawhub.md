---
summary: "ClawHub guide: public registry, native XClaw install flows, and ClawHub CLI workflows"
read_when:
...
ClawHub is the public registry for **XClaw skills and plugins**.

- Use native `xclaw` commands to search/install/update skills and install
...
```bash
xclaw plugins install clawhub:<package>
...
Native `xclaw` commands install into your active workspace and persist source
metadata so later `update` calls can stay on ClawHub.

## What ClawHub is

- A public registry for XClaw skills.
...
4. Users browse, download, and install skills in XClaw.
...
If you want to add new capabilities to your XClaw agent, ClawHub is the easiest way to find and install skills. You do not need to know how the backend works. You can:
...
2. Install a skill:
   - `xclaw skills install <skill-slug>`
3. Start a new XClaw session so it picks up the new skill.
...
## How it fits into XClaw

Native `xclaw skills install` installs into the active workspace `skills/`
directory. `xclaw plugins install clawhub:...` records a normal managed
...
current working directory. If an XClaw workspace is configured, `clawhub`
falls back to that workspace unless you override `--workdir` (or
`CLAWHUB_WORKDIR`). XClaw loads workspace skills from `<workspace>/skills`
and will pick them up in the **next** session. If you already use
`~/.xclaw/skills` or bundled skills, workspace skills take precedence.
...
A skill is a versioned bundle of files that teaches XClaw how to perform a
...
Interested in becoming a moderator? Ask in the XClaw Discord and contact a
...
- `--workdir <dir>`: Working directory (default: current dir; falls back to XClaw workspace).
...
`clawhub sync` scans your current workdir first. If no skills are found, it falls back to known legacy locations (for example `~/xclaw/skills` and `~/.xclaw/skills`). This is designed to find older skill installs without extra flags.
