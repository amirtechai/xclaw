...
- **Debugging blocked tools**: see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `xclaw sandbox explain`.
...
Legacy `agent.*` configs are migrated by `xclaw doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=xclaw-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${XCLAW_STATE_DIR:-$HOME/.xclaw}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
