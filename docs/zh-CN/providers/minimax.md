---
read_when:
  - 你想在 XClaw 中使用 MiniMax 模型
  - 你需要 MiniMax 设置指南
summary: 在 XClaw 中使用 MiniMax M2.5
title: MiniMax
x-i18n:
...
```bash
xclaw plugins enable minimax  # 如果已加载则跳过。
xclaw gateway restart  # 如果 gateway 已在运行，则重启
...
详情请参阅 [MiniMax plugin README](https://github.com/xclaw/xclaw/tree/main/extensions/minimax)。
...
- 使用 `xclaw models list` 和 `xclaw models set minimax/MiniMax-M2.5` 进行切换。
...
- 运行 `xclaw configure` 并选择 **MiniMax M2.5**，或者
