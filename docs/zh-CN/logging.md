...
所有日志配置都在 `~/.xclaw/xclaw.json` 的 `logging` 下。
...
- XClaw 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `xclaw plugins enable diagnostics-otel` 启用插件。
...
- `xclaw.tokens`（计数器，属性：`xclaw.token`、`xclaw.channel`、`xclaw.provider`、`xclaw.model`）
- `xclaw.cost.usd`（计数器，属性：`xclaw.channel`、`xclaw.provider`、`xclaw.model`）
- `xclaw.run.duration_ms`（直方图，属性：`xclaw.channel`、`xclaw.provider`、`xclaw.model`）
- `xclaw.context.tokens`（直方图，属性：`xclaw.context`、`xclaw.channel`、`xclaw.provider`、`xclaw.model`）

消息流：

- `xclaw.webhook.received`（计数器，属性：`xclaw.channel`、`xclaw.webhook`）
- `xclaw.webhook.error`（计数器，属性：`xclaw.channel`、`xclaw.webhook`）
- `xclaw.webhook.duration_ms`（直方图，属性：`xclaw.channel`、`xclaw.webhook`）
- `xclaw.message.queued`（计数器，属性：`xclaw.channel`、`xclaw.source`）
- `xclaw.message.processed`（计数器，属性：`xclaw.channel`、`xclaw.outcome`）
- `xclaw.message.duration_ms`（直方图，属性：`xclaw.channel`、`xclaw.outcome`）

队列 + 会话：

- `xclaw.queue.lane.enqueue`（计数器，属性：`xclaw.lane`）
- `xclaw.queue.lane.dequeue`（计数器，属性：`xclaw.lane`）
- `xclaw.queue.depth`（直方图，属性：`xclaw.lane` 或 `xclaw.channel=heartbeat`）
- `xclaw.queue.wait_ms`（直方图，属性：`xclaw.lane`）
- `xclaw.session.state`（计数器，属性：`xclaw.state`、`xclaw.reason`）
- `xclaw.session.stuck`（计数器，属性：`xclaw.state`）
- `xclaw.session.stuck_age_ms`（直方图，属性：`xclaw.state`）
- `xclaw.run.attempt`（计数器，属性：`xclaw.attempt`）
...
- `xclaw.webhook.error`
  - `xclaw.channel`、`xclaw.webhook`、`xclaw.chatId`、`xclaw.error`
- `xclaw.message.processed`
  - `xclaw.channel`、`xclaw.outcome`、`xclaw.chatId`、`xclaw.messageId`、`xclaw.sessionKey`、`xclaw.sessionId`、`xclaw.reason`
- `xclaw.session.stuck`
  - `xclaw.state`、`xclaw.ageMs`、`xclaw.queueDepth`、`xclaw.sessionKey`、`xclaw.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `xclaw doctor`。
