...
Run the [Agent Client Protocol (ACP)](https://agentclientprotocol.com/) bridge that talks to an XClaw Gateway.

This command speaks ACP over stdio for IDEs and forwards prompts to the Gateway
over WebSocket. It keeps ACP sessions mapped to Gateway session keys.

`xclaw acp` is a Gateway-backed ACP bridge, not a full ACP-native editor
...
| Per-session MCP servers (`mcpServers`)                                | Unsupported | Bridge mode rejects per-session MCP server requests. Configure MCP on the XClaw gateway or agent instead.                                                                                                                                     |
...
# Remote Gateway
xclaw acp --url wss://gateway-host:18789 --token <token>

# Remote Gateway (token from file)
xclaw acp --url wss://gateway-host:18789 --token-file ~/.xclaw/gateway.token

# Attach to an existing session key
xclaw acp --session agent:main:main

# Attach by label (must already exist)
xclaw acp --session-label "support inbox"

# Reset the session key before the first prompt
xclaw acp --session agent:main:main --reset-session
```
...
xclaw acp client --server-args --url wss://gateway-host:18789 --token-file ~/.xclaw/gateway.token

# Override the server command (default: xclaw)
xclaw acp client --server "node" --server-args xclaw.mjs acp --url ws://127.0.0.1:19001
```
...
it to drive an XClaw Gateway session.

1. Ensure the Gateway is running (local or remote).
2. Configure the Gateway target (config or flags).
3. Point your IDE to run `xclaw acp` over stdio.

Example config (persisted):

```bash
xclaw config set gateway.remote.url wss://gateway-host:18789
xclaw config set gateway.remote.token <token>
```
...
```bash
xclaw acp --session agent:main:main
xclaw acp --session agent:design:main
xclaw acp --session agent:qa:bug-123
```
...
XClaw bot over ACP, use `acpx` with its built-in `xclaw` target.

Typical flow:

1. Run the Gateway and make sure the ACP bridge can reach it.
2. Point `acpx xclaw` at `xclaw acp`.
3. Target the XClaw session key you want the coding agent to use.

Examples:

```bash
# One-shot request into your default XClaw ACP session
acpx xclaw exec "Summarize the active XClaw session state."

# Persistent named session for follow-up turns
acpx xclaw sessions ensure --name codex-bridge
acpx xclaw -s codex-bridge --cwd /path/to/repo \
  "Ask my XClaw work agent for recent context relevant to this repo."
```

If you want `acpx xclaw` to target a specific Gateway and session key every
time, override the `xclaw` agent command in `~/.acpx/config.json`:

```json
{
  "agents": {
    "xclaw": {
      "command": "env XCLAW_HIDE_BANNER=1 XCLAW_SUPPRESS_NOTES=1 xclaw acp --url ws://127.0.0.1:18789 --token-file ~/.xclaw/gateway.token --session agent:main:main"
    }
  }
}
```

For a repo-local XClaw checkout, use the direct CLI entrypoint instead of the
dev runner so the ACP stream stays clean. For example:

```bash
env XCLAW_HIDE_BANNER=1 XCLAW_SUPPRESS_NOTES=1 node xclaw.mjs acp ...
```

This is the easiest way to let Codex, Claude Code, or another ACP-aware client
pull contextual information from an XClaw agent without scraping a terminal.
...
In Zed, open the Agent panel and select “XClaw ACP” to start a thread.
...
- Prefer `--token-file`/`--password-file` or environment variables (`XCLAW_GATEWAY_TOKEN`, `XCLAW_GATEWAY_PASSWORD`).
- Gateway auth resolution follows the shared contract used by other Gateway clients:
  - local mode: env (`XCLAW_GATEWAY_*`) -> `gateway.auth.*` -> `gateway.remote.*` fallback only when `gateway.auth.*` is unset (configured-but-unresolved local SecretRefs fail closed)
...
- ACP runtime backend child processes receive `XCLAW_SHELL=acp`, which can be used for context-specific shell/profile rules.
- `xclaw acp client` sets `XCLAW_SHELL=acp-client` on the spawned bridge process.

### `acp client` options

- `--cwd <dir>`: working directory for the ACP session.
- `--server <command>`: ACP server command (default: `xclaw`).
