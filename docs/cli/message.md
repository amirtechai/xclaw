---
summary: "CLI reference for `xclaw message` (send + channel actions)"
read_when:
...
```
xclaw message <subcommand> [flags]
```
...
- On cache miss, XClaw will attempt a live directory lookup when the provider supports it.
...
- `xclaw message` resolves supported channel SecretRefs before running the selected action.
- Resolution is scoped to the active action target when possible:
  ...
  - when `--account` is omitted, XClaw does not force a `default` account SecretRef scope
...
```
xclaw message send --channel discord \
...
```
xclaw message poll --channel discord \
  --target channel:123 \
  --poll-question "Snack?" \
...
```
xclaw message poll --channel telegram \
  --target @mychat \
  --poll-question "Lunch?" \
  --poll-option Pizza --poll-option Sushi \
...
```
xclaw message poll --channel msteams \
...
```
xclaw message react --channel slack \
...
```
xclaw message react --channel signal \
...
```
xclaw message send --channel telegram --target @mychat --message "Choose:" \
...
```bash
xclaw message send --channel telegram --target @mychat \
