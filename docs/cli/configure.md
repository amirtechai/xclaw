---
summary: "CLI reference for `xclaw configure` (interactive configuration prompts)"
read_when:
...
Tip: `xclaw config` without a subcommand opens the same wizard. Use
`xclaw config get|set|unset` for non-interactive edits.
...
```bash
xclaw configure
xclaw configure --section model --section channels
```
