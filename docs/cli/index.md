---
summary: "XClaw CLI reference for `xclaw` commands, subcommands, and options"
read_when:
...
- `--dev`: isolate state under `~/.xclaw-dev` and shift default ports.
- `--profile <name>`: isolate state under `~/.xclaw-<name>`.
- `--no-color`: disable ANSI colors.
- `--update`: shorthand for `xclaw update` (source installs only).
...
XClaw uses a lobster palette for CLI output.
...
```
xclaw [--dev] [--profile <name>] <command>
  setup
  onboard
  configure
  config
...
Note: plugins can add additional top-level commands (for example `xclaw voicecall`).

## Security

- `xclaw security audit` — audit config + local state for common security foot-guns.
- `xclaw security audit --deep` — best-effort live Gateway probe.
- `xclaw security audit --fix` — tighten safe defaults and chmod state/config.

## Secrets

- `xclaw secrets reload` — re-resolve refs and atomically swap the runtime snapshot.
- `xclaw secrets audit` — scan for plaintext residues, unresolved refs, and precedence drift (`--allow-exec` to execute exec providers during audit).
- `xclaw secrets configure` — interactive helper for provider setup + SecretRef mapping + preflight/apply (`--allow-exec` to execute exec providers during preflight and exec-containing apply flows).
- `xclaw secrets apply --from <plan.json>` — apply a previously generated plan (`--dry-run` supported; use `--allow-exec` to permit exec providers in dry-run and exec-containing write plans).

## Plugins

Manage extensions and their config:

- `xclaw plugins list` — discover plugins (use `--json` for machine output).
- `xclaw plugins inspect <id>` — show details for a plugin (`info` is an alias).
- `xclaw plugins install <path|.tgz|npm-spec|plugin@marketplace>` — install a plugin (or add a plugin path to `plugins.load.paths`).
- `xclaw plugins marketplace list <marketplace>` — list marketplace entries before install.
- `xclaw plugins enable <id>` / `disable <id>` — toggle `plugins.entries.<id>.enabled`.
- `xclaw plugins doctor` — report plugin load errors.
...
- `xclaw memory status` — show index stats.
- `xclaw memory index` — reindex memory files.
- `xclaw memory search "<query>"` (or `--query "<query>"`) — semantic search over memory.
...
- `--workspace <dir>`: agent workspace path (default `~/.xclaw/workspace`).
...
Non-interactive config helpers (get/set/unset/file/validate). Running `xclaw config` with no
...
- `config set --dry-run`: validate assignments without writing `xclaw.json` (exec SecretRef checks are skipped by default).
...
- `channels status`: check gateway reachability and channel health (`--probe` runs extra checks; use `xclaw health` or `xclaw status --deep` for gateway health probes).
- Tip: `channels status` prints warnings with suggested fixes when it can detect common misconfigurations (then points you to `xclaw doctor`).
- `channels logs`: show recent channel logs from the gateway log file.
- `channels add`: wizard-style setup when no flags are passed; flags switch to non-interactive mode.
  - When adding a non-default account to a channel still using single-account top-level config, XClaw moves account-scoped values into `channels.<channel>.accounts.default` before writing the new account.
...
```bash
xclaw channels add --channel telegram --account alerts --name "Alerts Bot" --token $TELEGRAM_BOT_TOKEN
xclaw channels add --channel discord --account work --name "Work Bot" --token $DISCORD_BOT_TOKEN
xclaw channels remove --channel discord --account work --delete
...
- `xclaw message send --target +15555550123 --message "Hi"`
- `xclaw message poll --channel discord --target channel:123 --poll-question "Snack?" --poll-option Pizza --poll-option Sushi`
...
Binding specs use `channel[:accountId]`. When `accountId` is omitted, XClaw may resolve account scope via channel defaults/plugin hooks; otherwise it is a channel binding without explicit account scope.
...
XClaw can surface provider usage/quota when OAuth/API creds are available.

Surfaces:

- `/status` (adds a short provider usage line when available)
- `xclaw status --usage` (prints full provider breakdown)
...
- `gateway status` also surfaces legacy or extra gateway services when it can detect them (`--deep` adds system-level scans). Profile-named XClaw services are treated as first-class and aren't flagged as "extra".
...
`xclaw models` is an alias for `models status`.
...
- `node` resolves gateway auth from env/config (no `--token`/`--password` flags): `XCLAW_GATEWAY_TOKEN` / `XCLAW_GATEWAY_PASSWORD`, then `gateway.auth.*`. In local mode, node host intentionally ignores `gateway.remote.*`; in `gateway.mode=remote`, `gateway.remote.*` participates per remote precedence rules.
- Node-host auth resolution only honors `XCLAW_GATEWAY_*` env vars.
...
Browser control CLI (dedicated Chrome/Brave/Edge/Chromium). See [`xclaw browser`](/cli/browser) and the [Browser tool](/tools/browser).
