---
summary: "CLI reference for `xclaw onboard` (interactive onboarding)"
read_when:
...
```bash
xclaw onboard
xclaw onboard --flow quickstart
xclaw onboard --flow manual
xclaw onboard --mode remote --remote-url wss://gateway-host:18789
```

For plaintext private-network `ws://` targets (trusted networks only), set
`XCLAW_ALLOW_INSECURE_PRIVATE_WS=1` in the onboarding process environment.
...
- `--install-daemon` starts the managed gateway install path first. Without it, you must already have a local gateway running, for example `xclaw gateway run`.
...
- Fastest first chat: `xclaw dashboard` (Control UI, no channel setup).
