---
summary: "CLI reference for `xclaw config` (get/set/unset/file/validate)"
read_when:
...
Config helpers for non-interactive edits in `xclaw.json`: get/set/unset/validate
values by path and print the active config file. Run without a subcommand to
open the configure wizard (same as `xclaw configure`).
...
xclaw config set secrets.providers.vaultfile --provider-source file --provider-path /etc/xclaw/secrets.json --provider-mode json
...
xclaw config validate
xclaw config validate --json
```
...
xclaw config set gateway.port 19001 --strict-json
xclaw config set channels.whatsapp.groups '["*"]' --strict-json
```

## `config set` modes

`xclaw config set` supports four assignment styles:

1. Value mode: `xclaw config set <path> <value>`
2. SecretRef builder mode:

```bash
xclaw config set channels.discord.token \
...
```bash
xclaw config set secrets.providers.vault \
  --provider-source exec \
  --provider-command /usr/local/bin/xclaw-vault \
  --provider-arg read \
...
```bash
xclaw config set --batch-file ./config-set.batch.json --dry-run
```
...
xclaw config set secrets.providers.vaultfile \
  '{"source":"file","path":"/etc/xclaw/secrets.json","mode":"json"}' \
  --strict-json
```
...
Use `--dry-run` to validate changes without writing `xclaw.json`.
...
- `config file`: Print the active config file path (resolved from `XCLAW_CONFIG_PATH` or default location).
