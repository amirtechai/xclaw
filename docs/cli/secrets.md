---
summary: "CLI reference for `xclaw secrets` (reload, audit, configure, apply)"
read_when:
...
Use `xclaw secrets` to manage SecretRefs and keep the active runtime snapshot healthy.
...
Scan XClaw state for:

- plaintext secret storage
- unresolved refs
- precedence drift (`auth-profiles.json` credentials shadowing `xclaw.json` refs)
...
xclaw secrets audit
xclaw secrets audit --check
xclaw secrets audit --json
xclaw secrets audit --allow-exec
```
...
```bash
xclaw secrets configure
xclaw secrets configure --plan-out /tmp/xclaw-secrets-plan.json
xclaw secrets configure --apply --yes
xclaw secrets configure --providers-only
xclaw secrets configure --skip-provider-setup
xclaw secrets configure --agent ops
xclaw secrets configure --json
```
...
- `configure` targets secret-bearing fields in `xclaw.json` plus `auth-profiles.json` for the selected agent scope.
...
- On Windows, if ACL verification is unavailable for a provider path, XClaw fails closed. For trusted paths only, set `allowInsecurePath: true` on that provider to bypass path security checks.
...
xclaw secrets apply --from /tmp/xclaw-secrets-plan.json --json
```
...
- `xclaw.json` (SecretRef targets + provider upserts/deletes)
- `auth-profiles.json` (provider-target scrubbing)
- legacy `auth.json` residues
- `~/.xclaw/.env` known secret keys whose values were migrated
