...
read_when:
  - Setting up XClaw for the first time
...
XClaw reads an optional <Tooltip tip="JSON5 supports comments and trailing commas">**JSON5**</Tooltip> config from `~/.xclaw/xclaw.json`.

If the file is missing, XClaw uses safe defaults. Common reasons to add a config:
...
<Tip>
**New to configuration?** Start with `xclaw onboard` for interactive setup, or check out the [Configuration Examples](/gateway/configuration-examples) guide for complete copy-paste configs.
</Tip>
...
<Tabs>
  <Tab title="Interactive wizard">
    ```bash
    xclaw onboard       # full onboarding flow
    xclaw configure     # config wizard
    ```
  </Tab>
  ...
  <Tab title="Direct edit">
    Edit `~/.xclaw/xclaw.json` directly. The Gateway watches the file and applies changes automatically (see [hot reload](#config-hot-reload)).
  </Tab>
</Tabs>
...
<Warning>
XClaw only accepts configurations that fully match the schema. Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start**. The only root-level exception is `$schema` (string), so editors can attach JSON Schema metadata.
</Warning>

When validation fails:

- The Gateway does not boot
- Only diagnostic commands work (`xclaw doctor`, `xclaw logs`, `xclaw health`, `xclaw status`)
- Run `xclaw doctor` to see exact issues
- Run `xclaw doctor --fix` (or `--yes`) to apply repairs
...
<AccordionGroup>
  ...
  <Accordion title="Enable relay-backed push for official iOS builds">
    Relay-backed push is configured in `xclaw.json`.
    ...
    ```bash
    xclaw config set gateway.push.apns.relay.baseUrl https://relay.example.com
    ```
    ...
    - `XCLAW_APNS_RELAY_BASE_URL` and `XCLAW_APNS_RELAY_TIMEOUT_MS` still work as temporary env overrides.
    - `XCLAW_APNS_RELAY_ALLOW_HTTP=true` remains a loopback-only development escape hatch; do not persist HTTP relay URLs in config.
...
The Gateway watches `~/.xclaw/xclaw.json` and applies changes automatically — no manual restart needed for most settings.
...
<AccordionGroup>
  <Accordion title="config.apply (full replace)">
    ...
    <Warning>
    `config.apply` replaces the **entire config**. Use `config.patch` for partial updates, or `xclaw config set` for single keys.
    </Warning>
    ...
    ```bash
    xclaw gateway call config.get --params '{}'  # capture payload.hash
    xclaw gateway call config.apply --params '{
      "raw": "{ agents: { defaults: { workspace: \"~/.xclaw/workspace\" } } }",
      "baseHash": "<hash>",
  ...
  <Accordion title="config.patch (partial update)">
    ...
    ```bash
    xclaw gateway call config.patch --params '{
...
XClaw reads env vars from the parent process plus:

- `.env` from the current working directory (if present)
- `~/.xclaw/.env` (global fallback)
...
<Accordion title="Shell env import (optional)">
  If enabled and expected keys aren't set, XClaw runs your login shell and imports only the missing keys:
...
Env var equivalent: `XCLAW_LOAD_SHELL_ENV=1`
</Accordion>
...
```json5
{
  gateway: { auth: { token: "${XCLAW_GATEWAY_TOKEN}" } },
