...
- Use the macOS app’s **Remote over SSH** mode (Settings → General → “XClaw runs”).
...
- `xclaw health` and `xclaw status --deep` now reach the remote gateway via `ws://127.0.0.1:18789`.
- `xclaw gateway {status,health,send,agent,call}` can also target the forwarded URL via `--url` when needed.

Note: replace `18789` with your configured `gateway.port` (or `--port`/`XCLAW_GATEWAY_PORT`).
...
- URL override safety:
  ...
  - Env URL overrides (`XCLAW_GATEWAY_URL`) may use env credentials only (`XCLAW_GATEWAY_TOKEN` / `XCLAW_GATEWAY_PASSWORD`).
- Local mode defaults:
  - token: `XCLAW_GATEWAY_TOKEN` -> `gateway.auth.token` -> `gateway.remote.token` (remote fallback applies only when local auth token input is unset)
  - password: `XCLAW_GATEWAY_PASSWORD` -> `gateway.auth.password` -> `gateway.remote.password` (remote fallback applies only when local auth password input is unset)
- Remote mode defaults:
  - token: `gateway.remote.token` -> `XCLAW_GATEWAY_TOKEN` -> `gateway.auth.token`
  - password: `XCLAW_GATEWAY_PASSWORD` -> `gateway.remote.password` -> `gateway.auth.password`
- Node-host local-mode exception: `gateway.remote.token` / `gateway.remote.password` are ignored.
- Remote probe/status token checks are strict by default: they use `gateway.remote.token` only (no local token fallback) when targeting remote mode.
- Gateway env overrides use `XCLAW_GATEWAY_*` only.
...
- Plaintext `ws://` is loopback-only by default. For trusted private networks,
  set `XCLAW_ALLOW_INSECURE_PRIVATE_WS=1` on the client process as break-glass.
