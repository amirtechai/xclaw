...
read_when:
  - Running XClaw behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of XClaw
...
- You run XClaw behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. XClaw checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. XClaw extracts the user identity from the configured header
...
- XClaw can stay on loopback HTTP behind the proxy.
...
If XClaw itself serves HTTPS directly (no TLS-terminating proxy), set:
...
```yaml
routes:
  - from: https://xclaw.example.com
    to: http://xclaw-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
`xclaw security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update XClaw config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `xclaw security audit` and review findings
