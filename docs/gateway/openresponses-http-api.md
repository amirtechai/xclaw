...
XClaw’s Gateway can serve an OpenResponses-compatible `POST /v1/responses` endpoint.
...
`xclaw agent`), so routing/permissions/config match your Gateway.
...
- select agents with `model: "xclaw:<agentId>"`, `model: "agent:<agentId>"`, or `x-xclaw-agent-id`
- use `x-xclaw-session-key` for explicit session routing
