...
- `xclaw gateway status` shows `Runtime: running` and `RPC probe: ok`.
- `xclaw doctor` reports no blocking config/service issues.
- `xclaw channels status --probe` shows connected/ready channels.
...
xclaw logs --follow
xclaw models status
xclaw config get agents.defaults.models
```
...
| ---------------------------- | -------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `AUTH_TOKEN_MISSING`         | Client did not send a required shared token.             | Paste/set token in the client and retry. For dashboard paths: `xclaw config get gateway.auth.token` then paste into Control UI settings.                          |
...
| `PAIRING_REQUIRED`           | Device identity is known but not approved for this role. | Approve pending request: `xclaw devices list` then `xclaw devices approve <requestId>`.                                                                        |
...
- `Gateway start blocked: set gateway.mode=local` → local gateway mode is not enabled. Fix: set `gateway.mode="local"` in your config (or run `xclaw configure`). If you are running XClaw via Podman using the dedicated `xclaw` user, the config lives at `~xclaw/.xclaw/xclaw.json`.
...
xclaw approvals get --node <idOrNameOrIp>
xclaw logs --follow
xclaw status
```
...
xclaw browser start --browser-profile xclaw
