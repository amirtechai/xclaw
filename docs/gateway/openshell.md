---
title: OpenShell
summary: "Use OpenShell as a managed sandbox backend for XClaw agents"
read_when:
...
OpenShell is a managed sandbox backend for XClaw. Instead of running Docker
containers locally, XClaw delegates sandbox lifecycle to the `openshell` CLI,
...
- XClaw Gateway running on the host
...
2. Restart the Gateway. On the next agent turn, XClaw creates an OpenShell
...
- You edit files locally outside XClaw and want those changes visible in the
...
- When the sandbox is first created, XClaw seeds the remote workspace from
...
- XClaw does **not** sync remote changes back into the local workspace.
...
Important: if you edit files on the host outside XClaw after the initial seed,
the remote sandbox does **not** see those changes. Use
`xclaw sandbox recreate` to re-seed.
...
| `from`                    | `string`                 | `"xclaw"`  | Sandbox source for first-time create                  |
...
1. XClaw calls `openshell sandbox create` (with `--from`, `--gateway`,
   `--policy`, `--providers`, `--gpu` flags as configured).
2. XClaw calls `openshell sandbox ssh-config <name>` to get SSH connection
...
- [Sandbox CLI](/cli/sandbox) -- `xclaw sandbox` commands
