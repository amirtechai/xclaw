...
- During heartbeat runs, XClaw treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `xclaw sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, XClaw skips the heartbeat run entirely (no model call).
...
```bash
xclaw system event --text "Check for urgent follow-ups" --mode now
```
