...
XClaw supports additive SecretRefs so supported credentials do not need to be stored as plaintext in configuration.
...
- `gateway.auth.token` SecretRef is inactive for startup auth resolution when `XCLAW_GATEWAY_TOKEN` is set, because env token input wins for that runtime.
...
When onboarding runs in interactive mode and you choose SecretRef storage, XClaw runs preflight validation before saving:
...
- Set `allowSymlinkCommand: true` to allow symlink command paths (for example Homebrew shims). XClaw validates the resolved target path.
  ...
  secrets: {
    providers: {
      onepassword_openai: {
        ...
        args: ["read", "op://Personal/XClaw QA API Key/password"],
        passEnv: ["HOME"],
        jsonOnly: false,
      },
    },
  ...
  secrets: {
    providers: {
      vault_openai: {
        ...
        allowSymlinkCommand: true, // required for Homebrew symlinked binaries
        trustedDirs: ["/opt/homebrew"],
        args: ["kv", "get", "-field=OPENAI_API_KEY", "secret/xclaw"],
...
- XClaw resolves these refs during sandbox activation, not lazily during each SSH call.
...
- `REF_SHADOWED` (audit finding when `auth-profiles.json` credentials take precedence over `xclaw.json` refs)
...
When reload-time activation fails after a healthy state, XClaw enters degraded secrets state.
...
- Strict command paths (for example `xclaw memory` remote-memory paths and `xclaw qr --remote`) read from the active snapshot and fail fast when a required SecretRef is unavailable.
- Read-only command paths (for example `xclaw status`, `xclaw status --all`, `xclaw channels status`, `xclaw channels resolve`, `xclaw security audit`, and read-only doctor/config repair flows) also prefer the active snapshot, but degrade instead of aborting when a targeted SecretRef is unavailable in that command path.
...
- Snapshot refresh after backend secret rotation is handled by `xclaw secrets reload`.
...
```bash
xclaw secrets audit --check
xclaw secrets configure
xclaw secrets audit --check
```
...
- plaintext values at rest (`xclaw.json`, `auth-profiles.json`, `.env`, and generated `agents/*/agent/models.json`)
- plaintext sensitive provider header residues in generated `models.json` entries
- unresolved refs
- precedence shadowing (`auth-profiles.json` taking priority over `xclaw.json` refs)
- legacy residues (`auth.json`, OAuth reminders)

Exec note:

- By default, audit skips exec SecretRef resolvability checks to avoid command side effects.
- Use `xclaw secrets audit --allow-exec` to execute exec providers during audit.
...
- lets you select supported secret-bearing fields in `xclaw.json` plus `auth-profiles.json` for one agent scope
...
- `xclaw secrets configure --providers-only`
- `xclaw secrets configure --skip-provider-setup`
- `xclaw secrets configure --agent <id>`
...
```bash
xclaw secrets apply --from /tmp/xclaw-secrets-plan.json
xclaw secrets apply --from /tmp/xclaw-secrets-plan.json --allow-exec
xclaw secrets apply --from /tmp/xclaw-secrets-plan.json --dry-run
xclaw secrets apply --from /tmp/xclaw-secrets-plan.json --dry-run --allow-exec
```
...
XClaw intentionally does not write rollback backups containing historical plaintext secret values.
