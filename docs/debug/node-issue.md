...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in XClaw
...
Running XClaw via Node with `tsx` fails at startup with:

```
[xclaw] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `xclaw status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch xclaw.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node xclaw.mjs status` works on Node 25.
