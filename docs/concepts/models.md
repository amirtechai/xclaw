...
XClaw selects models in this order:
...
- `agents.defaults.models` is the allowlist/catalog of models XClaw can use (plus aliases).
...
- If you omit the provider, XClaw treats the input as an alias or a model for the **default provider** (only works when there is no `/` in the model ID).
...
xclaw models list
xclaw models status
xclaw models set <provider/model>
xclaw models set-image <provider/model>

xclaw models aliases list
xclaw models aliases add <alias> <provider/model>
xclaw models aliases remove <alias>

xclaw models fallbacks list
xclaw models fallbacks add <provider/model>
xclaw models fallbacks remove <provider/model>
xclaw models fallbacks clear

xclaw models image-fallbacks list
xclaw models image-fallbacks add <provider/model>
xclaw models image-fallbacks remove <provider/model>
xclaw models image-fallbacks clear
```

`xclaw models` (no subcommand) is a shortcut for `models status`.
...
`xclaw models scan` inspects OpenRouter’s **free model catalog** and can
...
agent directory (default `~/.xclaw/agents/<agentId>/agent/models.json`). This file
...
Marker persistence is source-authoritative: XClaw writes markers from the active source config snapshot (pre-resolution), not from resolved runtime secret values.
This applies whenever XClaw regenerates `models.json`, including command-driven paths like `xclaw agent`.
