...
- `/usage cost` in chats: local cost summary aggregated from XClaw session logs.
- CLI: `xclaw status --usage` prints a full per-provider breakdown.
- CLI: `xclaw channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
