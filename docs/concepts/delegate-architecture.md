---
summary: "Delegate architecture: running XClaw as a named agent on behalf of an organization"
...
Goal: run XClaw as a **named delegate** — an agent with its own identity that acts "on behalf of" people in an organization. The agent never impersonates a human. It sends, reads, and schedules under its own account with explicit delegation permissions.

This extends [Multi-Agent Routing](/concepts/multi-agent) from personal use into organizational deployments.

## What is a delegate?

A **delegate** is an XClaw agent that:
...
XClaw's default mode is a **personal assistant** — one human, one agent. Delegates extend this to organizations:
...
2. **Scope control**: the identity provider enforces what the delegate can access, independent of XClaw's own tool policy.
...
{
  id: "delegate",
  workspace: "~/.xclaw/workspace-delegate",
  tools: {
...
- Cron run history: `~/.xclaw/cron/runs/<jobId>.jsonl`
- Session transcripts: `~/.xclaw/agents/delegate/sessions`
- Identity provider audit logs (Exchange, Google Workspace)

All delegate actions flow through XClaw's session store. For compliance, ensure these logs are retained and reviewed.
...
```bash
xclaw agents add delegate
```

This creates:

- Workspace: `~/.xclaw/workspace-delegate`
- State: `~/.xclaw/agents/delegate/agent`
- Sessions: `~/.xclaw/agents/delegate/sessions`
...
```json5
{
  agents: {
    list: [
      { id: "main", workspace: "~/.xclaw/workspace" },
      {
        id: "delegate",
        workspace: "~/.xclaw/workspace-delegate",
        tools: {
...
~/.xclaw/agents/delegate/agent/auth-profiles.json
```
...
```json5
{
  agents: {
    list: [
      { id: "main", default: true, workspace: "~/.xclaw/workspace" },
      {
        id: "org-assistant",
        name: "[Organization] Assistant",
        workspace: "~/.xclaw/workspace-org",
        agentDir: "~/.xclaw/agents/org-assistant/agent",
