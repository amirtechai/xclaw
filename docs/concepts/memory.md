---
title: "Memory"
summary: "How XClaw memory works (workspace files + automatic memory flush)"
read_when:
...
XClaw memory is **plain Markdown in the agent workspace**. The files are the
...
- `MEMORY.md` (optional)
  ...
  - If both `MEMORY.md` and `memory.md` exist at the workspace root, XClaw loads both (deduplicated by realpath so symlinks pointing to the same file are not injected twice).
  - **Only load in the main, private session** (never in group contexts).

These files live under the workspace (`agents.defaults.workspace`, default
`~/.xclaw/workspace`). See [Agent workspace](/concepts/agent-workspace) for the full layout.

## Memory tools

XClaw exposes two agent-facing tools for these Markdown files:
...
When a session is **close to auto-compaction**, XClaw triggers a **silent,
...
XClaw can build a small vector index over `MEMORY.md` and `memory/*.md` so
