...
read_when:
  - You want to understand how XClaw assembles model context
...
A **context engine** controls how XClaw builds model context for each run.
It decides which messages to include, how to summarize older history, and how
to manage context across subagent boundaries.

XClaw ships with a built-in `legacy` engine. Plugins can register
...
cat ~/.xclaw/xclaw.json | jq '.plugins.slots.contextEngine'
```

### Installing a context engine plugin

Context engine plugins are installed like any other XClaw plugin. Install
...
xclaw plugins install -l ./my-context-engine
```
...
Every time XClaw runs a model prompt, the context engine participates at
...
XClaw currently calls one subagent lifecycle hook:
...
The `assemble` method can return a `systemPromptAddition` string. XClaw
...
The built-in `legacy` engine preserves XClaw's original behavior:
...
- `estimatedTokens` (required, `number`) — the engine's estimate of total
  tokens in the assembled context. XClaw uses this for compaction threshold
...
- `true` — the engine owns compaction behavior. XClaw disables Pi's built-in
...
`ownsCompaction: false` does **not** mean XClaw automatically falls back to
...
- **Delegating mode** — set `ownsCompaction: false` and have `compact()` call
  `delegateCompactionToRuntime(...)` from `xclaw/plugin-sdk/core` to use
  XClaw's built-in compaction behavior.
...
XClaw resolves when it needs a context engine.

## Relationship to compaction and memory

- **Compaction** is one responsibility of the context engine. The legacy engine
  delegates to XClaw's built-in summarization. Plugin engines can implement
...
- Use `xclaw doctor` to verify your engine is loading correctly.
...
- Engine errors are logged and surfaced in diagnostics. If a plugin engine
  fails to register or the selected engine id cannot be resolved, XClaw
  does not fall back automatically; runs fail until you fix the plugin or
  switch `plugins.slots.contextEngine` back to `"legacy"`.
- For development, use `xclaw plugins install -l ./my-engine` to link a
