...
read_when:
  - Setting up Twitch chat integration for XClaw
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. XClaw connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
xclaw plugins install @xclaw/twitch
```

Local checkout (when running from a git repo):

```bash
xclaw plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `XCLAW_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "xclaw", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use XCLAW_TWITCH_ACCESS_TOKEN env var)
...
```bash
XCLAW_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
