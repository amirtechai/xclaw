...
read_when:
  - Setting up Zalo Personal for XClaw
...
Status: experimental. This integration automates a **personal Zalo account** via native `zca-js` inside XClaw.
...
- Install via CLI: `xclaw plugins install @xclaw/zalouser`
- Or from a source checkout: `xclaw plugins install ./extensions/zalouser`
...
- `xclaw pairing list zalouser`
- `xclaw pairing approve zalouser <code>`
...
- On startup, XClaw resolves group/user names in allowlists to IDs and logs the mapping.
...
- When a group message is skipped because mention is required, XClaw stores it as pending group history and includes it on the next processed group message.
...
Accounts map to `zalouser` profiles in XClaw state. Example:
...
- XClaw sends a typing event before dispatching a reply (best-effort).
...
- For inbound messages that include event metadata, XClaw sends delivered + seen acknowledgements (best-effort).
...
- Re-login: `xclaw channels logout --channel zalouser && xclaw channels login --channel zalouser`
...
- The channel now runs fully in XClaw without external CLI binaries.
