---
summary: "Synology Chat webhook setup and XClaw config"
read_when:
  - Setting up Synology Chat with XClaw
...
```bash
xclaw plugins install ./extensions/synology-chat
```
...
1. Install and enable the Synology Chat plugin.
   - `xclaw onboard` now shows Synology Chat in the same channel setup list as `xclaw channels add`.
   - Non-interactive setup: `xclaw channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
2. In Synology Chat integrations:
   - Create an incoming webhook and copy its URL.
   - Create an outgoing webhook with your secret token.
3. Point the outgoing webhook URL to your XClaw gateway:
   - `https://gateway-host/webhook/synology` by default.
   - Or your custom `channels.synology-chat.webhookPath`.
4. Finish setup in XClaw.
   - Guided: `xclaw onboard`
   - Direct: `xclaw channels add --channel synology-chat --token <token> --url <incoming-webhook-url>`
...
- Pairing approvals work with:
  - `xclaw pairing list synology-chat`
  - `xclaw pairing approve synology-chat <CODE>`
...
```bash
xclaw message send --channel synology-chat --target 123456 --text "Hello from XClaw"
xclaw message send --channel synology-chat --target synology-chat:123456 --text "Hello again"
```
...
Give each enabled account a distinct `webhookPath`. XClaw now rejects duplicate exact paths
