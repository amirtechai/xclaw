...
Matrix is the Matrix channel plugin for XClaw.
It uses the official `matrix-js-sdk` and supports DMs, rooms, threads, media, reactions, polls, location, and E2EE.

## Plugin required

Matrix is a plugin and is not bundled with core XClaw.

Install from npm:

```bash
xclaw plugins install @xclaw/matrix
```

Install from a local checkout:

```bash
xclaw plugins install ./extensions/matrix
```
...
```bash
xclaw channels add
xclaw configure --section channels
```
...
- To resolve room names before saving them, use `xclaw channels resolve --channel matrix "Project Room"`.
...
```json5
{
  channels: {
    matrix: {
      ...
      deviceName: "XClaw Gateway",
    },
  },
}
```

Matrix stores cached credentials in `~/.xclaw/credentials/matrix/`.
...
By default, Matrix messages from other configured XClaw Matrix accounts are ignored.
...
- XClaw still ignores messages from the same Matrix user ID to avoid self-reply loops.
- Matrix does not expose a native bot flag here; XClaw treats "bot-authored" as "sent by another configured Matrix account on this XClaw gateway".
...
```bash
xclaw matrix verify status --include-recovery-key --json
```
...
```bash
xclaw matrix verify bootstrap --verbose
```

Force a fresh cross-signing identity reset before bootstrapping:

```bash
xclaw matrix verify bootstrap --force-reset-cross-signing
```
...
```bash
xclaw matrix verify device "<your-recovery-key>" --verbose
```
...
```bash
xclaw matrix verify backup restore
```

Verbose restore diagnostics:

```bash
xclaw matrix verify backup restore --verbose
```
...
```bash
xclaw matrix verify status --account assistant
xclaw matrix verify backup restore --account assistant
xclaw matrix devices list --account assistant
```
...
XClaw treats this Matrix device as verified only when it is verified by your own cross-signing identity.
In practice, `xclaw matrix verify status --verbose` exposes three trust signals:
...
Local trust by itself is not enough for XClaw to treat the device as fully verified.

### What bootstrap does

`xclaw matrix verify bootstrap` is the repair and setup command for encrypted Matrix accounts.
...
If the homeserver requires interactive auth to upload cross-signing keys, XClaw tries the upload without auth first, then with `m.login.dummy`, then with `m.login.password` when `channels.matrix.password` is configured.
...
If you intentionally want to discard the current room-key backup and start a new backup baseline for future messages, use `xclaw matrix verify backup reset --yes`.
...
If startup finds broken bootstrap state and `channels.matrix.password` is configured, XClaw can attempt a stricter repair path.
If the current device is already owner-signed, XClaw preserves that identity instead of resetting it automatically.

Upgrading from the previous public Matrix plugin:

- XClaw automatically reuses the same Matrix account, access token, and device identity when possible.
- Before any actionable Matrix migration changes run, XClaw creates or reuses a recovery snapshot under `~/Backups/xclaw-migrations/`.
- If you use multiple Matrix accounts, set `channels.matrix.defaultAccount` before upgrading from the old flat-store layout so XClaw knows which account should receive that shared legacy state.
- If the previous plugin stored a Matrix room-key backup decryption key locally, startup or `xclaw doctor --fix` will import it into the new recovery-key flow automatically.
...
- If the Matrix access token changes later for the same account, homeserver, and user, XClaw now prefers reusing the most complete existing token-hash storage root instead of starting from an empty Matrix state directory.
- On the next gateway start, backed-up room keys are restored automatically into the new crypto store.
- If the old plugin had local-only room keys that were never backed up, XClaw will warn clearly. Those keys cannot be exported automatically from the previous rust crypto store, so some old encrypted history may remain unavailable until recovered manually.
...
`~/.xclaw/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/`.
...
When the token changes but the account identity stays the same, XClaw reuses the best existing
...
XClaw currently provides that in Node by:
...
Under XClaw's security model, the gateway host and local XClaw state directory are already inside the trusted operator boundary, so this is primarily an operational durability concern rather than a separate remote trust boundary.

Planned improvement:

- add SecretRef support for persistent Matrix key material so recovery keys and related store-encryption secrets can be sourced from XClaw secrets providers instead of only local files
...
Incoming verification requests from another Matrix client are tracked and auto-accepted by XClaw.
For self-verification flows, XClaw also starts the SAS flow automatically when emoji verification becomes available and confirms its own side.
For verification requests from another Matrix user/device, XClaw auto-accepts the request and then waits for the SAS flow to proceed normally.
...
XClaw does not auto-accept self-initiated duplicate flows blindly. Startup skips creating a new request when a self-verification request is already pending.

Verification protocol/system notices are not forwarded to the agent chat pipeline, so they do not produce `NO_REPLY`.

### Device hygiene

Old XClaw-managed Matrix devices can accumulate on the account and make encrypted-room trust harder to reason about.
List them with:

```bash
xclaw matrix devices list
```

Remove stale XClaw-managed devices with:

```bash
xclaw matrix devices prune-stale
```

### Direct Room Repair

If direct-message state gets out of sync, XClaw can end up with stale `m.direct` mappings that point at old solo rooms instead of the live DM. Inspect the current mapping for a peer with:

```bash
xclaw matrix direct inspect --user-id @alice:example.org
```

Repair it with:

```bash
xclaw matrix direct repair --user-id @alice:example.org
```
...
Ack reactions use the standard XClaw resolution order:
...
```bash
xclaw pairing list matrix
xclaw pairing approve matrix <CODE>
```

If an unapproved Matrix user keeps messaging you before approval, XClaw reuses the same pending pairing code and may send a reminder reply again after a short cooldown instead of minting a new code.
...
Set `defaultAccount` when you want XClaw to prefer one named Matrix account for implicit routing, probing, and CLI operations.
If you configure multiple named accounts, set `defaultAccount` or pass `--account <id>` for CLI commands that rely on implicit account selection.
Pass `--account <id>` to `xclaw matrix verify ...` and `xclaw matrix devices ...` when you want to override that implicit selection for one command.

## Private/LAN homeservers

By default, XClaw blocks private/internal Matrix homeservers for SSRF protection unless you
...
Matrix accepts these target forms anywhere XClaw asks you for a room or user target:
...
- `autoJoinAllowlist`: rooms/aliases allowed when `autoJoin` is `allowlist`. Alias entries are resolved to room IDs during invite handling; XClaw does not trust alias state claimed by the invited room.
