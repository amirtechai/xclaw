...
```bash
xclaw plugins install @xclaw/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
xclaw plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "XClaw" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `xclaw pairing list nextcloud-talk`
  - `xclaw pairing approve nextcloud-talk <CODE>`
