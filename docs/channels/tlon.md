...
Tlon is a decentralized messenger built on Urbit. XClaw connects to your Urbit ship and can
...
```bash
xclaw plugins install @xclaw/tlon
```

Local checkout (when running from a git repo):

```bash
xclaw plugins install ./extensions/tlon
```
...
By default, XClaw blocks private/internal hostnames and IP ranges for SSRF protection.
...
Use these with `xclaw message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, XClaw replies in-thread.
