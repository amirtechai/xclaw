---
summary: "Mattermost bot setup and XClaw config"
read_when:
...
```bash
xclaw plugins install @xclaw/mattermost
```

Local checkout (when running from a git repo):

```bash
xclaw plugins install ./extensions/mattermost
```
...
4. Configure XClaw and start the gateway.
...
Native slash commands are opt-in. When enabled, XClaw registers `oc_*` slash commands via
...
- If `callbackUrl` is omitted, XClaw derives one from gateway host/port + `callbackPath`.
...
- Reachability requirement: the callback endpoint must be reachable from the Mattermost server.
  - Do not set `callbackUrl` to `localhost` unless Mattermost runs on the same host/network namespace as XClaw.
  - Do not set `callbackUrl` to your Mattermost base URL unless that URL reverse-proxies `/api/channels/mattermost/command` to XClaw.
  - A quick check is `curl https://<gateway-host>/api/channels/mattermost/command`; a GET should return `405 Method Not Allowed` from XClaw, not `404`.
...
- Approve via:
  - `xclaw pairing list mattermost`
  - `xclaw pairing approve mattermost <CODE>`
...
Use these target formats with `xclaw message send` or cron/webhooks:
...
XClaw resolves them **user-first**:

- If the ID exists as a user (`GET /api/v4/users/<id>` succeeds), XClaw sends a **DM** by resolving the direct channel via `/api/v4/channels/direct`.
...
When XClaw sends to a Mattermost DM target and needs to resolve the direct channel first, it
...
- If `interactions.callbackBaseUrl` is omitted, XClaw derives the callback URL from
...
- Reachability rule: the button callback URL must be reachable from the Mattermost server.
  `localhost` only works when Mattermost and XClaw run on the same host/network namespace.
...
1. Derive the secret from the bot token:
   `HMAC-SHA256(key="xclaw-mattermost-interactions", data=botToken)`
...
secret = hmac.new(
    b"xclaw-mattermost-interactions",
...
`xclaw message send` and cron/webhook deliveries.
