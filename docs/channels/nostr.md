...
read_when:
  - You want XClaw to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables XClaw to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- Onboarding (`xclaw onboard`) and `xclaw channels add` list optional channel plugins.
...
```bash
xclaw plugins install @xclaw/nostr
```

Use a local checkout (dev workflows):

```bash
xclaw plugins install --link <path-to-xclaw>/extensions/nostr
```
...
xclaw channels add --channel nostr --private-key "$NOSTR_PRIVATE_KEY" --relay-urls "wss://relay.damus.io,wss://relay.primal.net"
```
  ...
  channels: {
    nostr: {
      ...
      profile: {
        ...
        nip05: "xclaw@example.com",
        lud16: "xclaw@example.com",
      },
    },
  },
}
