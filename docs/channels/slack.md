...
```
  ...
  <Tab title="HTTP Events API mode">
    <Steps>
      ...
      <Step title="Configure XClaw HTTP mode">
...
<Tabs>
  <Tab title="DM policy">
    ...
    Pairing in DMs uses `xclaw pairing approve slack <code>`.
...
- The interactive callback values are XClaw-generated opaque tokens, not raw agent-authored values.
- If generated interactive blocks would exceed Slack Block Kit limits, XClaw falls back to the original text reply instead of sending an invalid blocks payload.
...
`typingReaction` adds a temporary reaction to the inbound Slack message while XClaw is processing a reply, then removes it when the run finishes. This is a useful fallback when Slack native assistant typing is unavailable, especially in DMs.
...
```json
{
  "display_information": {
    "name": "XClaw",
    "description": "Slack connector for XClaw"
  },
  "features": {
    "bot_user": {
      "display_name": "XClaw",
    ...
    "slash_commands": [
      {
        "command": "/xclaw",
        "description": "Send a message to XClaw",
        "should_escape": false
      }
    ]
  },
...
XClaw supports Slack native text streaming via the Agents and AI Apps API.
...
- If streaming fails mid-reply, XClaw falls back to normal delivery for remaining payloads.
