...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects XClaw to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.

---

## Bundled plugin

Feishu ships bundled with current XClaw releases, so no separate plugin install
is required.
...
```bash
xclaw plugins install @xclaw/feishu
```
...
If you just installed XClaw, run onboarding:
...
2. Configuring app credentials in XClaw
...
1. You already ran `xclaw channels add` for Feishu
2. The gateway is running (`xclaw gateway status`)
...
```bash
xclaw pairing approve feishu <CODE>
```
...
2. Run `xclaw logs --follow` and look for `chat_id`
...
2. Run `xclaw logs --follow` and look for `open_id`
...
| Command                    | Description                   |
| -------------------------- | ----------------------------- |
| `xclaw gateway status`  | Show gateway status           |
| `xclaw gateway install` | Install/start gateway service |
| `xclaw gateway stop`    | Stop gateway service          |
| `xclaw gateway restart` | Restart gateway service       |
| `xclaw logs --follow`   | Tail gateway logs             |
...
4. Check logs: `xclaw logs --follow`
...
5. Ensure the gateway is running: `xclaw gateway status`
6. Check logs: `xclaw logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.xclaw/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.xclaw/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
