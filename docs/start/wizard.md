...
CLI onboarding is the **recommended** way to set up XClaw on macOS,
...
`xclaw dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
</Info>
...
`xclaw configure --section web`. Docs: [Web tools](/tools/web).
</Tip>
...
2. **Workspace** — Location for agent files (default `~/.xclaw/workspace`). Seeds bootstrap files.
...
If the config is invalid or contains legacy keys, onboarding asks you to run `xclaw doctor` first.
</Note>
...
Use `xclaw agents add <name>` to create a separate agent with its own workspace,
...
- Default workspaces follow `~/.xclaw/workspace-<agentId>`.
...
- CLI command reference: [`xclaw onboard`](/cli/onboard)
