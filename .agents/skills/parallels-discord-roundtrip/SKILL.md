...
```bash
export XCLAW_PARALLELS_DISCORD_TOKEN="$(
  ssh peters-mac-studio-1 'jq -r ".channels.discord.token" ~/.xclaw/xclaw.json' | tr -d '\n'
)"

pnpm test:parallels:macos \
  --discord-token-env XCLAW_PARALLELS_DISCORD_TOKEN \
...
- Use the `xclaw` wrapper for guest `message send/read`; `node xclaw.mjs message ...` does not expose the lazy message subcommands the same way.
...
- Per-phase logs: `/tmp/xclaw-parallels-smoke.*`
...
- host inbound nonce appears in `xclaw message read` output
