const SETTINGS_KEY_PREFIX = "xclaw.control.settings.v1:";
const LEGACY_SETTINGS_KEY = "xclaw.control.settings.v1";
const LEGACY_TOKEN_SESSION_KEY = "xclaw.control.token.v1";
const TOKEN_SESSION_KEY_PREFIX = "xclaw.control.token.v1:";
...
function deriveDefaultGatewayUrl(): { pageUrl: string; effectiveUrl: string } {
  ...
  const configured =
    ...
    typeof window.__XCLAW_CONTROL_UI_BASE_PATH__ === "string" &&
    window.__XCLAW_CONTROL_UI_BASE_PATH__.trim();
...
function persistSettings(next: UiSettings) {
  ...
  try {
    ...
    const raw =
      storage?.getItem(scopedKey) ??
      storage?.getItem(SETTINGS_KEY_PREFIX + "default") ??
      storage?.getItem("xclaw.control.settings.v1");
    if (raw) {
      const parsed = JSON.parse(raw) as PersistedUiSettings;
