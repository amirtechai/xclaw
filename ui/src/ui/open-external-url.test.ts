...
describe("resolveSafeExternalUrl", () => {
  const baseHref = "https://xclaw.ai/chat";
  ...
  it("allows relative URLs resolved against the current origin", () => {
    expect(resolveSafeExternalUrl("/assets/pic.png", baseHref)).toBe(
      "https://xclaw.ai/assets/pic.png",
    );
  });

  it("allows blob URLs", () => {
    expect(resolveSafeExternalUrl("blob:https://xclaw.ai/abc-123", baseHref)).toBe(
      "blob:https://xclaw.ai/abc-123",
    );
  });
