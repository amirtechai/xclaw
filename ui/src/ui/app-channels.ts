...
export async function handleWhatsAppStart(host: XClawApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: XClawApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: XClawApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: XClawApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: XClawApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: XClawApp): string {
...
function resolveGatewayHttpAuthHeader(host: XClawApp): string | null {
...
function buildGatewayHttpHeaders(host: XClawApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: XClawApp) {
...
export function handleNostrProfileToggleAdvanced(host: XClawApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: XClawApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: XClawApp) {
  const state = host.nostrProfileFormState;
