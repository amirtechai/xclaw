...
const XCLAW_OPENSHELL_E2E = process.env.XCLAW_E2E_OPENSHELL === "1";
const XCLAW_OPENSHELL_E2E_TIMEOUT_MS = 12 * 60_000;
const XCLAW_OPENSHELL_COMMAND =
  process.env.XCLAW_E2E_OPENSHELL_COMMAND?.trim() || "openshell";
...
RUN echo "xclaw-openshell-e2e" > /opt/openshell-e2e-marker.txt
...
describe("openshell sandbox backend e2e", () => {
  it.runIf(process.platform !== "win32" && XCLAW_OPENSHELL_E2E)(
    "creates a remote-canonical sandbox through OpenShell and executes over SSH",
    { timeout: XCLAW_OPENSHELL_E2E_TIMEOUT_MS },
    async () => {
      if (!(await dockerReady())) {
        return;
      }
      if (!(await commandAvailable(XCLAW_OPENSHELL_COMMAND))) {
        return;
      }

      const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "xclaw-openshell-e2e-"));
      ...
      const gatewayName = `xclaw-e2e-${scopeSuffix}`;
      const scopeKey = `session:openshell-e2e-deny:${scopeSuffix}`;
      const allowSandboxName = `xclaw-policy-allow-${scopeSuffix}`;
      ...
      const pluginConfig = resolveOpenShellPluginConfig({
        command: XCLAW_OPENSHELL_COMMAND,
        gateway: gatewayName,
        from: dockerfilePath,
        mode: "remote",
        autoProviders: false,
      ...
      try {
        ...
        expect(stdout).toContain("xclaw-openshell-e2e");
