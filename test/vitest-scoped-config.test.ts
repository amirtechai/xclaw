...
describe("resolveVitestIsolation", () => {
  ...
  it("restores isolate mode when explicitly requested", () => {
    expect(resolveVitestIsolation({ XCLAW_TEST_ISOLATE: "1" })).toBe(true);
    expect(resolveVitestIsolation({ XCLAW_TEST_NO_ISOLATE: "0" })).toBe(true);
    expect(resolveVitestIsolation({ XCLAW_TEST_NO_ISOLATE: "false" })).toBe(true);
  });
});
...
describe("scoped vitest configs", () => {
  ...
  it("loads channel include overrides from XCLAW_VITEST_INCLUDE_FILE", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "xclaw-vitest-channels-"));
    try {
      ...
      const config = createChannelsVitestConfig({
        XCLAW_VITEST_INCLUDE_FILE: includeFile,
      });
